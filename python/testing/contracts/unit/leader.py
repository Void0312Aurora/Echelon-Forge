...
def _check_leader_training_env(spec: dict[str, Any]) -> tuple[bool, str]:
    try:
...
def _check_leader_policy_generalization(spec: dict[str, Any]) -> tuple[bool, str]:
    try:
        import gymnasium  # noqa: F401
    except ModuleNotFoundError as exc:
        raise ContractSkipped("gymnasium not installed") from exc
...
def _check_leader_phase_manager_approach_arm(spec: dict[str, Any]) -> tuple[bool, str]:
    import ef_py
...
_LEADER_CONTRACT_CHECKS = {
    "leader_training_env": _check_leader_training_env,
    "leader_policy_generalization": _check_leader_policy_generalization,
    "leader_phase_manager_approach_arm": _check_leader_phase_manager_approach_arm,
}


def run_leader_contract(check_kind: str, spec: dict[str, Any]) -> tuple[bool, str] | None:
    handler = _LEADER_CONTRACT_CHECKS.get(check_kind)
    if handler is None:
        return None
    return handler(spec)
