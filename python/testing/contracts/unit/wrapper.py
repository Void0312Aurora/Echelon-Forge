...
def _check_wrapper_scripted_mode_sequence(spec: dict[str, Any]) -> tuple[bool, str]:
    try:
...
def _check_wrapper_action_processing_sequence(spec: dict[str, Any]) -> tuple[bool, str]:
    try:
        import gymnasium as gym
    except ModuleNotFoundError as exc:
        raise ContractSkipped("gymnasium not installed") from exc
...
_WRAPPER_CONTRACT_CHECKS = {
    "wrapper_scripted_mode_sequence": _check_wrapper_scripted_mode_sequence,
    "wrapper_action_processing_sequence": _check_wrapper_action_processing_sequence,
}


def run_wrapper_contract(check_kind: str, spec: dict[str, Any]) -> tuple[bool, str] | None:
    handler = _WRAPPER_CONTRACT_CHECKS.get(check_kind)
    if handler is None:
        return None
    return handler(spec)
