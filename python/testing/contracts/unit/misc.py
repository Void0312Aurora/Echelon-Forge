...
def _check_takeoff_safe_action_bias(spec: dict[str, Any]) -> tuple[bool, str]:
    try:
...
def _check_replay_expert_actions(spec: dict[str, Any]) -> tuple[bool, str]:
    import tempfile
    import numpy as np
...
def _check_continuous_waypoint_template_geometry(spec: dict[str, Any]) -> tuple[bool, str]:
...
def _check_landing_entity_spawn_randomization(spec: dict[str, Any]) -> tuple[bool, str]:
    import numpy as np
...
def _check_env_config_resolution(spec: dict[str, Any]) -> tuple[bool, str]:
    import argparse
...
def _check_takeoff_curriculum_auto_gear_agl(spec: dict[str, Any]) -> tuple[bool, str]:
    import numpy as np
...
_MISC_CONTRACT_CHECKS = {
    "takeoff_safe_action_bias": _check_takeoff_safe_action_bias,
    "scripted_stable_flight_rudder_sign": _check_scripted_stable_flight_rudder_sign,
    "replay_expert_actions": _check_replay_expert_actions,
    "continuous_waypoint_template_geometry": _check_continuous_waypoint_template_geometry,
    "landing_entity_spawn_randomization": _check_landing_entity_spawn_randomization,
    "scripted_takeoff_takeoff2_throttle": _check_scripted_takeoff_takeoff2_throttle,
    "scripted_takeoff_clearance_hold": _check_scripted_takeoff_clearance_hold,
    "scripted_landing_controller": _check_scripted_landing_controller,
    "env_config_resolution": _check_env_config_resolution,
    "takeoff_curriculum_auto_gear_agl": _check_takeoff_curriculum_auto_gear_agl,
}


def run_misc_contract(check_kind: str, spec: dict[str, Any]) -> tuple[bool, str] | None:
    handler = _MISC_CONTRACT_CHECKS.get(check_kind)
    if handler is None:
        return None
    return handler(spec)
