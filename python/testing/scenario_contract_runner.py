...
def run_unit_regression_contract(spec_path: str) -> tuple[bool, str]:
    ...
    if check_kind == "wrapper_scripted_mode_sequence":
        ...
        import python.rl.wrappers as wrappers
    ...
    if check_kind == "kernel_flight_regression":
        ...
        controller_kind = str(spec.get("controller_kind", "")).strip().lower()
        scenario_path = resolve_repo_path(str(spec["scenario"]))
        seed = int(spec.get("seed", 0))
        checks = dict(spec.get("checks", {}) or {})
        ...
        if controller_kind == "midpoint_env_action":
            ...
            for idx in list(spec.get("midpoint_indices", []) or []):
            ...
            if final_ias <= final_ias_min:
                return False, f"expected IAS > {final_ias_min:.1f}, got {final_ias:.3f}"
            return True, f"kernel midpoint ground-roll contract passed with IAS={final_ias:.2f}"

        if controller_kind == "manual_takeoff":
            target_pitch = float(spec.get("target_pitch_deg", 15.0))
            success_alt = float(checks.get("success_alt_min", 300.0))
            success_speed = float(checks.get("success_speed_min", 150.0))
            max_steps = int(spec.get("max_steps", 2000))
            for step in range(max_steps):
                inst = sim.get_instrument_state(agent_id)
                ...
                pa.brake_left = False
                pa.brake_right = False
                if speed < float(spec.get("rotation_speed_mps", 100.0)):
                    pa.stick_pitch = 0.0
                else:
                    pa.stick_pitch = float(np.clip((target_pitch - pitch) * float(spec.get("pitch_gain", 0.05)), -1.0, 1.0))
                pa.gear_handle = 0.0 if alt > float(spec.get("gear_up_alt_m", 30.0)) else 1.0
                sim.set_pilot_action(agent_id, pa)
                sim.step()
                if alt > success_alt and speed > success_speed:
                    return True, f"kernel manual takeoff contract passed in {step + 1} steps"
            return False, f"manual takeoff did not reach alt>{success_alt:.1f} and speed>{success_speed:.1f}"
        ...
        if controller_kind == "stable_level_hold":
            ...
            pa.brake_left = False
            pa.brake_right = False
            thr = float(spec.get("initial_throttle", 0.6))
            alt_int = 0.0
            min_alt = float("inf")
            max_abs_roll = 0.0
            max_abs_pitch = 0.0
            max_abs_g = 0.0
            steps = int(round(float(spec.get("duration_s", 200.0)) / dt))
            for _ in range(steps):
                inst = sim.get_instrument_state(agent_id)
                truth = sim.get_agent_observation(agent_id)
                if float(truth.health) <= 0.0:
                    return False, "aircraft crashed during level-flight stability test"
                alt = float(inst.alt_baro)
                vvi = float(inst.vvi)
                ias = float(inst.ias)
                pitch = float(inst.pitch)
                ...
                for value in (alt, vvi, ias, pitch, roll, p, q, g_load):
                    if not _finite(value):
                        return False, f"non-finite instrument value during stable flight: {value!r}"
            ...
            if min_alt <= float(checks.get("min_alt_min", 800.0)):
                return False, f"altitude dipped too low: min_alt={min_alt:.1f}m"
            if max_abs_roll >= float(checks.get("max_abs_roll_max", 10.0)):
                return False, f"excessive roll during level flight: {max_abs_roll:.1f}deg"
            if max_abs_pitch >= float(checks.get("max_abs_pitch_max", 15.0)):
                return False, f"excessive pitch during level flight: {max_abs_pitch:.1f}deg"
            if max_abs_g >= float(checks.get("max_abs_g_max", 3.0)):
                return False, f"excessive G-load during level flight: {max_abs_g:.2f}"
            return True, (
        ...
        if controller_kind == "takeoff_then_stable_hold":
            ...
            for _ in range(max_steps):
                ...
                truth = sim.get_agent_observation(agent_id)
                if float(truth.health) <= 0.0:
                    return False, "aircraft crashed during takeoff/stable-flight test"
                for value in (
                    float(inst.alt_baro),
                    ...
                    if not _finite(value):
                        return False, f"non-finite instrument value: {value!r}"
                if stage == "takeoff":
                    pa.throttle = 1.0
                    pa.stick_roll = 0.0
                    pa.rudder = 0.0
                    if float(inst.ias) < float(spec.get("rotation_speed_mps", 100.0)):
                        pa.stick_pitch = 0.0
                    else:
                        pitch_err = float(spec.get("target_takeoff_pitch_deg", 13.0)) - float(inst.pitch)
                        pa.stick_pitch = float(np.clip(pitch_err * float(spec.get("takeoff_pitch_gain", 0.05)), -1.0, 1.0))
                    pa.gear_handle = 0.0 if float(inst.alt_baro) > float(spec.get("gear_up_alt_m", 30.0)) else 1.0
                ...
                sim.step()
            if stage != "stable":
                return False, "did not reach stable flight phase within max_steps"
            stable_steps_min = int(round(float(checks.get("stable_steps_min_s", 40.0)) / dt))
            if stable_steps < stable_steps_min:
                return False, f"stable phase too short: {stable_steps} steps"
            if min_alt_stable <= float(checks.get("min_alt_stable_min", 150.0)):
                return False, f"altitude dipped too low during stable flight: min_alt={min_alt_stable:.1f}m"
            if max_abs_roll >= float(checks.get("max_abs_roll_max", 20.0)):
                return False, f"excessive roll in stable phase: {max_abs_roll:.1f}deg"
            if max_abs_pitch >= float(checks.get("max_abs_pitch_max", 20.0)):
                return False, f"excessive pitch in stable phase: {max_abs_pitch:.1f}deg"
            return True, (
        ...
        raise ValueError(f"Unknown kernel_flight_regression controller_kind: {controller_kind}")
    ...
    if check_kind == "task_order_and_mission_link":
        ...
        order.station_type = getattr(ef_py.StationType, str(order_spec.get("station_type", "Racetrack")))
        ...
        report.report_type = getattr(ef_py.CommMsgType, str(report_spec.get("report_type", "REP_ON_STATION")))
        report.sender_id = int(report_spec.get("sender_id", entity_id))
        report.task_id = int(report_spec.get("task_id", order.task_id))
        report.phase_id = int(getattr(ef_py.LeaderPhase, str(report_spec.get("phase_id", "OnStation"))))
    ...
    if check_kind == "leader_phase_manager_approach_arm":
        ...
        loader = FakeLoader()
        mgr = RuleBasedLeaderPhaseManager(terminal_waypoint_count=int(spec.get("terminal_waypoint_count", 2)))
