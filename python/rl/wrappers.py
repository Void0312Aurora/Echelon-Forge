...
class MultiTimescaleActionController:
    """
    Stateful action preprocessor shared by the gym wrapper and the world-batch vec env.

    This keeps the current training semantics intact while allowing `WorldBatchVecEnv`
    to apply the same multi-timescale / scripted-residual action shaping without
    falling back to the standard per-env wrapper path.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._loader_getter = loader_getter
        self._dt_getter = dt_getter
    ...
    def _get_loader(self):
        if callable(self._loader_getter):
            try:
                return self._loader_getter()
            except Exception:
                return None
        return None

    def _get_reset_dt(self) -> float:
        if callable(self._dt_getter):
            try:
                return float(self._dt_getter())
            except Exception:
                return 0.05
        return 0.05

    @staticmethod
    def _obs_alt_agl_m(obs) -> float | None:
        try:
