...
class ScriptedLandingController(BaseScriptedController):
    """
    ...
    def step(self, obs: dict) -> np.ndarray:
        ...
        course_err = wrap_deg(float(self._course_ref_deg) - ground_track)
        heading_err = wrap_deg(float(self._course_ref_deg) - heading)
        ...
        if rollout_mode:
            trk_err_rollout = wrap_deg(float(ground_track) - float(self._course_ref_deg))
            yaw_err_rollout = wrap_deg(float(heading) - float(self._course_ref_deg))
