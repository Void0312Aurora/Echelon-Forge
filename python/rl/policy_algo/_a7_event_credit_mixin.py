...
class _A7EventCreditMixin:
    ...
    def _get_a7_first_event_rollout_history(
        self, n_envs: int
    ) -> list[list[_A7FirstEventRolloutRow]]:
        env_count = max(1, int(n_envs))
    ...
    def _a7_first_event_history_has_pending_shadow(
        self, rows: list[_A7FirstEventRolloutRow]
    ) -> bool:
        if not rows:
            return False
    ...
    def _first_event_policy_margin_loss(
        ...
        *,
        ...
        base_loss = compute_first_event_policy_margin_loss(
            ...
            coef=(
                float(self.a7_event_policy_margin_coef) if coef is None else float(max(0.0, coef))
            ),
