...
- `agent_layer="execution"` 是单策略 air/execution 路径。维护中的 production setup 使用 `runtime.world_batch_vec_env=true` 和 `python.rl.runtime.world_batch_vec_env.WorldBatchVecEnv`；raw `UniversalEnv` / SB3 vec-env compatibility 不再通过维护中的 training config 接受。
