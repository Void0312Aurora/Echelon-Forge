...
本页记录仓库内的着陆任务接口以及维护中的场景和配置入口。如文本与代码、
场景、配置 schema 或聚焦测试冲突，以后者为准。
...
- 本页是已实现接口的 reference，不证明 learned landing policy 已通过验收。
- reward hooks 只描述仓库行为，不构成特定机型试飞保真度声明。
- 侧风、拉平优化、复飞决策和更广课程均需独立后续验收。
