...
Verification boundary: 已复核所有者路径、生命周期、当前裁定摘要与命名门禁路由；
日期化代码引证仍只对其记录的基线负责。
基线提交：`dd292f4b`

状态：维护中的所有者本地信息状态登记册。本文源自已完成
[统一架构计划](../../plan/archive/unified_architecture_program_completed_20260727/README.zh.md)的 T8（信息状态架构）切片，记录：(a) 维护面观测/奖励消费者普查；(b) G4 层声明机制的落地范围；(c) 策略路径上的 World Truth 直读，逐条裁定。参照
...
全文使用的 G4 词汇即
[仿真系统架构设计](../standards/simulation_system_architecture_design.md)§3 的权威六层信息状态集合，逐字沿用 `python/rl/runtime/world_batch/core.py` 中 I32 阶段契约白名单（由 `tests/world_batch/test_world_batch_core.py` 钉住）：World Truth、Sensed State、Track State、Shared Tactical Picture、Agent Observation、Decision Belief。
...
- [统一架构计划](../../plan/archive/unified_architecture_program_completed_20260727/README.zh.md)
