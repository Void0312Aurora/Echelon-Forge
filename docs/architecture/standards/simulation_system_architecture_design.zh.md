...
语言：
- 英文规范版：[simulation_system_architecture_design.md](simulation_system_architecture_design.md)
- 中文伴随版：`simulation_system_architecture_design.zh.md`
...
Verification boundary: 已复核所有者路径、生命周期、权威关系与链接；技术基线仍以
2026-07-20 已接受设计及其可执行门禁为界。
...
历史 `WP0-WP6` 标签记录建立运行时与 backend-profile 边界的任务谱系，
不再作为当前工作入口。运行时围绕一条规范化仿真生命周期和一个带时钟域的
执行图组织，但执行图只是投影，不是系统本体。
...
- `PlatformFamily`：飞机、舰艇、潜艇、早期 ground 或未来空间单位。
- `MotionFamily`：空气动力、舰艇运动、潜艇运动，以及在 scoped land-runtime gate
  将当前 schema 证据提升之后的 ground mobility。
