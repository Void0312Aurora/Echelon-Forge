...
| `P2-B Default Provider Migration` | future engine owner | n/a | Move default model, factory, event-store, bridge, and release-service construction behind native providers and a kernel builder; publish the first production resolved-plan identity. | `src/core/engine/**`, `src/models/**` composition entries, approved provider files, focused tests, bounded evidence DTO joins | system-family split, Cordis package, Node host | default behavior/replay baseline, lifecycle tests, no dangling provider capture, identity roundtrip/mismatch tests | default compatibility profile constructs the current kernel without concrete model construction in `SimulationKernel`, and the run exports stable requested/resolved identity | after P2-A; serial with P3-A when engine registration files overlap | 3 + 1 repair | ready |
| `P2-C Cordis Default-Profile Vertical Slice` | future Cordis integration owner | n/a | Add a minimal repository-owned Cordis package/profile that consumes the typed runtime projection and emits the canonical default P1-B request for native revalidation and realization. | bounded `packages/cordis-runtime/**`, package manifests/lockfiles, canonical fixtures, conformance tests, subproject docs | Node-API hosting, system-family split, external plugins, full SDK ergonomics, hot-path services | Cordis package tests, schema validation, canonical-byte/hash parity, native rejection parity, offline-native regression | Cordis and native compatibility producers emit byte-equivalent default requests; the same native compiler/root realizes both | after P2-B production identity is stable; before broad profile/ecosystem work | 2 + 1 repair | planned |
...
| `P3-B Capability And Profile Projection` | future cross-domain integration owner | n/a | Lower typed capability/policy requests and compatibility profile names into owner-admitted model/component/system/stage bundles. | owner-approved projection/profile contracts, domain integration tests, composition fixtures | making air/naval/ground labels the permanent ontology, promoting domain maturity, implementing missing behavior | projection validation, forbidden dependency guards, owner contract suites | capability requirements are primary; named domain profiles are explicit compatibility bundles that cannot bypass owner standards | after P3-A declarations; owner rows may parallel when write sets are disjoint | 2 + 1 repair per bundle family | planned |
...
| `P5-A Composition Evidence Expansion` | future evidence owner | n/a | Extend the P2-B/P2-C requested/resolved identity baseline with provider versions, graph hash, backend profile, host mode, catalog lock, and scope generation through diagnostics/replay/comparison contracts. | evidence/replay contracts, facade diagnostics, schema generators, focused tests | claiming full replay for unsupported backend/state surfaces | DTO freshness, roundtrip, mismatch rejection, deterministic hash fixtures | maintained runs identify their realized composition and replay rejects unexplained mismatch | after P2-B and relevant P3/P4 joins | 2 + 1 repair | planned |
| `P6-A Cordis Package Maturation` | future Cordis integration owner | n/a | Extend the accepted P2-C producer with repository-owned profiles, configuration overlays, diagnostics, provenance, dependency resolution, and plugin SDK ergonomics. | approved `packages/cordis-runtime/**`, workspace manifests/lockfiles, fixtures, docs | simulation stepping in JavaScript, arbitrary external native plugins, replacing owner admission | package tests, projection/schema conformance, canonical parity, provenance and diagnostics tests | maintained Cordis profiles emit only owner-admitted requests accepted identically by the native validator | after P2-C and applicable P3/P4 owner contracts | 3 + 1 repair | planned |
...
- P0, contract freeze, shared runtime ownership, acceptance, and closure remain
  serial.
- P2-B and P2-C are separate bounded slices but serial on their shared default
  profile and evidence contract. P2-C is required for overall program closure;
  P2-B may retain its own bounded native acceptance.
...
- dispatch P2-C after P2-B to prove the intended Cordis context/profile/plugin
  model against the actual default provider path, not against fixtures alone.
