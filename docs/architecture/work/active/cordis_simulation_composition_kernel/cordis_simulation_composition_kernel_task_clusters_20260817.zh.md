...
本计划可以引入 Cordis 控制面、原生组合内核、版本化 manifest、provider/lifecycle
基础设施、受治理 system contribution、composition evidence 和可选 Node host。它必须
保持维护中的仿真架构：原生确定性 stage execution、单一 runtime truth path、backend
capability admission、replay identity，以及离线 C++/Python 运行。
...
| `P2-B Default Provider Migration` | future engine owner | n/a | 把默认 model、factory、event store、bridge、release service 构造迁入 native provider 和 kernel builder。 | `src/core/engine/**`、`src/models/**` composition 入口、准入 provider 文件、聚焦测试 | system-family 拆分、Node/Cordis 集成 | 默认行为/replay 基线、lifecycle test、无悬垂 provider capture | compatibility profile 在 `SimulationKernel` 不构造具体 model 的情况下构造当前 kernel | P2-A 后；与 P3-A 共享 engine registration 文件时串行 | 3 + 1 repair | ready |
...
| `P3-B Domain Composition Profiles` | future cross-domain integration owner | n/a | 定义 minimal、common、air、naval、ground、combined profile，显式进行 capability/extension-point admission。 | owner 批准的 profile/config contract、domain integration test、composition fixture | 提升领域成熟度、实现缺失领域行为 | profile validation、forbidden dependency guard、domain contract suite | 每个 profile 具有显式 contribution 且不能绕开 owner standard | P3-A 声明后；写集不重叠时各领域可并行 | 每个 profile family 2 + 1 repair | planned |
...
| `P5-A Composition Evidence` | future evidence owner | n/a | 通过 diagnostics/replay/comparison 导出 requested/resolved manifest identity、provider version、graph hash、backend profile、host mode 和 scope generation。 | evidence/replay contract、facade diagnostics、schema generator、聚焦测试 | 对不支持的 backend/state 宣称完整 replay | DTO freshness、roundtrip、mismatch rejection、deterministic hash fixture | 维护中的 run 可识别 realized composition，replay 拒绝无法解释的 mismatch | P1-B 且至少 P2-B/P3-A 后 | 2 + 1 repair | planned |
| `P6-A Cordis Control-Plane Package` | future Cordis integration owner | n/a | 增加仓库拥有的 Cordis package、plugin descriptor、配置加载、依赖解析和 canonical manifest 输出。 | 获批的 `packages/cordis-runtime/**`、Node workspace/lockfile、fixture、文档 | JavaScript 仿真步进、任意外部 native plugin | package test、schema conformance、与 native fixture 的 canonical-byte parity | Cordis 只输出被 native validator 同样接受的 manifest | P1-B 与 native validator 稳定后 | 3 + 1 repair | planned |
...
| `P7-A Host And Batch Parity` | future integration owner | n/a | 证明 native、Python、Node host 实现等价准入 composition，并保持 batch-scale determinism 与性能预算。 | integration test、benchmark/probe、evidence package、有界修复 | 通过调参掩盖语义 mismatch、无关优化 | host parity matrix、replay comparison、大 batch 内存/启动/吞吐 | 冻结 profile 通过正确性和另行批准的回归预算 | P4-A/P5-A/P6-B 后 | 2 + 1 repair | planned |
| `P8-A Migration Closure` | main integration owner | n/a | 删除被取代的 setter/构造真值，提升稳定规则，记录 accepted scope，拆分残余并同步 index/archive。 | 受影响 composition code、architecture standard/reference、本项目 acceptance/archive、owner index | 删除历史证据、静默扩大插件准入 | 全验收矩阵、link/bilingual audit、caller inventory、compatibility proof | 不再存在双 composition path，所有残余都有维护 owner | 最终串行任务簇 | 2 + 1 repair | planned |
...
- 实现 P2-B，同一切片不启动 system family、backend、Cordis 或 binding 迁移；
- 保持既有默认 behavior/replay baseline，并在声明 provider replacement 安全前移除
  environment-model raw capture。

后续：provider/system migration、backend admission、composition evidence、Cordis package、
Node host、plugin SDK ergonomics 和开发工具。
