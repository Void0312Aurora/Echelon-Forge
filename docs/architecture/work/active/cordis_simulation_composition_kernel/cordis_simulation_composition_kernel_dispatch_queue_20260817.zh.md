...
| 4 | `P2-A Native Lifecycle Kernel` | pass / independently repaired | P1-A/P1-B gate 已通过 | 隔离原生 composition library、聚焦 C++ test、architecture guard 与 CMake/CI wiring | 普通 MSVC 与 ASan build 均为 14 tests/381 assertions；hash、typed-scope、lifecycle-state、rollback、串行化 replacement rebuild、重入 lifetime、plugin/factory identity、稳定 service ABI、handover、invalidation 与 deterministic validation 证据 |
