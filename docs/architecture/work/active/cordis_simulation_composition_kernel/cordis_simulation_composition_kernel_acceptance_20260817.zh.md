...
已接受的 P2-A 边界仅包括独立 `ef_composition` library，以及 closed JSON ingestion、
原生 revalidation、transactional scoped construction、generation invalidation、rollback、
rebuild 与逆序 disposal 行为。普通 MSVC 与 MSVC AddressSanitizer 各自通过 8 个 test、
149 个 assertion。默认 provider 集成、reset/replay parity、system capture 修复、canonical
byte 重算和全部 Cordis/host 声明仍未完成。
