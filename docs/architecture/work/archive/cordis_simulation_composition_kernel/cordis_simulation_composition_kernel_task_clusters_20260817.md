...
| `P5-A Composition Evidence Expansion` | main thread + independent reviewer | `gpt-5.6-sol` / `max`: P0/P1/P2 = 0/0/0 | Bind the accepted default request/manifest/lock/profile/provider/backend/graph realization to per-world scope evidence and maintained replay/comparison admission. | evidence/replay contracts, facade diagnostics, schema generators, focused tests, CMake/CI, bounded docs | claiming full replay for unsupported backend/state surfaces or caller-language attestation | DTO/schema/header freshness, ASCII/signed-int64 totality, zero-world failure, 11-provider and 83+2+34 graph joins, all-world/five-scope evidence, resize/configure ABA rejection, commit sealing, mismatch rejection | maintained default CPU-exact runs identify their realized composition/catalog lock and replay/comparison reject unexplained mismatch | after P2-C1 and relevant P3/P4 joins | 3 + repair | accepted bounded default CPU-exact slice; broader profile/backend, caller-language attestation, and complete state replay residual |
