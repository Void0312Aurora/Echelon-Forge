...
语言：
- 英文规范版：[system_layering_and_engine_encapsulation_plan.md](system_layering_and_engine_encapsulation_plan.md)
- 中文伴随版：`system_layering_and_engine_encapsulation_plan.zh.md`
...
Content status: 迁移后的旧草案；严格架构标准以及当前代码与测试优先于本文的日期化提案。
...
- 本文档服从严格仿真架构标准，仅作为规划输入，不是架构权威或冻结执行单。
- 任何具体实施都应下沉到契约文档或单独冻结的执行计划中。

本文档保留一条从“模块级清理”走向“具有明确引擎边界的真实分层架构”的
候选路线。
