...
语言：
- 英文规范版：[cpp_exact_runtime_refactor_plan.md](cpp_exact_runtime_refactor_plan.md)
- 中文伴随版：`cpp_exact_runtime_refactor_plan.zh.md`
...
Content status: 迁移后的候选计划；历史落地状态只对 I43 快照负责，使用前须重新核验。
...
状态：draft issue。本文记录 2026-04-03 提案与 2026-07-21 I43 落地事实
普查；两个快照都不提供当前执行授权。证据边界见“工作包”之后的
“T4 普查（I43）”。
文档角色：
...
2026-07-21（I43）快照记录：已落地。`ExecutionEpisodeState`
...
2026-07-21（I43）快照记录：就其既定范围已落地。`execution_episode_batch_prepare.{h,cpp}`
...
2026-07-21（I43）快照记录：部分落地，且为选配（opt-in）。`WorldBatchRuntime`
...
3. **execution-episode-controller 主线**（`execution_episode_controller_mainline=True`）
   ...
   WP4 自身的退出条件也尚未达成（见上文 WP4 的日期化快照记录）。
