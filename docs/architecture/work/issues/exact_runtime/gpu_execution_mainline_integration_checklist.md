...
Content status: migrated checklist snapshot; unchecked and checked items must
be revalidated against current code and runtime evidence before activation.

Status: draft issue originating from the checklist opened on 2026-04-18 after
narrowing the maintained `p5` default back to the compiled CPU-mainline visual
path. It is not currently active; the later CUDA-resident programs closed
without promotion and future work requires fresh authorization.
This checklist records a candidate route for turning the closed Phase 0-4 GPU
