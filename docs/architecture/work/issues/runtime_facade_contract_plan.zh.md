...
语言：
- 英文规范版：[runtime_facade_contract_plan.md](runtime_facade_contract_plan.md)
- 中文伴随版：`runtime_facade_contract_plan.zh.md`
...
Content status: 迁移后的接口提案；晋级为 active task 前必须重新普查当前公开面与代码。
...
本文档提出一条 `runtime facade` 契约候选方向，用于把前端与底层
