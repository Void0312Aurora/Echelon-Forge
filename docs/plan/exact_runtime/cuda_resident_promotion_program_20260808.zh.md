...
**2. 已安装的 Nsight Systems 版本低于钉住的版本。** 资源 schema 校验器要求
`nsight_systems_version == "2025.3.2"` 精确匹配。本机装的是 2024.6.2（另有 2022.4.2）。
Nsight Compute 没问题——已装 2025.3.1，而采集器要求恰好 `2025.3.1.0`。也就是说计数器一侧
已就绪，**资源**一侧未就绪；而这很关键，因为资源捕获正是计数器捕获必须链接到的父级产物。

三条可行路径，都不应被静默选定：

- 安装 Nsight Systems 2025.3.2 以匹配该钉定；
- 放宽资源校验器以接受版本区间，并把实际版本记入证据，将精确钉定视为过度约束；
- 把 CP-4c 限制在 Nsight Compute 计数器路径上，推迟任何需要 `nsys` SQLite trace 的 v2
  静态捕获，接受 achieved 计数器届时无法链接到 v2 父级。

第一条对证据完整性最干净；第二条改动最小但削弱了一条刻意设置的溯源门禁；第三条最快拿到
计数器，但会让它们无父级。这是关于证据标准的判断，因此交由所有者决定，而不是擅自假设。

需要注意**没有**成为阻塞的部分：CUDA-on 构建、探针、语义目录、trace signature 等价性
全部正常。剩余障碍属于工具版本与证据管道，不是后端本身。
