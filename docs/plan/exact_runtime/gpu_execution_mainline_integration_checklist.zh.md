<!-- Machine-translated draft generated on 2026-05-18 from docs/plan/exact_runtime/gpu_execution_mainline_integration_checklist.md. Review before treating this file as authoritative. -->

# GPU 执行主线路集成检查清单

状态：于 2026-04-18 开放，在将维护的 `p5` 默认值回退至经过编译的 CPU 主线路可视化路径之后。
这是将已关闭的第 0-4 阶段 GPU 运行时工作转变为维护的执行层加速的实时后续检查清单。
...
此检查清单特意比 [gpu_execution_runtime_research_and_design.md](../archive/gpu_execution_runtime_research_and_design.md) 中的研究/设计文档范围更窄。
...
- 本阶段的下一个冻结后续工作是 [gpu_execution_phase4_rollout_hot_path_freeze.md](gpu_execution_phase4_rollout_hot_path_freeze.md)，该文件在任何进一步默认更改之前，隔离了 `WorldBatchVecEnv` 的主机拷贝语义。
