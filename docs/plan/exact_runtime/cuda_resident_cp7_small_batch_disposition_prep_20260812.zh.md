...
路由规则显式冻结。CP-4 计数器已经定位了开销所在：设备在 256 worlds 时都近乎
空闲，而每个窗口都要付一笔固定的 host 侧成本，在矩阵底端没有 world 数可以
摊薄它。CP-5 已消掉最大的一段 launch 链（六个窗口提交 launch 融合为一），
剩下的就是下文盘点的同步与拷贝骨架。
...
数量级判断（不是测量）：五次同步加十三次拷贝提交，每窗口约 100-250 微秒的
固定 host 侧延迟。world 1 的 CPU 步进是几十微秒量级。这个比值在没有任何
kernel 变慢的情况下就复现了实测的 7-36 倍——与 CP-4 计数器结论（设备近乎
空闲、零 local 流量、零 divergence）一致。

## 修复候选，按预期收益对爆炸半径排序

以下均未经测量；CP-7 采纳哪个就必须测哪个，并经 CP-8 复测。任何执行图
变更都是新的证据世代（v4）——得益于契约派生的计数器链，这如今只是一次契约
扩展，不再是采集器重写。
...
- 本说明不构成性能主张：上文微秒级算术是为决策定尺度，不是证据。
