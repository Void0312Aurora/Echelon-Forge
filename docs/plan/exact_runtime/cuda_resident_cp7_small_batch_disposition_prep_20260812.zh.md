...
本清单**没有**确立的事情：每一项花多少墙钟时间。CP-4 计数器测的是设备侧
利用率，不是 host API 延迟，所以本说明不给出任何微秒数字，也不声称这副
骨架解释了实测的 7-36 倍。骨架之所以是头号候选，是因为计数器已排除设备侧
工作是瓶颈（设备近乎空闲、零 local 流量、零 divergence）；但确认它——以及
按实测贡献对下文候选排序——需要一次 host 侧时间线捕获（对 world-1 窗口跑
Nsight Systems），这是 CP-7 的第一个动作。
