...
文档种类：`reference`
生命周期：`maintained`
正本：`docs/plan/unified_architecture_program/t2_learning_runtime_note_20260726.md`
...
- [仿真系统架构设计](../architecture/simulation_system_architecture_design.zh.md)第 17 节（本文档据以度量的基线契约）
- [T8 G4 真值泄漏清单](t8_g4_truth_leak_inventory.zh.md)（G3-1 的去处）
- [T10 证据脊柱普查](t10_evidence_spine_census_20260721.zh.md)（G2-1/G2-2 的依赖）
- [Agency 权限普查](agency_authority_census_20260721.zh.md)（G3-2 的 `decision_model_ref` 词汇）
- [C++ 精确运行时重构计划](../exact_runtime/cpp_exact_runtime_refactor_plan.zh.md)（第 6 节所记被搁置的 clock-domain 背景）
