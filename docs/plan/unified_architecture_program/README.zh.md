...
T0-T11 行是架构整合主脊柱。另有两条领域交付支线与其并行，它们有意不
被 PR19/PR20 落地波隐式吸收。在此登记边界，是为了避免把活动领域分支
误判成遗漏的 T 系列落地，也避免把它们误标成已验收的架构迭代。

| 支线 | 分支与已观测边界 | 范围 | 落地规则 |
| --- | --- | --- | --- |
| 空战杀伤链校准 | `codex/kill-chain-calibration-five-stage`（远端 tip `fda18daf`） | 制导、武器运用校准与审阅包 | 先完成独立领域评审、证据/fixture 门禁并新增迭代/登记行；不得把校准产物整体并入 T 系列主脊柱。 |
| 陆军/Arnis 环境基座 | `codex/ground-environment-scene-pipeline`（远端 tip `8423fa89`；本地工作树可能另有 WIP） | 陆军场景/环境基座导入、provenance 与 fixture | 保全 WIP，完成字节稳定的 fixture/provenance 评审并建立独立落地行后，才可合入 `main`。 |

这些支线可以消费已维护的架构契约，但在各自证据与落地门禁满足前，
其领域载荷仍与 T 系列分离。
