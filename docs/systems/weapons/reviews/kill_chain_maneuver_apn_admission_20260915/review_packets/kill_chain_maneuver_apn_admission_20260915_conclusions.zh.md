...
- 阻断原因：记录的生成提交不是当前分支祖先，生成时工作树不干净，AIM-120 输入 digest 已过期，原始运行包没有经过逐文件 digest 验证的可检索 artifact index，且 `ef_py` 没有绑定到记录修订的构建 attestation。
...
只有在干净最终修订上重新生成、发布可检索的原始包，逐文件验证 artifact index，并以构建 attestation 将 `ef_py` digest 绑定到该修订后，才能重新评估 promotion。该结果不构成真实 AIM-120 性能、Pk、默认武器参数或交战规则权威。
