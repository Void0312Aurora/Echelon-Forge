...
Language:
- English canonical: `air_combat_damage_model_cross_eval_20260522.md`
- Chinese companion: not available.

Document kind: `review`
Lifecycle: `maintained`
Canonical: `docs/systems/effects/reviews/air_combat_damage_model_cross_eval_20260522.md`
...
| `armor_mm` loaded but never read | `src/components/combat/damage.h:15` — field declaration; zero references in any `.cpp` | **Correct** |
| `NavalDamageStateUpdate` queries `ShipPlatform` → won't match aircraft | `src/systems/combat/damage_system.h:130` — `ecs.system<Health, PlatformDamageState, const ShipPlatform>` | **Correct — critical blocker not called out explicitly** |
| RNG-based hit probability | `src/systems/combat/damage_system.h:109-112` — `damage_rand_uniform01(m[i].rng_state) > hit_prob` | **Correct** |
...
| **`PlatformLossState` has no `ForcedLanding` slot.** | `src/components/combat/damage.h:33-39` — `CombatCapable=0, MissionKill=1, MobilityKill=2, SensorKill=3, Lost=4`. Adding `ForcedLanding` requires renumbering or inserting a new enum value. | **HIGH** — Enum renumbering may break existing naval state machines |
...
| **`NavalDamageStateUpdate` is filtered by `ShipPlatform` tag** — so it will never tick on aircraft even with `PlatformDamageState` present. Phase 1 must either remove that filter or create a parallel aircraft system. | `src/systems/combat/damage_system.h:130` — `ecs.system<..., const ShipPlatform>(...)` — `ShipPlatform` is a required component for this system to match. | **HIGH** — Undocumented architectural blocker |
...
- `src/systems/combat/damage_system.h:163-173` — loss state assignment
- `src/components/combat/damage.h:33-39` — definition
