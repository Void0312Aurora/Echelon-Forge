...
| Stage-C guard 对齐 | 历史实现：`git show c0e4f31f:tools/maintenance/candidate_artifacts/component_probability_surface_probe.py` | 修复后的侧向部件几何会产生 `surface_incidence_cos=0.0`；已关闭的 Stage-C candidate producer 后续退役，其 runtime authority invariant 已迁移到 test-local fixture |
