...
状态：`A2-D0` 已接受；`2026-05-26` Phase 0 已接受，Phase 1 最小补丁正在推进，Phase 2 已从 generated fallback 推进到首批 authored aircraft hitbox 内容，并新增 `AircraftDamageState` 飞机专用 overlay、overlay-driven 飞行动力学/推进/传感器派生、受损结构在高动压/高速包线下继续劣化的最小闭环，以及火灾/燃油泄漏/液压损伤的最小时间级联，Phase 3 已从最小 `WarheadProfile` 数据通路推进到首个弹头族 effects 分配补丁、按弹头族 footprint 的近炸空间投射闭环、最小 relative-velocity-axis 空间方向耦合、`EffectsEvent` 几何与引爆姿态证据字段、引爆姿态轴驱动的参数化 orientation-pattern 证据、hitbox armor / projected exposure / warhead mechanism 可审计采样字段、surface-incidence obliquity 证据字段、机制特定 component-threshold 尺度字段、合成 component-failure probability 采样字段、显式 `FuzeProfile` 证据字段、proximity fuze delay 的最小延迟爆轰调度、首个 fuze type trigger semantics 行为分支、组件级几何入口、数据库级 F-16/Su-35/MQ-9/MH-60R/E-3 代表性组件样例、`EffectsEvent` 主组件身份字段、critical/redundancy 对组件失效概率的最小调制，以及组件依赖到相关系统/overlay 的最小传播，Phase 5 已启动 synthetic vulnerability evidence scaffold、补入 calibrated evidence gate，并把 vulnerability profile/evidence/authority/scale 与 surface-incidence row gate 写入 `EffectsEvent` 审计面；structured-air physical effects 已补“不直接写 RL Score”的行为/静态守卫；1v1 consumer 已从 terminal/objective 迁移推进到非终局 `DamageReport` 连续 shaping 的最小一次性消费。`PN miss-distance baseline` 已闭合，不再阻塞 HP-first bypass 最小反转；deterministic fuze 仍 deferred；contact/impact/timed 的触发语义已开始脱离单一 proximity radius，live missile 的引爆姿态也已可审计并能派生参数化方向证据；surface-incidence 目前只是 evidence/row gate，不是 kill、Pk、deterministic fuze 或 calibrated lethality authority。
...
|----|------|------|--------|--------|------|----------|
| `A2-D0 文档与边界冻结` | accepted | 建立子项目，冻结高真实度毁伤原则和 Phase 0 gate。 | `docs/systems/effects/reviews/a2_high_fidelity_damage_model_20260602/**`、air_combat 索引 | 行为代码 | 文档 diff、索引可达 | 子项目能作为后续实现入口 |
...
| `A2-P3 Warhead profile` | profile_fuze_component_identity_started | 引入 blast/frag/rod/HTK profile 与显式 fuze profile，旧 JSON synthetic 兼容，并让近炸空间 footprint 随弹头族、速度轴、引爆姿态轴、hitbox armor、projected exposure、surface-incidence obliquity、组件机制阈值、组件身份、合成组件失效概率、组件 critical/redundancy、fuze reliability、fuze delay 调度、引爆姿态证据和 fuze type trigger semantics 变化。 | weapon definitions、loader、effects model、engagement contracts、aircraft database | external Pk 数据、完整破片云、校准姿态方向性效应、校准组件失效概率、校准引信性能、全库组件数据、完整冗余依赖图 | warhead runtime/effects tests | warhead/fuze family/mass/radius/delay/reliability/provenance 可审计，scalar damage 标记为兼容层；near miss 投射按弹头族半径/衰减/影响数量、relative velocity axis 和 detonation attitude axis 区分；contact/impact fuze 不再把 near-miss radius 当作触发条件；timed fuze 能按发射后 delay 独立起爆；EffectsEvent 暴露 miss distance、局部起爆点、引爆姿态、闭合速度、导弹速度轴、引爆姿态轴、orientation pattern scale、direct/projection 命中形态、fuze type/radius/delay/reliability、nearest approach 与 detonation time 分离、空间效应尺度、装甲耦合、投影暴露、机制效应尺度、surface-incidence cos、component-threshold 尺度、component identity 和 component-failure probability/sample/count |
...
| `A2-P5 Vulnerability evidence` | event_audit_component_probability_rows_started | 引入 weapon/target/aspect/closure 脆弱性或 Pk 校准数据。 | content/data/contracts | 黑箱替代物理模型、把 surface-incidence 当杀伤权威 | provenance/event tests | F-16 synthetic vulnerability profile 可审计并调制 physical effects；authority gate 与 `EffectsEvent.vulnerability_*` 可证明 synthetic/profile 自声明不放行 Pk/确定性引信；授权 rows 可驱动 scale 或组件失效概率，surface-incidence 只可作为 row gate，未授权 rows 不被消费 |
...
- `docs/systems/effects/reviews/a2_high_fidelity_damage_model_20260602/phase0_preflight_YYYYMMDD.zh.md`
...
- 新增 `AircraftDamageState` overlay，记录结构、飞控、液压、roll/pitch/yaw control authority、control asymmetry、推进、燃油、航电、机组、pilot、mission crew、command/navigation、火灾、燃油泄漏、结构过载/颤振暴露、forced landing 和 subsystem kill 标志；
...
- `AircraftDamageBaseline` 也保存初始 `Sensor` 基线，damage update 每帧从 `avionics_integrity`、`crew_effectiveness`、`mission_crew_effectiveness` 与 `command_navigation_integrity` 派生 BVR sensor range、detection probability、measurement noise 与 track memory；
- 机组/任务岗位后果已从单一 `crew_effectiveness` 扩展到最小 role-aware overlay：F-16 cockpit crew station 影响 pilot/control，E-3 mission operator consoles 影响 mission crew 与传感器任务执行，E-3 command/navigation suite 影响 command/navigation 与任务/传感器能力；这是岗位语义脚手架，不是校准人员伤害模型。
...
- `test_phase3_surface_incidence_cos_reports_obliquity_evidence`
- `test_phase3_warhead_spatial_sampling_reports_fragment_and_rod_evidence`
- `test_phase3_blast_scaled_distance_tracks_standoff_and_pressure`
...
- `test_phase5_effect_scale_rows_can_use_surface_incidence_gate`
- `test_phase5_effect_scale_rows_can_use_blast_scaled_distance_gate`
...
- `test_phase5_component_failure_rows_can_use_surface_incidence_gate`
- `test_phase5_component_specific_probability_rows_override_global_rows`
...
- Pk authority 必须由匹配 descriptor 授予；确定性引信 authority 继续 deferred，当前 vulnerability descriptor 不放行。缺失 descriptor、synthetic placeholder descriptor、target/status/证据轴不匹配或 descriptor 未授权的能力都会被关闭，即 aircraft JSON 自声明不能成为权威；
...
- 新增只读 vulnerability evidence descriptor fixtures：placeholder descriptor 固定 target/weapon/aspect/closure/miss-distance key 和 authority=false 元数据；schema fixture 固定 `a2.vulnerability_evidence.v1` row 形状、机制载荷区间字段和 component-specific 字段。二者当前只证明 evidence 数据形状可审计，不参与 damage 计算，也不授予 Pk 或 deterministic fuze authority。
- 新增临时数据库回归：缺失 descriptor 不放行，synthetic placeholder descriptor 不放行，缺失 evidence axes、当前 schema 版本或 source ref 的 descriptor 不放行，未知/工程化 source kind 或缺少可审计 `validation_manifest` 的 physics surrogate 不放行，非 synthetic 且 target/status/weapon/aspect/closure/miss-distance 证据轴齐备并来自允许 source kind 且具备 schema/source/provenance 的测试 descriptor 才能按字段授予 Pk；deterministic-fuze authority 仍不由 vulnerability descriptor 授权。该测试 descriptor 只证明门控机制，不代表已有正式校准数据。
...
- `EffectsEvent` 新增 `vulnerability_profile_present`、`vulnerability_profile_synthetic`、`vulnerability_calibrated_evidence`、`vulnerability_pk_authority`、`vulnerability_deterministic_fuze_authority`、`vulnerability_evidence_dataset_valid/ref`、`vulnerability_evidence_schema_version`、`vulnerability_evidence_source_kind/source_ref/validation_artifact_ref`、surrogate validation manifest metadata、`vulnerability_calibration_status`、`vulnerability_provenance`、`vulnerability_aspect_bucket`、`vulnerability_family/aspect/closure/miss_distance/effect_scale` 和模型实际使用的径向 `vulnerability_closure_mps`。该事件面证明 vulnerability 调制与 descriptor 来源可审计，但 synthetic profile、synthetic descriptor 或 JSON 自声明仍不能获得 Pk / deterministic-fuze authority。
- descriptor 可新增 `effect_scale_authority`、`component_failure_probability_authority` 与 `rows[]`，使通过 gate 的非 synthetic/calibrated dataset row 按 weapon family、aspect、closure 和 miss-distance 匹配并驱动 effects model 的 vulnerability scale 或组件失效概率；未授予对应 authority 的 rows 即使存在也不会被消费。descriptor 的 `schema_version`、`source_kind`、`source_ref`、`validation_artifact_ref` 与 surrogate validation manifest metadata 会进入 `AircraftVulnerabilityProfile`、`EffectsResult` 和 `EffectsEvent`。component-specific rows 可声明 `component_name` / `component_system` / `component_redundancy_group_id`，优先于同一 descriptor 内的全局概率行，并在 `ComponentMechanismLoadRow` 暴露 evidence component provenance。rows 还可声明机制载荷区间门槛（破片能量、破片面密度、穿透裕度、爆轰超压/冲量、blast scaled distance、连续杆切割裕度、surface-incidence cos），使同一 weapon/aspect/closure/miss-distance 粗桶内的 effect-scale 覆盖和组件失效概率覆盖都继续受当前事件/候选组件实际 mechanism-load vector 约束。rows 必须携带 `row_id`、`source_ref` 与 `provenance` 才会进入运行时 authoritative row 集合；effect-scale row 被消费后写入 `EffectsEvent.vulnerability_effect_scale_*`，component-failure row 被消费后写入顶层 component probability 字段与对应 `ComponentMechanismLoadRow`。`EffectsEvent` 会把组件失效概率来源标记为 `synthetic_sigmoid` 或 `vulnerability_evidence_row`，并携带 calibrated flag、dataset ref 和具体 row provenance；新增回归固定高门槛行不能被低载荷误消费，低/高杆切割载荷 bucket、blast scaled-distance bucket、fragment areal-density bucket 和 surface-incidence bucket 可选择不同概率/scale 行，且被选中的 descriptor source、effect-scale row、component-specific row 和 mechanism-load row 可由事件字段反查；缺少 row provenance metadata 的授权 rows 会被拒绝消费。surface-incidence gate 只筛选当前证据行的适用性，不放行 Pk、确定性引信或校准杀伤。当前 rows 仍只在测试 fixture 中证明数据通路，不代表已有正式 calibrated vulnerability/Pk 数据。
