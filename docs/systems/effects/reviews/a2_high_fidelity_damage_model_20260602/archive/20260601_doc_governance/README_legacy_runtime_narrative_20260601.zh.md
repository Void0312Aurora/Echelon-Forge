...
> 任务粒度说明：本目录下 Phase、TC、RES、candidate package、runtime
> contract 和 authority promotion 不是同一层级。继续推进前先参阅
> [A2 高保真杀伤模型任务粒度与协调总账](../../task_granularity_and_coordination_20260601.zh.md)。
> 除非明确标注 `G4 authority promotion` 或 `G5 kill-chain authority`，
> “闭合/收口”只表示对应子层级完成，不代表整个高保真杀伤模型完成。
...
按 [梯度真实性原则](../../../../../standards/foundation/gradient_realism_principles.zh.md)，本 README 标题中的“高真实度”表示任务方向，不等于当前仓库已经完成全高保真 kill chain。

当前统一口径如下：

- 当前可稳定声明的是：结构化 aircraft 的交战生命周期、事件审计和 damage/effects runtime 主链已经进入维护路径。
- 当前不应直接声明的是：full `G6` 可信空战真实性、已校准单发 `Pk`、已校准 deterministic fuze、或“全机型/全武器族”高保真毁伤完成。
- 当前 authority 主线只在 `AIM-120C-class blast_fragmentation -> F-16C_Block50` 窄域内优先推进 `effect_scale_authority` 与 `component_failure_probability_authority`；`pk_authority` 与 `deterministic_fuze_authority` 保持 `false / deferred`。
- 当前阻断项是：stock 数据库默认 authority 尚未放开，synthetic/scaffold/test-local descriptor 不得上卷成正式 runtime authority，legacy HP fallback 仍存在于非结构化目标路径。
...
- 新增 runtime 回归 `tests/runtime/air_combat/weapon_guidance_realism/vulnerability_scaffold.py`：即使 aircraft JSON 伪造 `calibrated=true` 并指向 `damage_model.py candidate-artifacts validation-scaffold` 生成的 schema-aligned non-authoritative draft，运行时也只允许该 descriptor 元数据进入 `EffectsEvent.vulnerability_evidence_*` 审计面，不会把 `engineering_surrogate + unvalidated + authority=false` 草案误提升为 row-backed authority。
- 同一回归文件现已补入首个 `AIM-120C-class / blast_fragmentation / F-16C_Block50 / beam / high / near_miss` 的 test-local 正向闭环：测试先从 stock blast-fragmentation 近失事件采样 runtime mechanism-load，再构造 runtime-aligned 的 `validated_physics_surrogate` descriptor，只放行 `effect_scale_authority`；回归固定 `vulnerability_effect_scale_source=vulnerability_evidence_row`，同时继续保持 `component_failure_probability_authority=false`、`pk_authority=false` 与 `deterministic_fuze_authority=false`。这证明 Stage B 的 effect-scale 数据路径已可在 test-local 窄域内端到端消费，但不代表 stock 数据库已经放权。
- 本轮进一步补齐了 broad spatial projection 的组件证据链：`blast` / `fragmentation` / `blast_fragmentation` 的 near-miss hitbox 投影现在会为每个被选中的 hitbox 补一个最佳 projected component 候选，导出 `component_mechanism_load_rows`、`component_primary_*` 和组件条件失效采样，而不取消原有 box-level 覆盖。基于这条运行时链，同一回归文件现已补入首个 `AIM-120C-class / blast_fragmentation / F-16C_Block50 / beam / high / near_miss` 的 test-local Stage C 正向闭环：它只为 `right_aileron_actuator` 放行 component-specific `component_failure_probability_authority`，其余 projected components 仍保持 `synthetic_sigmoid`。这说明 Stage C 已能在原窄域 near-miss 子轴上做 component-specific authority 演练，但 stock 数据库仍未默认授权。
- 新增 `tools/maintenance/damage_model.py candidate-artifacts runtime-authority-exercise` 与 `tests/architecture/damage_model/test_candidate_artifact_contracts.py`：前者把 Stage B / Stage C 的 runtime-aligned descriptor builder 从测试私有 helper 提升为独立 maintenance 工具，固定导出 baseline stock near-miss event summary、projected component rows，以及仅限 `test_local_authority_exercise_only` 的 effect-scale / component-probability descriptor candidate；后者固定该工具的 package 边界、可复现性与 CLI 输出形状。该工具只服务于 test-local authority exercise，不创建 stock 数据库默认 authority。
- 新增 `tools/maintenance/damage_model.py candidate-artifacts package-bundle` 与 `tests/architecture/damage_model/test_candidate_artifact_contracts.py`：前者把候选包目录中的 source ledger、surrogate model card、validation report draft、validation scaffold 和 runtime-aligned authority exercise 汇总成一个 package-level candidate bundle JSON，后者固定该 bundle 的 non-authoritative 边界、open residual 列表和文档完备性。该 bundle 解决的是 Stage A 的“候选包成形与可审计”问题，不等于 stock runtime authority 已放行。
- 同时，`tools/maintenance/damage_model.py candidate-artifacts validation-scaffold` 现在已把 `BFM-BM-001..006` 全部落成可执行 scaffold：新增的 `BFM-BM-002` 覆盖非型号化 fragment mass/velocity/energy toy benchmark，`BFM-BM-004` 覆盖 penetration-margin / domain-rejection toy benchmark。它们仍然只服务于候选包验证规划和 residual 收口，不构成 runtime authority。
