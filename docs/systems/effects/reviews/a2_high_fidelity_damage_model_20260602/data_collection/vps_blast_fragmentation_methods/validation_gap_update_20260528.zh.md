...
| `VPS-BFM-014` | `official_public_artifact_externally_verified / candidate_only` | `benchmark_design_reference_candidate`, `method_ref_candidate`, future `reproducibility` | DENIX DDESB TP-20/BEC-O route 已通过 author-side external verification 固定 URL、HTTP 200、content-type 和 sha256，可作为 blast implementation benchmark design 的公开候选路线。 | BEC-O package/version freeze、canonical retention、allowed-output policy 和 comparison-output hashes remain open；不得把 spreadsheet outputs 当 source truth。 |
| `VPS-BFM-015` | `official_public_artifact_externally_verified / candidate_only` | `benchmark_design_reference_candidate`, `validation_criteria_candidate` | DENIX DDESB TP-21 route 已通过 author-side external verification 固定 URL、HTTP 200、content-type 和 sha256，可作为 debris collection/density/mass/velocity vocabulary 的候选路线。 | canonical retention、reviewer-frozen criteria usage boundary 和 release-grade benchmark integration remain open；debris safety domain 不是 missile warhead truth。 |
...
|---|---|---|---|---|
| `BFM-BM-001` | `partial_yes_for_unit_design` | `VPS-BFM-001/002/005` 足以设计 scaled-distance、unit/domain 和 monotonicity checks；`VPS-BFM-014` 可作为 BEC-O comparison 的候选路线。 | `VPS-BFM-003` official artifact；`VPS-BFM-014` canonical retention/ref freeze；BEC-O tool/package hash；selected comparison output hashes；frozen Z-domain and monotonic tolerances；reviewer signoff。 | 可设计非权威 blast unit benchmark；不足以 validation pass 或 calibration。 |
...
| `BFM-BM-003` | `yes_for_sampler_design / partial_for_debris_metrics` | `VPS-BFM-013` 与 fragment method refs 足以设计 isotropy 和 witness-surface bookkeeping；`VPS-BFM-015` 可作为 debris/areal-density vocabulary 候选路线。 | `VPS-BFM-015` retained artifact ref；geometry witness config hash；sample-count schedule；seed list；convergence threshold；closed-surface conservation tolerance；explicit directional-pattern residual。 | 可设计 sampler/areal-density accounting 的 reproducibility benchmark；debris metrics 仍 candidate-only。 |
| `BFM-BM-004` | `partial_yes_for_formula_shape_and_domain_design` | `VPS-BFM-010/011/012` 足以设计 BLE/residual-velocity/V50 vocabulary and domain separation；`VPS-BFM-015` 可作为 debris vocabulary 候选路线；`VPS-BFM-009` rejected。 | MIL-STD-662 artifact/hash 或保持 ASSIST route metadata-only；`VPS-BFM-015` retained artifact ref / criteria usage boundary；public numeric examples for residual velocity if desired；toy material/projectile config hash；monotonic/domain-rejection thresholds。 | 可设计 penetration-margin formula-shape 和 fail-closed tests；不是 aircraft component penetration validation。 |
...
|---|---|---|
| Blast scaled distance / pressure / impulse | IATG + UFC 3-340-02 + Baker 提供公开 method/design chain；DDESB TP-20/BEC-O route 可作为候选 comparison route。 | Kingery-Bulmash official artifact 仍 open；DDESB artifact existence/hash 已 externally verified，但 retained refs、BEC-O executable/version 和 comparison-output hashes 仍 open。 |
...
| Fragment/debris areal density | Marsaglia DOI + IATG/GICHD 支撑 sampler design；DDESB TP-21 route 可作为 debris-metric candidate。 | DDESB artifact existence/hash 已 externally verified，但 retained refs、directional warhead pattern 和 casing breakup 仍为 open residual。 |
| Penetration / ballistic threshold | NASA-HDBK-8719.14 + Recht-Ipson DOI + MIL-STD-662 ASSIST/QuickSearch route 支撑 formula-shape and threshold vocabulary；DDESB TP-21 route 可作为 debris vocabulary candidate。 | 无 scope-matched aircraft material/component benchmark；MIL-STD artifact/hash 仍 pending；DDESB retained refs / benchmark integration 仍 pending；UFC 3-340-01 cannot be used。 |
