...
| rod 字段 | accepted event surface | `WarheadMechanismEvent::rod_cut_margin`、`ComponentLoadEvent::rod_cut_margin`、`EffectsEvent::mechanism_rod_cut_margin`、component primary rod 字段；`test_continuous_rod_event_surface.py` | 只验收标准切割事实，不验收失效 |
| 通用 continuous-rod 几何 | accepted slice | `test_continuous_rod_geometry_response.py` | 只作为通用趋势证据，不提供真实 rod count/velocity |
| 部件切割投影 | accepted slice | `test_continuous_rod_component_cut_projection.py` | 只验收 component-load 切割事实，不做部件损伤、integrity 修改或失效 |
| 诊断和 gate | accepted slice | `test_continuous_rod_diagnostic_projection.py` | 只验收诊断解释和未起爆/非 rod 保护，不做击毁/坠毁/结构结论 |
...
- `continuous_rod` 起爆后的标准 rod/cutting 事实，已由 `test_continuous_rod_event_surface.py` 验收。
- 聚焦测试证明距离、侧向/方位、方向轴和 family 会改变 rod/cut 事实，包括 `test_continuous_rod_geometry_response.py`。
- 部件受载行能暴露每个受影响部件的 rod cut margin，已由 `test_continuous_rod_component_cut_projection.py` 验收。
- 诊断行和快照能解释 rod/cut 事实，但不声明失效；已由 `test_continuous_rod_diagnostic_projection.py` 验收。
