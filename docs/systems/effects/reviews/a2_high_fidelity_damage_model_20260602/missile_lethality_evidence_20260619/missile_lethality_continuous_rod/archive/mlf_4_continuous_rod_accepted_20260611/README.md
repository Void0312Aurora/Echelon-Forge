...
| Rod fields | accepted event surface | `WarheadMechanismEvent::rod_cut_margin`, `ComponentLoadEvent::rod_cut_margin`, `EffectsEvent::mechanism_rod_cut_margin`, component primary rod fields; `test_continuous_rod_event_surface.py` | Accepts standard cutting facts only, not failure |
| Generic continuous-rod geometry | accepted slice | `test_continuous_rod_geometry_response.py` | Generic trend evidence only; no real rod count/velocity |
| Component cut projection | accepted slice | `test_continuous_rod_component_cut_projection.py` | Component-load cut facts only; no component damage, integrity mutation, or failure |
| Diagnostics and gates | accepted slice | `test_continuous_rod_diagnostic_projection.py` | Diagnostics explanation and no-detonation/non-rod guards only; no kill/crash/structural conclusion |
...
- Standard rod/cutting facts for `continuous_rod` detonations, accepted by `test_continuous_rod_event_surface.py`.
- Focused tests showing range, side/aspect, orientation, and family change rod/cut facts, including `test_continuous_rod_geometry_response.py`.
- Component-load rows that expose rod cut margin per affected component, accepted by `test_continuous_rod_component_cut_projection.py`.
- Diagnostics rows and snapshots that explain rod/cut facts without declaring failure, accepted by `test_continuous_rod_diagnostic_projection.py`.
