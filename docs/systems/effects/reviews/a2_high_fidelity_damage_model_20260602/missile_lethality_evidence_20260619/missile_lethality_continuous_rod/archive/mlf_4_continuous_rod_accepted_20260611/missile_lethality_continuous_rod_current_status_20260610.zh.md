...
| 现有 rod 字段 | accepted standard event surface | 标准事件/effects 记录里的 `rod_cut_margin` 字段，以及 `test_continuous_rod_event_surface.py` | 只验收切割事实，不验收失效 |
...
| 通用 rod 几何 | accepted slice | `test_continuous_rod_geometry_response.py` | 没有真实弹种参数 |
| 部件切割投影 | accepted slice | `test_continuous_rod_component_cut_projection.py` | 不做部件失效概率或 integrity 修改 |
| 诊断和 gate | accepted slice | `test_continuous_rod_diagnostic_projection.py` | 不做击毁/坠毁/结构结论 |
