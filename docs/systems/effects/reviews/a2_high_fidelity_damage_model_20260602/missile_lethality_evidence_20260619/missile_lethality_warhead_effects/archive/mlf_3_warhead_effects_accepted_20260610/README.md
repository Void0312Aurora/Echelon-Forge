...
| Current effects model | generic load-shape + spatial-component projection focused pass | `default_effects_model` has mechanism / spatial / component fields; `test_warhead_blast_fragmentation_loads.py` pins range / direction / family changes in standard load facts; `test_warhead_spatial_component_projection.py` pins spatial coverage changing standard component-load facts | Still folded mainly into `EffectsEvent`; default constants do not carry full source category / scope / unit / uncertainty / replacement-rule runtime metadata |
