...
| `G0-I Branch Expansion Integration` | main thread integration | n/a | 在必要时把 `G0-F/G/H` 返回 diagnostics 整合进 source inventory、architecture plan、terrain-branch plan、task clusters 与父级文档。 | `docs/systems/environment/reviews/environment_substrate_g0_closure_20260606/*.md`、`docs/task/ground/README*.md`、`docs/task/ground/ground_current_progress_20260524*.md`、`docs/task/ground/ground_subagent_dispatch_queue_20260521*.md` | 本子阶段不实现 generator，不释放 runtime。 | 本地 review worker packets，并对 touched docs 跑 `git diff --check`。 | branch expansion evidence 已整合或显式拒绝，且 held capabilities 保持。 | 等 `G0-F/G/H` 返回后串行。 | 1 | pass |
