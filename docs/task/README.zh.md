...
- [runtime 性能/](performance_runtime/README.zh.md)：当前真实性冻结后的 runtime 性能推进线。请先从局部 README 查看当前分层规则、任务板和活跃二级条目边界。
- [可视化/](viz/README.zh.md)：仍在推进的统一入口可视化工作线。请先从局部 README 查看当前状态，再进入活跃的冻结/设计文档了解实现边界与 `WP-V4` / `WP-V5` 的已落地状态。
- [海军/](naval/README.zh.md)：仍在推进的海军真实性工作线。请先从局部 README 查看最新检查点、场景边界计划和委派 backlog 入口。
...
- [通用空海军/](common_air_naval/README.zh.md)：`common / air / naval` 拆分工作线的收敛入口。当前活动主计划仍保留在子目录根部，已被吸收的拆分前分析已移入本地 archive。
