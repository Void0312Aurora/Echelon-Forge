...
状态：`2026-06-08` active implementation。射击记录、部件故障词表、公开故障模式行和
第一段动力消费方切片已经落地；翼面/操纵气动响应仍是 active 工作。
...
| 飞行和动力消费 | active implementation | 显式发动机调参启用时，动力系统也会消费推进损伤；气动和默认控制律还没有充分消费结构、操纵能力和不对称损伤。 | 受损机翼、卡滞舵面、左右不平衡还没有强烈进入力和力矩。 |
| 测试证据 | active | 已有 MQ-9/AIM-120C 固定检查、公开故障模式守卫和调参发动机损伤推力检查。 | 运行时测试是工程链路证据，不是真实世界杀伤概率证据。 |
...
| `P4 Consumer Integration` | 把具体损伤送入动力、燃油、传感器、火灾和飞行力学。 | P3 效果存在。 | 发动机、翼面/操纵、燃油、传感器损伤能通过维护中的仿真路径表现。 | partial：动力调参消费方通过 |
| `P5 Scenario Validation` | 用固定 MQ-9 / AIM-120C 样例证明链路。 | P4 实现通过聚焦测试。 | 测试解释尾部、翼面/操纵、燃油和传感器/数据链结果。 | partial pass |
...
- 公开射击行、具体部件故障词表、MQ-9/AIM-120C 固定检查和调参发动机动力消费方已整合。
- 下一段运行时工作应接一个翼面/操纵气动或轴向响应，仍不得增加直接坠毁或“还能不能飞”捷径。
