...
| 测试证据 | active | 已有 MQ-9/AIM-120C 固定检查、公开故障模式守卫、调参发动机损伤推力检查、固定 MQ-9 右副翼气动响应检查和 300 秒稳定长时程响应检查。 | 运行时测试是工程链路证据，不是真实世界杀伤概率证据。 |
...
| `P4 Consumer Integration` | 把具体损伤送入动力、燃油、传感器、火灾和飞行力学。 | P3 效果存在。 | 发动机、翼面/操纵、燃油、传感器损伤能通过维护中的仿真路径表现。 | partial：动力和翼面/操纵气动消费方通过 |
...
- 公开射击行、具体部件故障词表、MQ-9/AIM-120C 固定检查、调参发动机动力消费方和一段
  翼面/操纵气动消费方已整合。
- 后续运行时工作应继续只通过已有维护系统扩大消费方覆盖，尤其要谨慎处理燃油/质量/火灾和
  飞机专用飞控律校准。
