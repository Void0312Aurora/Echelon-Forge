...
固定数据链命中现在会通过维护中的平台状态继续表现为任务/传感器能力下降；窄的地面接触生命周期
表面现在可以区分安全着陆机体和严重撞击残骸，不再把删除飞机作为唯一公开结果。
...
| 地面接触生命周期 | active implementation | 触地现在通过调试表面暴露 `landed_airframe` 和 `crashed_wreck` 状态，并有安全跑道接触、严重撞击和低速非坠毁检查。 | 这不是碎片实体生成，也不会让武器命中更早触发坠毁。 |
| 测试证据 | active | 已有 MQ-9/AIM-120C 固定检查、公开故障模式守卫、调参发动机损伤推力检查、固定 MQ-9 右副翼短/长时程响应检查、固定中心油箱泄漏/质量响应检查、固定数据链任务/传感器响应检查和地面接触生命周期检查。 | 运行时测试是工程链路证据，不是真实世界杀伤概率证据。 |
...
| `P4 Consumer Integration` | 把具体损伤送入动力、燃油、传感器、火灾和飞行力学。 | P3 效果存在。 | 发动机、翼面/操纵、燃油、传感器损伤能通过维护中的仿真路径表现。 | partial：动力、翼面/操纵气动、燃油泄漏/质量和数据链任务/传感器证据通过 |
| `P5 Scenario Validation` | 用固定 MQ-9 / AIM-120C 样例证明链路。 | P4 实现通过聚焦测试。 | 测试解释尾部、翼面/操纵、燃油和传感器/数据链结果。 | partial pass：包含固定燃油泄漏/质量和数据链任务/传感器样例 |
| `P6 Acceptance` | 决定 accepted 或 held 并记录残余。 | P5 证据完成。 | 父级 README 和状态文档给出诚实能力边界和剩余缺口。 | planned；第七轮 partial pass |
...
- 覆盖尾部发动机、翼面/操纵、燃油泄漏/质量、火灾风险、传感器/数据链的
  MQ-9 / AIM-120C 回归测试。
...
- 公开射击行、具体部件故障词表、MQ-9/AIM-120C 固定检查、调参发动机动力消费方、一段
  翼面/操纵气动消费方、一个中心油箱泄漏/质量运行时检查、一个数据链任务/传感器运行时检查
  和一个地面接触生命周期表面已整合。
- 后续运行时工作应继续只通过已有维护系统扩大消费方覆盖，尤其要谨慎处理更完整的火灾行为、
  尾部动力/燃油样例和飞机专用飞控律校准。
- 右副翼长时程损伤会让受损 MQ-9 在干净基线保持平飞时进入近地响应。严重触地现在可以观察为
  `crashed_wreck`，安全或低速触地保持为 `landed_airframe`。
- 碎片/残留实体和更完整火灾行为仍是后续工作。
