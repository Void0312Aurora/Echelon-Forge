...
| 测试证据 | active | 已有 MQ-9/AIM-120C 固定检查、公开故障模式守卫、调参发动机损伤推力检查、固定 MQ-9 右副翼短/长时程响应检查和固定中心油箱泄漏/质量响应检查。 | 运行时测试是工程链路证据，不是真实世界杀伤概率证据。 |
...
- 公开射击行、具体部件故障词表、MQ-9/AIM-120C 固定检查、调参发动机动力消费方、一段
  翼面/操纵气动消费方，以及一个中心油箱泄漏/质量运行时检查已整合。
- 后续运行时工作应继续只通过已有维护系统扩大消费方覆盖，尤其要谨慎处理更完整的火灾行为、
  传感器/数据链后果和飞机专用飞控律校准。
- 右副翼长时程损伤会让受损 MQ-9 在干净基线保持平飞时进入近地响应，但地面撞击后的坠毁状态传播
  仍不是已实现的维护结果。当前触地路径需要公开生命周期状态或残骸/碎片表面，之后才可验收为
  不只是立即丢失或删除。
