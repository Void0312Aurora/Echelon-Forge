...
| `A8-DEC-G Acceptance And Index Sync` | main thread | n/a | 决定 accepted 或 held，同步父级 README/status，并记录残余。 | A8 README/status/acceptance、父级 air-combat README、必要 archive index | 把 docs-only 工作标成 runtime pass | docs link check 和 accepted validation commands | 能力声明有证据，过度声明继续被拒绝。 | last, after H or explicitly held | 1 | partial pass：第八轮 W17/W18 已验收；P6 验收决策仍待定 |
...
- 剩余近期工作是 P6 验收决策，而不是继续添加直接坠毁或“还能不能飞”捷径。
