...
- 公开射击行、具体部件故障记录和调参发动机动力消费方已经整合。
- 剩余近期运行时切口是一个翼面/操纵气动或轴向控制响应，仍不得增加直接坠毁或“还能不能飞”捷径。
