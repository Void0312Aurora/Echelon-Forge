...
| `A8-DEC-G Acceptance And Index Sync` | main thread | n/a | Decide accepted or held, sync parent README/status, and record residuals. | A8 README/status/acceptance, parent air-combat README, archive index if needed | Marking docs-only work as runtime pass | docs link check plus accepted validation commands | Capability statement is evidence-backed and overclaims remain refused. | last, after H or explicitly held | 1 | partial pass: sixth wave W13 accepted; W14 held |
...
- Public shot rows, concrete part-failure records, the tuned-engine propulsion
  consumer, one wing/control aero consumer, and one fixed center-fuel-cell
  leak/mass response check are integrated.
- Remaining near-term cuts should add downstream response evidence for rear
  propulsion, broader fire behavior, sensor/data-link cases, and the
  ground-impact lifecycle without a direct crash or can-fly shortcut.
