...
Status: `2026-06-08` finite task-cluster plan for
[README.md](README.md).
...
- Public shot rows, concrete part-failure records, the tuned-engine propulsion
  ...
  leak/mass response check, and one data-link mission/sensor response check are
  integrated.
- Remaining near-term cuts should add downstream response evidence for rear
  propulsion and broader fire behavior, while keeping debris/residue decisions
  out of direct crash or can-fly shortcuts.
...
- The first `A8-DEC-H` implementation exposes landed-airframe and crashed-wreck
  lifecycle state on the original entity; debris/residue objects remain held.
