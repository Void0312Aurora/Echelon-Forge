...
状态：`2026-06-08 closed / historical first-event timing line superseded`。A6
保留 hazard、deadline 与 launch-window labels 没能形成稳定独立发射时机方案的历史证据。
当前可执行发射门由后续 M3-S2 包关闭：
...
A6 残余只作为发射时机质量研究历史保留，不代表当前发射闭合状态。
...
| 区域 | 状态 | 证据 | 边界 |
| --- | --- | --- | --- |
| 生命周期 | closed；superseded | M3-S2 后续在 active scenario/config pair 上验收有边界发射门。 | A6 timing experiments 不再是当前发射阻塞项。 |
...
| `P6 Closure/Re-scope` | 同步 A3/A4/A5/M1/M2，并选择下一 objective wave。 | P5 evidence complete。 | A6 历史残余已记录，下一 wave 限定为 deadline bootstrap，而不是 M2 release。 | pass；historical re-scope |
...
| `P13 Root-Cause Re-scope` | 停止 L 调参并识别机制 blocker。 | P12 held evidence exists。 | 根因记录解释 stochastic hazard 累积、吸收式 first-event censoring 与缺失 counterfactual hold/fire credit；继续训练前先重新定义下一 contract。 | pass；closed by M3-S2 migration |
...
这是历史 A6 验收门。A6 现在作为发射时机质量研究历史关闭，不作为当前发射方案 accepted。
...
- A6 原地关闭，作为历史发射时机证据保留。
- 保留结论是：hazard/deadline/launch-window labels 暴露了有用梯度，但没有形成稳定独立
  发射/时机答案。
- 当前 launch behavior 不要默认重开 A6。当前发射门由 M3-S2 关闭；只有明确开启新的
  timing-quality research task 时，A6 证据才作为输入使用。
- M2/model-family 问题不属于这个已关闭的 A6 包。

## 归档

完整 A6 包已归档到 `docs/task/air_combat/archive/`。原任务路径现在只保留轻量指针
README。
