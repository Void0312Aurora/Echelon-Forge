...
| `A6-EVT-J Event-Head Update-Strength Audit` | main thread | n/a | 判定为什么持续正例只把 event probability 推到约 `0.5%`。 | `tests/policy/test_event_head_update_contracts.py`、A6 evidence note | M2 release、update audit 前实现 value-head、reward-only legality | focused gradient/update probe；unit probe 不能单独作为 learned-policy acceptance | Audit 识别 optimizer/head scaling blocker，或为 event-value head 清路。 | After I；serial | 1 | pass；held outcome |
