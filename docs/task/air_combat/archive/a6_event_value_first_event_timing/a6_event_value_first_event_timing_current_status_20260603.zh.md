...
| Event-head update audit | pass；held outcome | [Event-head audit](a6_event_value_first_event_timing_event_head_update_audit_20260603.zh.md) 与 `tests/policy/test_event_head_update_contracts.py` 显示 gradients 到达 shared/HMoE event heads，但当前 `3e-5` LR 和受抑制 residual lane 让 event delta 移动太慢。 | 只是 diagnostic evidence，不是 learned-policy acceptance。 |
