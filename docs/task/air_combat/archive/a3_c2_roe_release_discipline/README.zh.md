...
状态：`2026-06-04` archived accepted evidence packet。有边界的 C2/ROE implementation、
P4 evidence、learned-policy probe 与 post-launch observation 修复已记录；M2 继续 held。
本子项目定义空战 C2、ROE 与发射纪律约束层；在这层明确前，不再把同一目标多枚导弹问题直接
归因为策略记忆失败。
