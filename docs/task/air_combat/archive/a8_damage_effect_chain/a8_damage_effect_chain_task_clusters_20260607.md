...
Status: `archived historical task-cluster plan; accepted with deferred
residuals on 2026-06-08`.

The retained evidence now lives under
`docs/task/air_combat/archive/a8_damage_effect_chain/`; the original
`docs/task/air_combat/a8_damage_effect_chain/` path is a pointer README only.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `A8-DEC-A Boundary` | main thread | n/a | Create A8 docs, parent links, current status, task clusters, and archive boundary. | `docs/task/air_combat/archive/a8_damage_effect_chain/**`, `docs/task/air_combat/README.md`, `docs/task/air_combat/README.zh.md` | Runtime implementation, new physics claims | `git diff --check -- docs/task/air_combat` | Standard files exist and parent README links A8. | first, serial | 1 | pass |
...
| `A8-DEC-E Consumer Integration` | current-session diagnostics workers | high reasoning | Route concrete damage into propulsion, fuel/mass, sensors, fire, and flight/aerodynamic behavior. | `src/systems/combat/damage_system.h`, `src/systems/physics/aerodynamics_system.h`, `src/systems/physics/propulsion_system.h`, related physics tests | Direct crash rule, independent flight verdict | Focused runtime tests and flight-dynamics guards | Engine, fuel, sensor, wing/control, fire, and structural damage are consumed by maintained systems. | after D; serial with physics write set | 2 | pass for accepted A8 slice; calibration and platform-family expansion deferred |
| `A8-DEC-F MQ-9 / AIM-120C Validation` | current-session worker | medium/high reasoning | Build fixed cases that prove rear-engine, wing/control, fuel/fire, and sensor/data-link outcomes. | `tests/runtime/air_combat/**`, optional test-only fixtures under examples/scenarios | Treating one smoke run as real-world probability | `python -m pytest -q tests/runtime/air_combat/test_weapon_guidance_realism_guards.py tests/runtime/air_combat/test_air_combat_1v1_fire_missile.py` plus new focused tests | Tests check both immediate records and later aircraft response. | after C/D/E; can prepare fixture plan after B | 2 | pass for fixed MQ-9/AIM-120C-like engineering cases; real lethality/Pk deferred |
| `A8-DEC-H Ground-Impact Lifecycle` | current-session worker plus scout | high reasoning | Turn post-flight ground contact into an observable lifecycle state or residue surface when impact is severe enough. | `src/systems/physics/ground_contact_system.h`, lifecycle/damage components, runtime contracts/bindings if needed, focused runtime tests | Direct touch-kill, direct disappearance, treating safe landing as crash | Ground-contact lifecycle tests plus affected contract/runtime guards | Safe runway contact stays active; damaged severe impact records landed/crashed/residue state; low-speed contact does not create a crash. | after W12 scout; before final G acceptance | 2 | partial pass: landed-airframe/crashed-wreck lifecycle accepted for this slice; debris/residue entity deferred |
| `A8-DEC-G Acceptance And Index Sync` | main thread | n/a | Decide accepted or held, sync parent README/status, and record residuals. | A8 README/status/acceptance, parent air-combat README, archive index if needed | Marking docs-only work as runtime pass | docs link check plus accepted validation commands | Capability statement is evidence-backed and overclaims remain refused. | last, after H or explicitly held | 1 | pass: accepted with deferred residuals |
...
- Public shot rows, concrete part-failure records, the tuned-engine propulsion
  consumer, one wing/control aero consumer, one fixed center-fuel-cell
  leak/mass response check, one broader-fire response pair, and one data-link
  mission/sensor response check are integrated.
- A8 P6 is accepted with deferred residuals; near-term work should not add
  another direct crash or can-fly shortcut.
- `A8-DEC-H` is now the scoped home for ground-impact lifecycle work so
  `A8-DEC-E` does not keep absorbing unrelated post-flight object-lifecycle
  responsibilities.
- The first `A8-DEC-H` implementation exposes landed-airframe and crashed-wreck
  lifecycle state on the original entity; debris/residue objects are deferred.
