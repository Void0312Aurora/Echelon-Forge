...
- 已验收第七轮工作：
  ...
  - `A8-W16 Broader Fire Consequence Scout`：只读证据通过。它给出下一段 test-only
    ...
    断言纯发动机火区会自行增长。
- 已验收第八轮工作：
  - `A8-W17 Broader Fire Consequence Writer`：通过。它增加固定 MQ-9/AIM-120C-like
    检查，覆盖左翼油箱火势增长和二次损伤，以及尾部发动机火区播种和动力后果。它没有改生产代码，
    也没有要求无可燃暴露的纯发动机命中必须产生火势增长。
...
- 尾部命中：发动机、供油控制或螺旋桨损伤通过动力系统降低推力，并可以播种发动机火区；没有
  可燃暴露时，不要求火势自行增长。
- 翼面/操纵命中：翼梁、副翼、襟翼或作动器损伤通过飞行路径改变控制能力、不对称、阻力、升力或结构余量。
- 燃油命中：油箱损伤导致漏油、质量变化、标记火源，并可以通过维护中的运行时路径增长火势和二次损伤。
