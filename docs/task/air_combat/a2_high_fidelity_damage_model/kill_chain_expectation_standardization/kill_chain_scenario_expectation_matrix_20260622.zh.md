...
`2026-07-15` 的运行时主网格与局部加密复核显示，在冻结的 `35 g`、`N=4`、
`APN=0.5` 工程代理制导下，`4..8 km` 的 `R_fuze=15 m` 进入边界稳定落在约
`36..38 deg`，而 `45 deg` 最近点约为 `22..24 m`。单参数扫掠只有把导航常数推到
约 `10..12`，或把横向过载推到约 `50 g`，才能强制该 cell 进入 `R_fuze`。因此
`4/6 km x 45 deg` 从 `N` 修正为 `M`；这属于 launch-window oracle 校准，不通过
大幅重调制导参数来维持旧人工分类。
