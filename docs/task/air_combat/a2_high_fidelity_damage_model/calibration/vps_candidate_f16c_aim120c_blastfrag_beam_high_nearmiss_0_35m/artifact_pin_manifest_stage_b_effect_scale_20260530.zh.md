...
| `forbidden_release_action` | `do not treat pending or sanity-only artifacts as acquired calibration inputs` |
...
| `PIN-BFM-001` | `VPS-BFM-014` | `Tier A candidate` | DENIX TP-20 / BEC-O official route candidate | official route identified, DNS failed locally | `pending_acquisition` | `missing` | [vps validation gap update](../../data_collection/vps_blast_fragmentation_methods/validation_gap_update_20260528.zh.md) | `not_consumed_for_stage_b_release` | future blast implementation comparison route | 未固定 artifact、rights、hash 前不得写成 acquired benchmark input | `RES-001`, `RES-006` |
| `PIN-BFM-002` | `VPS-BFM-015` | `Tier A candidate` | DENIX TP-21 official route candidate | official route identified, DNS failed locally | `pending_acquisition` | `missing` | [vps validation gap update](../../data_collection/vps_blast_fragmentation_methods/validation_gap_update_20260528.zh.md) | `not_consumed_for_stage_b_release` | future debris / areal-density vocabulary route | 未固定 artifact、rights、hash 前不得写成 acquired benchmark input | `RES-001`, `RES-005`, `RES-011` |
...
> `the Stage B package now has an explicit artifact-pin surface, but several critical routes are still pending or sanity-only, so source provenance remains open`.
