...
| `MET-002` | component probability residual | `component_failure_probability` | `Brier/log-loss/calibration-curve route reserved` | `deferred_to_stage_c` | `not_run` | 不授权 |
...
但 benchmark 仍未运行，Stage C `component_failure_probability` 也尚未进入同等冻结状态。
...
| `VAL-002` | `F-16C_Block50` | `blast_fragmentation` | `beam` | `high` | `near_miss_0_35m` | `projected component load rows` | future component probability residual | `not_run` |
...
当前结果：已有脚手架、scope probe、fixed-seed candidate benchmark snapshot 与 test-local authority 演练，
...
| `component_failure_probability` | `test_local_exercise_only / not_validated` | [a2_blastfrag_runtime_aligned_authority_pack.py](/home/void0312/Workshop/CMO/tools/maintenance/a2_blastfrag_runtime_aligned_authority_pack.py) | 否 |
