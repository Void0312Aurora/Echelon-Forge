...
| `RES-003` | target geometry | Stage B witness-geometry bookkeeping 已窄域 closeout；真实 F-16C Block 50 组件几何、材料、遮挡和暴露面积来源仍未审计 | Stage B 采样 bookkeeping 不再单独阻塞；仍可能把工程 hitbox 当成真实 vulnerability | effect scale / component probability | geometry 来源、假设和误差界限可追溯 | open_stage_b_witness_geometry_bookkeeping_closed_global_geometry_blocked |
| `RES-004` | warhead scope | Stage B AIM-120C-class blast-fragmentation family scope 已窄域 closeout；具体 AIM-120C 战斗部真值、质量、破片形态和 fuze 证据仍未关闭 | Stage B family-scope 标签不再单独阻塞；仍可能过拟合或暗示未知具体弹药参数 | effect scale / mechanism load | warhead class 假设、范围和敏感性分析完成 | open_stage_b_family_scope_closed_specific_warhead_truth_blocked |
