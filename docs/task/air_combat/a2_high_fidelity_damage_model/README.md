...
- `EffectsEvent` exposes primary component integrity and named redundancy-group availability/member/failure counts, so repeated component damage has runtime memory instead of being only a one-shot probability discount.
...
Recent focused result for the latest air-combat increment: `cmake --build build-workshop --target ef_py -j4` passed; the air-combat guard + evidence descriptor subset passed with `85 passed, 52 subtests passed`; the engagement/binding contract subset passed with `27 passed`; `git diff --check` passed.
