...
Status: Phase 0 accepted on `2026-05-26`; the Phase 1 minimal patch is in progress with focused tests passing; Phase 2 has moved from generated fallback into the first authored aircraft-hitbox coverage plus an aircraft-specific `AircraftDamageState` overlay, overlay-driven flight/propulsion/sensor derivation, a minimal high-energy envelope loop for damaged-structure degradation, a minimal fire/fuel-leak/hydraulic cascade timeline, and a minimal control-axis/asymmetry overlay for aileron/elevon component damage; Phase 3 has moved from minimal `WarheadProfile` plumbing into the first warhead-family effects-distribution patch, a warhead-family spatial-footprint projection loop, minimal relative-velocity-axis coupling, explicit `EffectsEvent` geometry evidence, an auditable hitbox-armor / projected-exposure / warhead-mechanism sampling scaffold, the first mechanism-specific component-threshold scaffold, a synthetic component-failure probability sampling scaffold, explicit `FuzeProfile` evidence, minimal proximity-fuze delayed-detonation scheduling, the first fuze-type trigger-semantics branch, the first component-level geometry entry, a database-level F-16 component example, primary-component evidence on `EffectsEvent`, and minimal critical/redundancy semantics for component-failure probability; Phase 5 has started a synthetic vulnerability-evidence scaffold plus a calibrated-evidence gate; structured-air physical effects now have behavior/static guards proving they do not directly write RL `Score`, and the 1v1 consumer now reads `DamageReport` for terminal/objective semantics plus minimal nonterminal damage shaping. The current loop covers PN miss-distance baselines, HP-first bypass reversal for structured aircraft, live-missile `EffectsEvent`/`DamageReport` recording, structured-air score-authority guards, aircraft damage-state synchronization, nose/fuselage/wing subsystem effect differentiation, F-16/Su-35/MQ-9/MH-60R/E-3 migration onto authored structured hitboxes, air-specific structure/flight-control/hydraulic/roll-pitch-yaw-control/asymmetry/propulsion/fuel/avionics/crew overlay evidence, overlay-derived FlightModel/Propulsion/fuel-leak and sensor range/Pd/noise/track-memory constraints, damaged-airframe high dynamic pressure/Mach exposure, real `FuelSystem` / `Mass` fuel depletion from fuel leaks, time-cascade fire/hydraulic/fuel propagation into structure/avionics/crew/flight-control/platform capability, auditable warhead family/mass/lethal-radius/synthetic provenance and fuze type/radius/delay/reliability/synthetic provenance in missile runtime, effects events, diagnostics-only local-hit effects tests, distance-attenuated near-miss projection tests, warhead-family footprint tests, continuous-rod velocity-axis near-miss direction tests, armor/exposure mechanism sampling tests, component-threshold mechanism-sensitivity tests, component-failure probability sampling tests, database F-16 primary-component evidence tests, aileron component damage to roll-axis authority tests, component critical/redundancy probability modulation tests, fuze runtime/event tests, fuze delay scheduling tests, contact-fuze near-miss non-trigger tests, timed-fuze independent-delay detonation tests, `EffectsEvent` miss distance/local detonation point/closure/velocity-axis/direct-or-projected hit/fuze/spatial-effect/armor-coupling/exposure/mechanism-effect/component-threshold/component-identity/component-failure evidence, an F-16 synthetic vulnerability profile that modulates structured damage by warhead/aspect/closure/near-miss evidence, a guard proving synthetic vulnerability is not Pk or deterministic-fuze authority, and a one-shot consumer guard for nonterminal `DamageReport` shaping.
...
- `EffectsEvent` exposes miss distance, target-body detonation point, closure speed, and missile velocity-axis evidence for later fuze/Pk calibration.
- `EffectsEvent` exposes direct/projection hit form, spatial effect scale, armor-coupling scale, projected-exposure scale, and mechanism-effect scale.
...
- `EffectsEvent` exposes component-failure probability/sample/count as synthetic sampling evidence; these fields are not calibrated Pk authority.
- `EffectsEvent` exposes component hit count and primary component identity so component geometry is auditable outside logs.
- The database-level F-16 and Su-35S component examples distinguish wing fuel-cell and aileron/elevon-actuator consequences and exercise reusable component-geometry evidence beyond one airframe.
- Aileron/elevon component damage can derive roll-axis authority loss and control-asymmetry evidence before any full flight-control torque-law rewrite.
- The first vulnerability evidence dataset descriptor loads as audit metadata only and keeps Pk / deterministic-fuze authority disabled.
...
Recent focused result for the latest air-combat increment: `cmake --build build-local-win --target ef_py --config Release -j 4` passed; the air-combat guard + evidence descriptor subset passed with `57 passed, 19 subtests passed`; the engagement/binding contract subset passed with `51 passed`.
