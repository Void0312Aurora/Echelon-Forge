...
状态：`2026-06-14` finite task-cluster plan with TG-P7-R5 damage-event trace completed，用于
...
- TG-P7 可以在 separate proxy database、`runtime.database_path` opt-in、proxy
  ...
  damage-event traces 中可观测后，进入更长维护训练。
...
- 结构解体和残骸：后续独立子项目。
- Pk 与真实弹种校准：后续独立子项目。
- 运行时主路径替换：TG-P7-R5 已有 opt-in proxy database，包含带 feature flag 的
  ...
  覆盖；默认 runtime replacement 仍等待更长 proxy training 和后续 acceptance decision。
