...
Status: `2026-06-14` finite task-cluster plan with TG-P7-R4 active training
probe completed for [README.md](README.md).
...
| `TG-P7` | main thread | high | Decide runtime-interface boundary and wire the initial training proxy only after tests. | `target_geometry_runtime_activation_candidate_20260613.json`; `target_geometry_runtime_activation_candidate_20260613.csv`; `target_geometry_runtime_activation_results_20260613.md`; `target_geometry_runtime_behavior_regression_*`; `target_geometry_training_proxy_database_20260613*`; `target_geometry_training_proxy_results_20260613.md`; `target_geometry_training_probe_results_20260614.md`; training bootstrap/train entrypoints; active proxy config; README/status/queue docs | No unreviewed proxy in the default active main path; default unit database remains the control path | parser/contract checks, focused pytest, C++ loader smoke, packet regeneration, in-memory behavior regression, `runtime.database_path` contract tests, RuntimeFacade proxy database load, local training smoke, active 8k proxy/baseline comparison | parse-ready patch candidate, in-memory behavior regression, opt-in proxy database, local 64-step training smoke, and active 8k proxy/baseline comparison exist; targeted damage-event trace inspection remains pending | depends on `TG-P6` | 4 | started; R4 active training probe pass |
...
- TG-P7 can only advance into longer maintained training with a separate proxy
  database, `runtime.database_path` opt-in, `32` proxy components, `26` default
  components, passing loader/training-entry contracts, local proxy smoke, and
  active 8k proxy/baseline comparison.
...
- Main runtime-path replacement: TG-P7-R4 has an opt-in proxy database with a
  ...
  proxy/baseline comparison; default runtime replacement is still pending
  targeted damage-event trace inspection and a later acceptance decision.
