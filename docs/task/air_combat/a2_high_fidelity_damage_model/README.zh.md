...
- `EffectsEvent` 至少暴露 component hit count 和 primary component identity，证明组件级几何不是只在日志中存在；
...
结果：`cmake --build build-workshop --target ef_py -j4` 已通过；聚焦空战 guard 与 evidence descriptor 已通过，`85 passed, 52 subtests passed`；engagement/binding contract 子集已通过，`27 passed`；`git diff --check` 已通过。
