...
状态：`2026-06-18` 归档指针 / active follow-on 导航，MLF-6 ready for user acceptance。完整项目包已移入
...
- [missile_lethality_structural_failure/README.zh.md](missile_lethality_structural_failure/README.zh.md)：
  **active / ready-for-acceptance MLF-6 子项目**，消费 MLF-5 部件失效输出（ECS `ComponentDamageState`），
  写入 `StructuralBreakupEvent` 行并产生具名断裂事实；不修改 `structural_integrity`、
  飞行动力学或失能状态（属于 MLF-7）。P7 更广回归已通过（`447 passed`）；
  archive 移动等待用户明确指令。不声明 Pk、残骸生命周期或具体弹种杀伤结论。
...
MLF-6（结构失效/机体断裂）是按 `docs/agent` 标准维护的 active / ready-for-acceptance 子项目，尚未归档：
[missile_lethality_structural_failure/README.zh.md](missile_lethality_structural_failure/README.zh.md)。
