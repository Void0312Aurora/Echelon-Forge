...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `MLF-7A Boundary And Index` | main thread | n/a | 创建 MLF-7 子项目入口：README、任务簇、当前状态、派发队列、验收记录、archive 占位和父级导航。 | `docs/task/air_combat/a2_high_fidelity_damage_model/archive/missile_lethality_secondary_consequence_coupling/**`、父 README | runtime edits、实现声明 | `git diff --check -- docs/task/air_combat/a2_high_fidelity_damage_model/archive/missile_lethality_secondary_consequence_coupling docs/task/air_combat/a2_high_fidelity_damage_model/README*.md docs/task/air_combat/README*.md` | 后续 Agent 不依赖聊天历史即可恢复 MLF-7；P0 范围和非目标明确 | first, serial | 1 | complete |
