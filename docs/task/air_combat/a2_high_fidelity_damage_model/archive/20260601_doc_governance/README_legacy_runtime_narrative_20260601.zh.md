...
- 新增 `tools/maintenance/a2_blastfrag_runtime_aligned_authority_pack.py` 与 `tests/architecture/test_a2_blastfrag_runtime_aligned_authority_pack.py`：前者把 Stage B / Stage C 的 runtime-aligned descriptor builder 从测试私有 helper 提升为独立 maintenance 工具，固定导出 baseline stock near-miss event summary、projected component rows，以及仅限 `test_local_authority_exercise_only` 的 effect-scale / component-probability descriptor candidate；后者固定该工具的 package 边界、可复现性与 CLI 输出形状。该工具只服务于 test-local authority exercise，不创建 stock 数据库默认 authority。
- 新增 `tools/maintenance/a2_candidate_vps_bundle.py` 与 `tests/architecture/test_a2_candidate_vps_bundle.py`：前者把候选包目录中的 source ledger、surrogate model card、validation report draft、validation scaffold 和 runtime-aligned authority exercise 汇总成一个 package-level candidate bundle JSON，后者固定该 bundle 的 non-authoritative 边界、open residual 列表和文档完备性。该 bundle 解决的是 Stage A 的“候选包成形与可审计”问题，不等于 stock runtime authority 已放行。
- 同时，`tools/maintenance/a2_blastfrag_validation_scaffold.py` 现在已把 `BFM-BM-001..006` 全部落成可执行 scaffold：新增的 `BFM-BM-002` 覆盖非型号化 fragment mass/velocity/energy toy benchmark，`BFM-BM-004` 覆盖 penetration-margin / domain-rejection toy benchmark。它们仍然只服务于候选包验证规划和 residual 收口，不构成 runtime authority。
