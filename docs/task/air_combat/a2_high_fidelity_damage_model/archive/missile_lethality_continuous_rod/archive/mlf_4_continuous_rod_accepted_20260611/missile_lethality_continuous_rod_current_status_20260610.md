...
| Existing rod fields | accepted standard event surface | `rod_cut_margin` fields in standard events/effects records plus `test_continuous_rod_event_surface.py` | Cutting facts only, not failure |
...
| Generic rod geometry | accepted slice | `test_continuous_rod_geometry_response.py` | No true weapon parameters |
| Component cut projection | accepted slice | `test_continuous_rod_component_cut_projection.py` | No component failure probability or integrity mutation |
| Diagnostics and gates | accepted slice | `test_continuous_rod_diagnostic_projection.py` | No kill/crash/structural conclusion |
