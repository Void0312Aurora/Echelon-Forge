...
- `test_continuous_rod_event_surface.py`：标准 rod/cut 事件面。
- `test_continuous_rod_geometry_response.py`：距离、侧向/方位和方向轴响应。
- `test_continuous_rod_component_cut_projection.py`：部件切割曝光投影。
- `test_continuous_rod_diagnostic_projection.py`：诊断解释、非连续杆零切割、未起爆不合成虚假 rod 行。
- `test_diagnostics_probe_contracts.py`：诊断字段合同回归。
