...
Status: `2026-06-09` finite task-cluster plan for [README.md](README.md). MLF-2B, MLF-2C, and MLF-2D are accepted; MLF-2E diagnostics projection audit is next.
...
| `MLF-2E Diagnostics Projection` | future diagnostics worker | n/a | Make process probe emit geometry/fuze stage rows and summary per munition | `tools/diagnostics/**`, `tests/diagnostics/**`, this subproject evidence record | reward semantics, runtime physics decision, long-term legacy aliases | diagnostics pytest; controlled export sample | trigger reason does not depend on `last_effect_*`; no-detonation cases report reasons | after 2C/2D API names freeze | 2 | planned / ready for read-only audit |
| `MLF-2F Runtime Handoff Gate` | integration worker | n/a | Hand detonation state to existing effects model; no-trigger path explicitly produces no effects event | narrow weapon lifecycle path, effects invocation tests, diagnostics/reward guard tests | warhead mechanism, target breakup, training win/loss, entity deletion | focused runtime tests + existing reward/diagnostics guards | only detonation enters later effects; no-trigger/failure/not-armed cases do not silently disappear | after 2C/2D/2E | 2 | planned |
| `MLF-2G Acceptance And Archive Prep` | main thread | n/a | Summarize evidence and update status, parent navigation, and residual map | this subproject README/status/task cluster/dispatch queue/archive index; A2 README | overclaiming real AIM-120C/MQ-9, Pk, or breakup authority | docs diff check + referenced focused tests | accepted/held state matches evidence and MLF-3+ residuals are explicit | last, serial | 1 | planned |
