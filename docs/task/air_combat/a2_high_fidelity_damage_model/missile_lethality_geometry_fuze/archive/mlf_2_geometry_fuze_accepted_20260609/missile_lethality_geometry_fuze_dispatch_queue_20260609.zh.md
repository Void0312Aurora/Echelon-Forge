...
| `MLF-2E-W1` | `MLF-2E Diagnostics Projection` | main thread | `tools/diagnostics/air_combat_weapon_employment_process_probe.py`；`tests/runtime/air_combat/test_diagnostics_probe_contracts.py` | 导出每枚弹的几何/引信阶段行，不依赖旧 `last_effect_*`。 | accepted |
...
- 现状：`tools/diagnostics/air_combat_weapon_employment_process_probe.py` 已有 `nearest_approach` 和 `fuze` 诊断行，但此前主要从 `EffectsEvent` 投影；无 `EffectsEvent` 的未触发/近失路径不能完整读出。
...
- 触碰文件：`tools/diagnostics/air_combat_weapon_employment_process_probe.py`、`tests/runtime/air_combat/test_diagnostics_probe_contracts.py`。
