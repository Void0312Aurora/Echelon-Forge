...
状态：`2026-06-09` MLF-2F accepted / MLF-2G next。`MLF-2B` 受控几何测试夹具已验收；`MLF-2C` 最近接近事件 writer 已验收；`MLF-2D` 引信评估事件 writer 已验收；`MLF-2E` 诊断投影已验收；`MLF-2F` runtime handoff gate 已验收。下一步进入验收收尾。
...
1. 下一步执行 `MLF-2G`，汇总证据并决定本子项目是否进入 accepted/held 收尾。
2. 收尾时必须继续保留破片、连续杆、结构解体、残骸、Pk 和具体弹种结论为后续阶段。
