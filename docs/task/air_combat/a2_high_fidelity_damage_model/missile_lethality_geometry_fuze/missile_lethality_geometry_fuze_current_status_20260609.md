...
Status: `2026-06-09` MLF-2F accepted / MLF-2G next. `MLF-2B` controlled geometry fixtures are accepted; `MLF-2C` nearest-approach writer is accepted; `MLF-2D` fuze-evaluation writer is accepted; `MLF-2E` diagnostics projection is accepted; `MLF-2F` runtime handoff gate is accepted. Acceptance closeout is next.
...
1. Next step is `MLF-2G`, evidence summary and accepted/held closeout.
2. Closeout must keep fragmentation, continuous rod, structural breakup, debris/wreck, Pk, and weapon-specific conclusions in later phases.
