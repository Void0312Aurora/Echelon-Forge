...
状态：`2026-06-09` finite task-cluster plan for [README.zh.md](README.zh.md)。MLF-2B、MLF-2C 和 MLF-2D 已验收，下一步进入 MLF-2E 诊断投影审计。
...
| `MLF-2E Diagnostics Projection` | future diagnostics worker | n/a | 让 process probe 按一枚弹输出 geometry/fuze 阶段行和 summary | `tools/diagnostics/**`、`tests/diagnostics/**`、本子项目证据记录 | reward 语义、runtime 物理判定、旧字段长期别名 | diagnostics pytest；controlled export sample | 不依赖 `last_effect_*` 判断触发原因；无起爆也能报告原因 | after 2C/2D API names freeze | 2 | planned / ready for read-only audit |
| `MLF-2F Runtime Handoff Gate` | integration worker | n/a | 将起爆状态交给现有效果模型，未触发路径明确不产生效果事件 | weapon lifecycle narrow path、effects invocation tests、diagnostics/reward guard tests | 战斗部机制、目标破碎、训练胜负、实体删除 | focused runtime tests + existing reward/diagnostics guard | 起爆才进入后续效果；未触发、失败、未解保不沉默消失 | after 2C/2D/2E | 2 | planned |
| `MLF-2G Acceptance And Archive Prep` | main thread | n/a | 汇总证据，更新状态、父级导航和残余地图 | 本子项目 README/status/task cluster/dispatch queue/archive index；A2 README | 过度声明真实 AIM-120C/MQ-9、Pk 或结构解体 | docs diff check + referenced focused tests | accepted/held 状态与证据一致，并明确 MLF-3+ 残余 | last, serial | 1 | planned |
