...
状态：`2026-06-09` finite task-cluster plan for [README.zh.md](README.zh.md)。MLF-2B、MLF-2C、MLF-2D 和 MLF-2E 已验收，下一步进入 MLF-2F runtime handoff gate 审计。
...
| `MLF-2F Runtime Handoff Gate` | integration worker | n/a | 将起爆状态交给现有效果模型，未触发路径明确不产生效果事件 | weapon lifecycle narrow path、effects invocation tests、diagnostics/reward guard tests | 战斗部机制、目标破碎、训练胜负、实体删除 | focused runtime tests + existing reward/diagnostics guard | 起爆才进入后续效果；未触发、失败、未解保不沉默消失 | after 2C/2D/2E | 2 | planned / ready for read-only audit |
