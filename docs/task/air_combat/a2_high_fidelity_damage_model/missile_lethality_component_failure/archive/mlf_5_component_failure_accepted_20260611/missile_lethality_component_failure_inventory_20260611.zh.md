...
- `git diff --check -- docs/task/air_combat/a2_high_fidelity_damage_model/missile_lethality_component_failure`：worker 返回与主线程验收均通过，无输出。
- `rg -n "[[:blank:]]$" docs/task/air_combat/a2_high_fidelity_damage_model/missile_lethality_component_failure`：主线程验收无匹配。
