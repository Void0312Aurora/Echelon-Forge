...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `PF-P0` | main thread | n/a | 创建符合 `docs/agent` 的 planning surface 和父级导航 | `docs/task/air_combat/a2_high_fidelity_damage_model/missile_lethality_proximity_fuze_realism/**`；父 A2 README | runtime 修改、测试、reward 修改 | Markdown inspection、本地链接、`git diff --check` | 子项目可导航，并明确当前 implementation 边界 | 无依赖 | 1 | pass |
