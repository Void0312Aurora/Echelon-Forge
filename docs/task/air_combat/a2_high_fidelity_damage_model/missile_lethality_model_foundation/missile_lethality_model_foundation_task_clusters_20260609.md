...
Status: `2026-06-09` finite task-cluster plan for [README.md](README.md).
...
This plan advances the generic missile lethality foundation only. It does not
tune AIM-120C/MQ-9 outcomes and does not claim real weapon Pk. The first round is
`MLF-1 Chain Contract`: standardize event and diagnostic fields before geometry,
fuze, fragmentation, continuous rod, structural breakup, and wreck/debris work.
...
| `MLF-1E` | main thread | n/a | Accept module boundary and decide whether to split `lethality_chain_contracts.h` | README, chain contract, task cluster, acceptance note | event store does not become a physics model | `git diff --check` + relevant 1B-1D tests | contract, diagnostics, and training-consumer boundaries are clear | after 1B-1D; serial | 1 | planned |
| `MLF-2A` | future geometry/fuze worker | n/a | Build controlled approach-geometry and fuze probe | weapon runtime/tests/diagnostics docs | no AIM-120C/MQ-9 outcome tuning | controlled geometry/fuze tests | distance/aspect explain trigger and no-trigger cases | after MLF-1 accepted | 2 | planned |
...
| --- | --- | --- |
| Fragmentation model missing | `MLF-3` | `MLF-2` geometry/fuze probe passes |
| Continuous rod/cutting model missing | `MLF-4` | generic fragmentation and component-load fields are reusable |
| Structural breakup missing | `MLF-6` | component damage and target-vulnerability fields are stable |
| Wreck/debris entities missing | `MLF-8` | structural failure can produce non-whole-airframe results |
| Pk/statistical layer missing | `MLF-9` | high-detail chain works; Pk remains low-detail or trend-check only |
