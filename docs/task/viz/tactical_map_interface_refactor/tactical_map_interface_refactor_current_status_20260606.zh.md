...
状态：`2026-06-06`，`P0` 已通过，`P1` 壳布局已作为切片接受，`P2` 地图工作区计划为下一步。
...
| Runtime 壳布局 | accepted slice | [P1 验收](tactical_map_interface_refactor_p1_shell_layout_acceptance_20260606.zh.md) | 不接受多地图工作区或新的仿真行为。 |
...
1. 决定 `VIZ-TMAP-P2` 第一版落 tabbed maps 还是 split-map mode。
2. 在 `VIZ-TMAP-P3` 增加分组图层/符号规则。
3. 只有当 runtime UI 需要稳定持久默认值时，才在 `VIZ-TMAP-P4` 扩展 profile UI 默认值。
4. 随后续实现切片落地，在 `VIZ-TMAP-P5` 记录浏览器 smoke、截图和残余。
