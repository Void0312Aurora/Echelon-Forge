...
|--------------|---------------|------|
| `tests/architecture/test_runtime_facade_layering.py` | design/boundary 主证据，覆盖 facade layering 与 raw-runtime escape hatch containment。 | 保留，是 WP5-B 最强 immediate anchor。 |
| `tests/architecture/test_cmake_target_readiness.py` | design 证据，覆盖 source grouping 与 mission controller ownership。 | 保留。 |
