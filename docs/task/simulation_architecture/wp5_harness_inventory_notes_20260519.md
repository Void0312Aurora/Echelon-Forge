...
|--------------|-------------------|----------------|
| `tests/architecture/test_runtime_facade_layering.py` | Primary design and boundary evidence for facade layering and raw-runtime escape-hatch containment. | Keep. This is the strongest immediate WP5-B smoke anchor. |
| `tests/architecture/test_cmake_target_readiness.py` | Design evidence for source grouping and mission-controller ownership. | Keep. It is low-level but valuable for architecture drift. |
