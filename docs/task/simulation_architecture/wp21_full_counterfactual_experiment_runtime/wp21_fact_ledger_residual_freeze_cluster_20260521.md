...
| Residual | Owner / disposition | What remains | Pass / block criterion |
|---|---|---|---|
| `WP21-B` Snapshot Restore And Worldline Boundary | `WP21-B` | Broaden the existing selected-slice runtime into a bounded, facade-owned snapshot/restore and worldline boundary with explicit barrier, seed, provider, and evidence refs. | Pass when restore stays bounded and fail-closed; block if it claims full restore, exact GPU, resident-state, or arbitrary unbounded worldline trees. |
| `WP21-C` Counterfactual Rollout And Causal Difference | `WP21-C` | Execute parent/branch worldlines and causal-difference runtime after B. | Pass when rollout consumes B's boundary and keeps raw mutation out; block until B is complete. |
| `WP21-D` Scenario Intervention Generation Runtime | `WP21-D` | Turn the WP15 request surface into deterministic scenario/intervention generation with non-mutation guards and artifact lineage. | Pass when generation is deterministic, metadata-backed, and non-mutating; block if it forces scenario schema migration or edits C++ rollout. |
| `WP21-E` Experiment Facade And Evidence Collection | `WP21-E` | Expose the maintained experiment facade, collect observations/comparisons/traces, and preserve ancestry without truth or support promotion. | Pass when evidence collection is ancestry-safe and non-promotional; block until C and D are complete. |
| `WP21-F` Final Cleanup And Acceptance Handoff | `WP21-F` | Integrate A-E, close legacy residuals, sync indexes/docs, and prepare final acceptance after implementation evidence exists. | Pass only after A-E are complete; block if it tries to accept planned docs alone. |
| `Retained compatibility: ScenarioLoader mirror` | retained compatibility | Keep `ScenarioLoader` and the Python world-batch mirrors gated as frontend/compatibility surfaces until WP18 split/gate work finishes. | Pass when the mirror stays narrow and guarded; block if it becomes maintained truth. |
| `Retained compatibility: typed setup baseline` | retained compatibility | Keep the WP20 typed setup path additive and compatibility-preserving; do not force scenario-schema migration or replace the legacy setup path wholesale. | Pass when typed setup remains additive; block if it is promoted into a mandatory mainline schema change. |
| `Retained compatibility: backend/resident-state boundary` | retained compatibility | Keep resident-state and exact-GPU support blocked, with `RuntimeFacade.runtime()` remaining diagnostics-only. | Pass when capability projection stays fail-closed; block any promotion of resident-state or exact GPU support. |
