...
| `WP7-B4 Layering Guard` | 确保 facade/core 不调用或链接 GPU helper/probe implementation 来投影 capability。 | `tests/architecture/test_runtime_facade_layering.py`。 | 中高。 |
...
以及 `tests/architecture/test_runtime_facade_layering.py`。
