...
| `WP18-D` | worker | `gpt-5.4`, high | 添加 maintained-path raw runtime/world/batch access guard prework，不删除 compatibility APIs。 | `tests/architecture/runtime_facade` 与 allowlist docs/comments；final hard bans 等 B/C。 |
...
- `bash tools/maintenance/cmo_env.sh python -m pytest -q tests/architecture/runtime_facade` 通过：`17 passed`。
...
- `git diff --check` 通过。
- `python3 tools/maintenance/wp_doc_closure_audit.py --wp WP18 --summary` 通过；WP18 仍在推进中，因此 acceptance review 继续保持缺席。
- `bash tools/maintenance/cmo_env.sh python -m pytest -q tests/architecture/runtime_facade` 通过：`18 passed`。
- `bash tools/maintenance/cmo_env.sh python -m pytest -q tests/runtime/execution/test_execution_episode_state.py` 通过：`5 passed`。
