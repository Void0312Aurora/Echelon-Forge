...
- 现有测试验证的是共存，而不是退场：
  ...
  后 legacy read，`tests/architecture/test_runtime_facade_layering.py:976-1008`
...
- 当前 architecture guards 是局部化，不是删除：
  `tests/architecture/test_runtime_facade_layering.py:499-607` 检查 `.batch_runtime`
  与 `RuntimeFacade.runtime()` consumers 留在显式 allowlist；
  `tests/architecture/test_wp16_legacy_path_gates.py:43-86` 在 replacement gates
...
- 现有 guards 已经局部化关键 escape hatches：
  `tests/architecture/test_runtime_facade_layering.py:499-607` 覆盖 `.batch_runtime`
  与 `RuntimeFacade.runtime()` allowlists；
  `tests/architecture/test_wp16_legacy_path_gates.py:43-86` 将 public compatibility
