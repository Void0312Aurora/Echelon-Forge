...
|---|---|
| `status` | `partial` |
| `commands run` | `git diff --check` -> pass; `python -m pytest -q tests/architecture/runtime_facade -k "runtime_facade_runtime_consumers or leader_world_batch_runtime_does_not_call_runtime_facade_runtime or batch_runtime"` -> collection-limited by missing `ef_py.ConditionalObjectiveProperty`; focused runtime opt-in/quarantine tests pass; focused env/world_batch tests `2+3+2` pass |
