...
default-factory spawn command projection ownership into `default_factory_spawn_command_projection.h`.
...
new `default_factory_spawn_command_projection.h` seed seam remains evaluation/guard
...
helper systems;
`default_factory_spawn_command_projection.h` owns the remaining default-factory
command projection seam and is still evaluation/guard-only in this pass.
...
| `F-004` | `default_unit_factory.h` is still a `1459`-line mixed header that builds capability bundles and spawn plans and performs entity spawn. It no longer direct-includes `legacy_command.h`; the remaining command mirror projection is isolated in `default_factory_spawn_command_projection.h`, which still blocks typed control-state closure. | `wc -l src/models/core/default_unit_factory.h src/components/command/default_factory_spawn_command_projection.h`; `src/models/core/default_unit_factory.h:14`; `src/models/core/default_unit_factory.h:1259-1277`; `src/components/command/default_factory_spawn_command_projection.h:3-58` | `migrate / scoped seed-narrowing pass` | Must coordinate with `WP22-D` because spawn ownership and legacy-command retirement share this file/helper pair. Only one worker should own the spawn-init range at a time. The helper seam remains evaluation/guard-only until typed control-state replacement lands. |
...
wc -l src/runtime/facade/runtime_facade.cpp src/runtime/facade/runtime_window_coordinator.h src/models/core/default_unit_factory.h src/components/command/default_factory_spawn_command_projection.h src/interfaces/python/bindings_core.cpp
...
rg -n "build_platform_capability_bundle_template|resolve_platform_spawn_plan|type_name_projection_preserved|legacy_command|default_factory_spawn_command_projection|spawn\\(" src/models/core/default_unit_factory.h src/components/command/default_factory_spawn_command_projection.h
```
...
- God-file counts should be remeasured before each structural dispatch; latest
  ...
  `default_factory_spawn_command_projection.h = 58`, and `bindings_core.cpp = 965`.
...
| `F-004` | `coordinate with WP22-D` | Spawn-plan extraction and command mirror projection are shared ownership areas with `WP22-D`; include both `default_unit_factory.h` and `default_factory_spawn_command_projection.h`. |
...
| `G-004` | `default_factory_spawn_command_projection.h` keeps a named `SpawnCommandProjectionControlStateSeed` seam and `default_unit_factory.h` calls it through a narrow helper | This is quarantined seed ownership, not typed control-state replacement. |
...
| `remaining blockers` | runtime-facade boundary mix remains structural debt; factory spawn and legacy-command ownership remain coupled through `default_factory_spawn_command_projection.h`; public raw `WorldBatchRuntime::world()` remains a compatibility/diagnostics surface; fat service and broad binding surfaces remain live |
...
- `remaining blockers`:
  ...
  `default_factory_spawn_command_projection.h`;
...
- `integration notes`:
  ...
  and any `default_unit_factory.h` / `default_factory_spawn_command_projection.h`
...
| `commands run` | `git diff --check` -> pass; `python3 -m pytest -q tests/architecture/platform_spawn/test_default_factory_spawn_command_projection.py tests/architecture/structural_boundaries tests/architecture/runtime_facade -k "wp22 or bindings or world_batch_runtime or gpu_visual_binding or visual_binding_raw_world_access or escape_hatch or batch_runtime"` -> `32 passed, 16 deselected`; `cmake --build build-workshop --target ef_py -j4` -> pass in the returned implementation packets. |
| `remaining blockers` | `runtime_facade.cpp = 2951`; `default_unit_factory.h = 1459` plus behavior-bearing `default_factory_spawn_command_projection.h`; broad binding surface remains; diagnostics/legacy raw ECS remains quarantined; raw `WorldBatchRuntime::world()` remains a public compatibility/diagnostics surface; `WorldBatchRuntime` still has a fat service surface. |
