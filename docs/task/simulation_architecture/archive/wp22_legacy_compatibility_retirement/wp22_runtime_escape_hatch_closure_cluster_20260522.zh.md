...
|---|---|
| `status` | `partial` |
| `commands run` | `git diff --check` -> 通过；`python -m pytest -q tests/architecture/runtime_facade/test_layering.py -k "runtime_facade_runtime_consumers or leader_world_batch_runtime_does_not_call_runtime_facade_runtime or batch_runtime"` -> 受缺失 `ef_py.ConditionalObjectiveProperty` 影响而被 collection 限制；聚焦 runtime opt-in/quarantine 测试通过；聚焦 env/world_batch 测试 `2+3+2` 通过 |
