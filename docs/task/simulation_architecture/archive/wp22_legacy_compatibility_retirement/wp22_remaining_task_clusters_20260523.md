...
| `default_factory_spawn_command_projection.h` | Still projects behavior-bearing `MovementCommand` / `LaggedCommand` mirrors. | `R1-B` default-factory projection deletion remains blocked after `R1-A`; repeated deletion attempts are a planning smell, not a new dispatch target. |
