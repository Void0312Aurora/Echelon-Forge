...
- diagnostics 与 viz 是人工/操作入口，应该随工具或可视化 runtime 迁移处理。
...
AST 口径统计结果：13 个活跃文件，15 个布尔 opt-in 调用。
...
| manual diagnostics | 1 | `tools/diagnostics/arma_proxy_backend_echelon_env.py` | 先作为 Arma proxy diagnostic 保留，后续随 proxy runtime 决策迁移或归档。 |
| manual visualization | 1 | `examples/viz/runtime/viz_session.py` | 先作为手动可视化入口保留，随 viz runtime 迁移处理。 |
...
| P3 | diagnostics/viz manual 入口 | 决定迁移到 maintained runtime adapter，或明确归档为 manual probe。 |
