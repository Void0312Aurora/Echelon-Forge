...
状态：`2026-06-10`，直接退役兼容入口并关闭宽 architecture guard 后 accepted。
...
routing 和 sensor routing 均已落地并通过聚焦验证。旧 domain-split 公开兼容路径现在
退役而不是保留：源码 consumer 直接 include common/domain owner header，generic
logistics/instrument 代码通过 `components/physics/propulsion_readouts.h` 读取 propulsion
readout，不再消费 Air system readout helper。刷新后的 build、runtime selector、退役
include search、structural selector、compatibility quarantine、完整 structural-boundary guard
与 scoped diff check 均通过。子项目 acceptance gate 已满足。
...
| Air runtime systems | canonical Air owner；旧 physics/tuning 公开路径已退役 | `src/systems/domains/air/**`, `src/components/domains/air/platform/**`；旧 physics/tuning 路径已删除 | pass | 聚焦 build/guard 证据已刷新 |
| Combat damage data | domain-owned header；旧公开聚合头已退役 | `src/components/combat/{common,air,naval,ground}/damage_*.h`；已删除 `src/components/combat/damage.h` | pass | include guard 证据已刷新 |
| Combat damage ECS | domain-owned system header；旧公开 registrar 已退役 | `src/systems/combat/damage_system_{common,air,naval,ground}.h`；已删除 `src/systems/combat/damage_system.h` | pass | include guard 证据已刷新 |
| Weapon data | domain-owned header；旧公开聚合头已退役 | `src/components/combat/{common,air,naval,ground}/weapon_*.h`；已删除 `src/components/combat/weapon.h` | pass | include guard 证据已刷新 |
...
| `src/systems/combat/damage_system.h` | Common fuze / event routing，加 Air/Naval/Ground domain-owned update system 或 adapter。 | 单个 header 同时 include `components/combat/damage.h`、`components/combat/weapon.h`、`components/domains/naval/platform/ship_platform.h`、physics、logistics、sensor、EW 和 effects interface。单个 `register_damage_system` 同时拥有 common `ProximityFuze`、Air `AircraftDamageStateUpdate` 和 Naval `NavalDamageStateUpdate`；没有 Ground update path。 | `rg` 显示 naval include 在第 19 行，`register_damage_system` 第 1171 行，`ProximityFuze` 第 1172 行，`AircraftDamageStateUpdate` 第 1690 行并由 `UnitType::Aircraft` / `UnitType::C2Node` gate，`NavalDamageStateUpdate` 第 1840 行并绑定 `ShipPlatform`。Air helper block 包含 structural envelope、sensor、fuel leak、cascade 与 component dependency consumer，范围约第 758-1124 行。 | `DS-S1-A`，但要等 `DS-C1-A` 稳定公开 component surface。 |
...
| `src/models/systems/default_sensor_model.cpp` | Common sensor model，加 Naval maritime adapter/router 处理 ship-specific state。 | generic sensor model 直接 include `components/domains/naval/platform/ship_platform.h`，读取 `ShipPlatform` 生成 maritime state，用 `UnitType::Ship` 触发 sea clutter，并读取目标舰艇高度做 radar horizon。 | `rg` 显示 naval include 第 5 行，`maritime_state_for` 中 `entity.get<ShipPlatform>()` 第 143 行，ship sea-state 字段第 149-151 行，`target_is_ship` / `UnitType::Ship` 第 173-174 行，目标 `ShipPlatform` 高度使用第 359-360 行。 | `DS-M1-B`。 |
| Air partial candidate：`src/systems/domains/air`、`src/components/air`、旧 wrapper | `systems/domains/air` / `components/air` 作为 canonical Air runtime/tuning owner；旧 physics 路径只保留 compatibility wrapper。 | Air owner 目录存在，并已被 `simulation_kernel_systems.cpp`、`content/unit_definition.h`、Python bindings 和 model factory 直接使用。旧 `systems/physics/{aero_state,aerodynamics,control,propulsion}_system.h` 与 `components/physics/flight_dynamics_tuning.h` 是 include-only wrapper。但该候选仍是 partial：Air systems 仍通过 `components/combat/damage.h` 获取 `AircraftDamageState`，且 generic physics/logistics 文件仍 include `systems/domains/air/propulsion_system.h`。 | `rg --files` 列出 `src/systems/domains/air/{aero_state_system.h,aerodynamics_system.h,control_system.h,propulsion_system.h}` 与 `src/components/domains/air/platform/flight_dynamics_tuning.h`。wrapper 检查显示旧 air physics header 只 include 新 Air header。include 使用显示 `simulation_kernel_systems.cpp` 第 36-44 行直接 include Air headers，并在第 182-187 行注册 Air systems；`systems/domains/air/aerodynamics_system.h` 与 `propulsion_system.h` 仍 include `components/combat/damage.h`。 | 现在验证 `DS-S1-B`；与 `DS-C1-A` 协调 Air damage type include dependency。 |
...
1. Naval/Ground effects 路径只作为 placeholder ownership shell 记录，不宣称完整 domain damage fidelity。
2. 若需要完整 Ground runtime 成熟度，再规划 Ground movement/sensing/fires/damage 实现包。
3. 校准和真实性升级继续与本 ownership split 分开。
