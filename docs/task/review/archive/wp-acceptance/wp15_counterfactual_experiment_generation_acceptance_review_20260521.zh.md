...
| `WP15-D Scenario And Adversary Generation Request Surface` | pass | `tests/scenario/test_scenario_generation_contracts.py` 已通过，且 `tests/scenario/test_scenario_compiler.py -k "wp15 or branch or runtime"` 已通过；request surface 保持 additive 且 non-mutating。 |
