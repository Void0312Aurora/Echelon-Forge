...
| `P3 Root-Cause Decision` | Decide the next model or environment contract. | P2 evidence exists. | Current status names the primary breakpoint and candidate remedies. | active |
| `P4 Remediation Plan` | Open the next implementation slice only after decision. | P3 accepted. | Event-window implementation evidence records whether direct actor supervision is sufficient. | held |
| `P5 Closure` | Sync parent indexes and archive superseded notes. | P4 direction exists or audit is intentionally held. | Parent docs point to the maintained evidence. | active |
...
- oracle diagnostics can reproduce the four breakpoints: hold, early high,
  legal pulse, and delayed legal pulse;
- the audit records whether release, effects, damage, health, and return are
  distinguishable;
- docs state that current evidence supports a learnability diagnosis, not a
  learned-policy success claim;
- the next implementation slice is blocked from opening until it chooses
  whether to repair action transport, reward/effects observability, or model
