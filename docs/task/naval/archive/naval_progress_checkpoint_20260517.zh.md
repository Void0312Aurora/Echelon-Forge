...
更新：`2026-05-18` 工作区复核补记。
以下补记只更新“当前工作区抽样复核”口径，不改写 `2026-05-17` 这份检查点作为阶段快照的主体叙事。
...
- `tests/runtime/naval/test_naval_screen_scenario.py`
- `tests/runtime/naval/test_naval_ship_database.py` 中与红方模板、抽象库存和补给窗口相关的定向用例
...
- `tests/runtime/naval/test_naval_ship_database.py` 中与结构化武器、`VLS-SAM`、主炮、`CIWS`、中间毁伤态、持续毁伤传播相关的定向用例
- 当前工作区抽样复核结果：
