<!-- Machine-translated draft generated on 2026-05-18 from docs/task/naval/naval_delegated_execution_backlog_20260517.zh.md. Review before treating this file as authoritative. -->
...
- [Naval Warfare Realism Hierarchical Checklist and Next Steps for Current Scenario](naval_realism_layering_and_next_step_plan_20260516.zh.md)
...
2. `Professional public materials`
   ...
   - Must be marked in documentation or JSON as "public mechanism / engineering approximation basis".

3. `Community / unofficial materials`
   - Only used for: initial engineering magnitudes, empirical parameters, conservative calibration starting points.
   - Must be explicitly marked as `engineering calibration` or `community-derived approximation`.

4. `Prohibited from being hardcoded as fact`
   - DDG-51 exact tactical diameter, fixed sea state speed loss, specific Harpoon batch public range, specific number of compartments, number of penetrated compartments, precise sonar detection range.

## V. Directly Issuable Main Tasks

If we start directly in the next round, I recommend issuing these three main tasks first, rather than me personally continuing to expand:

1. To `Hilbert`: Multi-sensor + maritime radar specialization + shipboard ESM MVP
2. To `Wegener`: Sea state input + wave attitude proxy + low-speed rudder effectiveness completion
3. To `Nietzsche`: Red force minimal enemy ship template + data link de-flooding + screening closed-loop stabilization

After these three are completed, decide whether to let `Galileo` open `VLS-SAM + damage state`.
