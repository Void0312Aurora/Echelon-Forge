<!-- Machine-translated draft generated on 2026-05-18 from docs/task/naval/naval_progress_checkpoint_20260517.zh.md. Review before treating this file as authoritative. -->
...
- [Naval Warfare Realism Layered Checklist and Next Steps for Current Scenario](naval_realism_layering_and_next_step_plan_20260516.zh.md)
- [Naval Warfare Subsequent Task Execution Sheet](naval_delegated_execution_backlog_20260517.zh.md)
...
- `tests/runtime/test_naval_screen_scenario.py`
- Directed test cases related to red force templates, abstract inventory, and replenishment windows in `tests/runtime/test_naval_ship_database.py`
...
- `5in gun / CIWS` have advanced from pure database items to runtime structural surfaces, but the engagement chain is not yet stably closed.
...
- The command-driven path `MissionCommand -> CIWS` has been integrated into the mainline code, but the directed tests in the current workspace are still not green; it cannot be described as "completed automatic close-in defense loop."

Validation:

- Directed test cases related to structured weapons, `VLS-SAM`, main gun, `CIWS`, intermediate damage states, and continuous damage propagation in `tests/runtime/test_naval_ship_database.py`
...
- The main gun direct fire chain is not yet stably passing directed regression.
- The `MissionCommand -> CIWS` command-driven chain is not yet stably passing directed regression.
...
4. Engagement layer:
   - It has entered an operational skeleton of "minimal `VLS-SAM` + main gun/CIWS structured runtime + intermediate damage states + continuous damage propagation," but the main gun direct fire and `MissionCommand -> CIWS` command chain are still not green.
...
- The currently stably reproducible naval red flags are specifically on the main gun direct fire and `MissionCommand -> CIWS` command path, rather than a generic `sensor/naval` summary.
- `screen-hold` is no longer suitable as an independent red flag occupying top priority.

### 4.2 Secondary Suggestions

It is recommended to continue with:

1. If necessary, supplement a more complete maritime state linkage scenario
2. Review whether occasional read / `MemoryError` noise under `pytest` environment is unrelated to current logic

Rationale:

- Current naval warfare needs to cooperate with the mainline to close sensor/linkage risks, rather than continuing to expand the single functional surface.

### 4.3 Deferrable Suggestions

It is recommended to continue with:

1. If needed, further connect the main gun trigger path to a higher-level behavior layer
2. Continue verifying the stability of the `MissionCommand`-driven shipborne weapon chain in more complex scenarios

Rationale:

- The `CIWS` command-driven path has been integrated into the code, but directed regression is not yet green; the next step remains closure rather than pure expansion.

### 4.4 Deferrable Suggestions

It is recommended to continue with:

1. If necessary, supplement finer runtime assertions for `wave_heading / wave_period`
2. Keep maritime state rule documentation and tests synchronized with actual implementation

Rationale:

- The major structure of this line is complete; the remainder is mainly finer test coverage, not further semantic expansion.

## 5. Current Priority

It is suggested to handle in the following order:

1. First close the two stable red flags of the naval weapon command chain.
2. Then cooperate with the mainline to close `DataLink / MissionCommand / naval` shared semantics and maritime sensor linkage.
3. Finally decide whether to continue expanding `MissionCommand -> gun`, more complex maritime linkage scenarios, or finer wave runtime assertions.

Benefits of this approach:

- The most valuable role of naval warfare currently is as a high-value acceptance surface, not continuing to expand features independently.
- It is more beneficial to clear the currently stably reproducible naval weapon command chain red flags first before continuing expansion.
