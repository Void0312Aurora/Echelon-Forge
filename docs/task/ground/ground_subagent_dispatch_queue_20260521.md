...
- `G6-E0` records the minimum native ground platform schema package.
- `G6-E1` accepts the source-inventory/design preflight: E2 should use
...
| `G2-C` | main-thread integration | current main thread | Accepted: integrate G2 worker results, validation, status docs, and G3 residuals. | Archived evidence under `docs/task/ground/archive/g2_content_test_seed/**`; original path is a pointer README only. This dispatch queue and validation only. |
...
| `G3-D` | main-thread integration | current main thread | Integrate G3-A/B/C into the authoritative G3 packet and decide whether G4 can be released. | Archived evidence under `docs/task/ground/archive/g3_execution_surface_design/**`; original path is a pointer README only. `docs/task/ground/README*.md` and queue sync only. |
...
| `G6-A` | worker | `gpt-5.4`, medium | Accepted: create the realism-gradient MVP planning surface. | Archived evidence under `docs/task/ground/archive/g6_realism_gradient_mvp_scenarios/**`; original path is a pointer README only. |
...
| `G6-C` | main-thread integration | current main thread | Accepted: route-move boundary guardrails without releasing movement behavior. | Archived evidence under `docs/task/ground/archive/g6_route_move_boundary/**`; original path is a pointer README only. Runtime/test paths and ground README/queue/progress sync only. |
| `G6-D0` | main-thread integration | current main thread | Accepted: open the route-move release decision and select the schema-first path. | Archived evidence under `docs/task/ground/archive/g6_route_move_release_decision/**`; original path is a pointer README only. Ground README/queue/progress/plan sync only. |
...
| `G6-E0` | main-thread integration | current main thread | Accepted: plan the minimal native ground platform schema implementation package. | Archived evidence under `docs/task/ground/archive/g6_native_ground_platform_schema/**`; original path is a pointer README only. Ground README/queue/progress/plan sync only. |
