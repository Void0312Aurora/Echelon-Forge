...
| `2.3` LOS 率计算使用真实目标位置 | `未解决` | truth-based target state 依赖仍未真正切断 |
...
| `6.2` 部件毁伤是二值瞬杀 | `未解决` | 连续退化式 subsystem damage 仍未接通 |
...
| `8.2` 发射高度/速度对 `Pk` 影响为零 | `部分解决` | 动力学已不再完全无关，但更真实 envelope/`Pk` 建模仍缺 |
