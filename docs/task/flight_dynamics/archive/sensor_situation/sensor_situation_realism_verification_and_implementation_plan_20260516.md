...
- The main detection chain from [default_sensor_model.cpp:127](../../../../src/models/systems/default_sensor_model.cpp) to [default_sensor_model.cpp:212](../../../../src/models/systems/default_sensor_model.cpp) is still:
  `FOV/range gating -> range_factor -> aspect_factor -> doppler_factor -> weather/sun factor -> detection_prob`
- `signal_strength` is calculated only after [default_sensor_model.cpp:235](../../../../src/models/systems/default_sensor_model.cpp) and is used only for outputting `Detection.signal_strength`, without feeding back into detection success probability.
...
- [SensorSystem:47](../../../../src/systems/systems/sensor_system.h) directly generates `fresh_contacts` per scan.
- [TrackManagerSystem:131](../../../../src/systems/systems/track_manager_system.h) creates a track immediately upon first seeing a contact.
...
- [track_manager_system.h:135](../../../../src/systems/systems/track_manager_system.h) and [track_manager_system.h:183](../../../../src/systems/systems/track_manager_system.h) both "directly overwrite with latest measurement."
- [SystemTrack:24](../../../../src/components/systems/track_management.h) has `vx/vy/vz`, but the current system has no path to assign values.
...
- [data_link_system.h:103](../../../../src/systems/systems/data_link_system.h) directly merges the sender's `ContactList` into the receiver's `ContactList`.
- [data_link_system.h:143](../../../../src/systems/systems/data_link_system.h) also sends `ReportContact` via `CommQueue`.
- The message uses the target entity's true coordinates [data_link_system.h:147](../../../../src/systems/systems/data_link_system.h), not the sender's measurement or filtered estimate.
...
- Both local detections and data link tracks use this path [track_manager_system.h:141](../../../../src/systems/systems/track_manager_system.h), [track_manager_system.h:190](../../../../src/systems/systems/track_manager_system.h).
...
- `check_line_of_sight()` only checks if endpoints are underground [default_environment_model.cpp:147](../../../../src/models/environment/default_environment_model.cpp).
- `get_weather_attenuation()` always returns `0.0` [default_environment_model.cpp:153](../../../../src/models/environment/default_environment_model.cpp).
- Sun interference is just a multiplication by `0.1` for visual/infrared detection probability [default_sensor_model.cpp:106](../../../../src/models/systems/default_sensor_model.cpp).
...
- `TrackSource::Fused` already exists in [track_management.h:7](../../../../src/components/systems/track_management.h).
- The observation interface has already exposed `source` and `classification` to the agent observation [simulation_kernel_observation_api.cpp:214](../../../../src/core/engine/simulation_kernel_observation_api.cpp).
...
- The data link already has a standard approximate formula `3.57 * (sqrt(h1) + sqrt(h2))` [data_link_system.h:91](../../../../src/systems/systems/data_link_system.h).
- But sensor LOS still only has endpoint underground determination [default_environment_model.cpp:147](../../../../src/models/environment/default_environment_model.cpp).
...
- In [default_sensor_model.cpp:252](../../../../src/models/systems/default_sensor_model.cpp) onward, if a target has noise suppression jamming and distance is greater than burn-through distance, it directly `return`s.
...
- The `Sensor` comment says radar should be close to `R^4` [sensor.h:19](../../../../src/components/systems/sensor.h).
...
- The loader reads `has_data_link` and `data_link_network_id` [unit_definition_loader.cpp:250](../../../../src/content/unit_definition_loader.cpp).
- But the default factory actually forces assignment by side: `Blue=1, Red=2` [default_unit_factory.h:475](../../../../src/models/core/default_unit_factory.h).
...
Currently [data_link_system.h:103](../../../../src/systems/systems/data_link_system.h) directly places the sender’s contacts into the receiver’s contact list, which confuses “local detection” with “shared track”.
...
The current class already has `weather_zones_`, but it is not actually used [default_environment_model.cpp:70](../../../../src/models/environment/default_environment_model.cpp).
