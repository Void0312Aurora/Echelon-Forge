...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `M1-AS-A Evidence And Boundary` | main thread | current main thread | Record why `full` continuous switch dimensions are training-hostile while runtime release gates are still valid. | `docs/learning/reviews/air_combat_action_interface_split_20260602/**`, `docs/learning/README*` | code implementation, M2 release, missile kernel changes | `git diff --check -- docs/learning` | README and cluster docs link parent model entry and refuse overclaims | serial first | 1 + 1 repair | pass |
| `M1-AS-B Action Contract Design` | main thread or future integration worker | high-reasoning design | Define the accepted air-combat action surface, including field names, reset behavior, held behavior, selector ranges, pulse duration and `proprio` semantics. | `docs/domains/air/standards/pilot_action_contract.md`, `docs/domains/air/standards/pilot_action_contract.zh.md`, `gym_envs/universal_env_parts/spaces.py`, `gym_envs/universal_env_parts/actions.py`, focused tests | tactical memory, damage, missile envelope changes | env-config tests, action adapter unit tests, markdown diff check | contract names a stable mode and all switch/pulse semantics | after `M1-AS-A`; serial with implementation touching same action tables | 1 + 1 repair | pass |
...
git diff --check -- docs/learning docs/domains/air gym_envs python examples/config/training/active/air_combat tests tools
```
