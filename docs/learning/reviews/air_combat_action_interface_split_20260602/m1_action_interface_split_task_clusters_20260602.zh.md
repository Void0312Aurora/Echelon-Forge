...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `M1-AS-A Evidence And Boundary` | main thread | current main thread | 记录为什么 `full` 连续开关维度不利于训练，同时说明 runtime release gate 仍有效。 | `docs/learning/reviews/air_combat_action_interface_split_20260602/**`, `docs/learning/README*` | 代码实现、M2 release、导弹内核改动 | `git diff --check -- docs/learning` | README 与 cluster docs 链接父 model 入口并拒绝过度声明 | 首先串行 | 1 + 1 repair | pass |
