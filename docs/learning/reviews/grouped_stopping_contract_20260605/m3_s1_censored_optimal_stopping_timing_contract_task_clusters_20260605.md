...
| `M3S1-P6 Closure And Archive Sync` | main thread | current main thread | Sync model/A7/M3 indexes and archive superseded local repair docs only if replacement evidence exists. | `docs/learning/**`, selected A7 docs/archive pointers | deleting evidence, claiming success without probes | `git diff --check -- docs/learning docs/task/air_combat/a7_event_value_advantage_credit_head` | Docs distinguish accepted slices, held learned behavior, and residuals. | after P5 | 1 | held |
