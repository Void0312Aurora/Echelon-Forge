...
语言：
- 英文规范版：[multi_agent_cooperative_training_foundation_and_performance_plan.md](multi_agent_cooperative_training_foundation_and_performance_plan.md)
- 中文伴随版：`multi_agent_cooperative_training_foundation_and_performance_plan.zh.md`
...
Content status: 迁移后的调研快照；本文提供方向与风险，不是 active implementation package 或验收权威。

状态：基于 `2026-05-11` 调研快照的 draft issue。
...
- 已新增 `python/rl/support/multi_agent_benchmark.py` 与 `scripts/benchmark_multi_agent.py`，形成可重复执行的性能基线入口。
- 当前基线已覆盖：
  - `single_agent`
  - `leader`
