...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `DCR-A Boundary And Observable Map` | main thread | n/a | 把 held idea 升级为 active A2 follow-on，并列出可读的后果来源。 | `docs/learning/work/active/air_combat_damage_consequence_reward/**`，必要时 A2 指针 README | 改标准文档、创建 A9、重开 sealed A2 archive | `git diff --check -- docs/learning/work/active/air_combat_damage_consequence_reward` | README 和任务簇存在，且禁止声明清楚。 | first, serial | 1 | pass |
...
| `DCR-F Closure And Index Sync` | main thread | n/a | 按证据标记 accepted slice 或 residual，并同步父级指针。 | 本工作包、`docs/learning/README*` 与 `docs/domains/air/README*` | 过度声明真实杀伤或 Stage-2 最终验收 | docs diff check 和聚焦测试 | status line 与 residual map 和证据一致。 | last, serial | 1 | planned |
