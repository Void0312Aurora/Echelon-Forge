# The National Interest — J-15 specifications

Source ID: `p5-cn-air-j15-tni`
Tier: `C`
Publisher: The National Interest
Author / maintainer: The National Interest editorial staff; article by Brandon J. Weichert
Title: The J-15 Flying Shark Fighter Is Just What China's Aircraft Carriers Need
URL: https://nationalinterest.org/blog/buzz/j-15-flying-shark-fighter-just-what-chinas-aircraft-carriers-need-211360
Retrieval:
  attempted_at: 2026-09-17T17:05:00Z
  method: none
  status: no_record
  returned: nothing
  did_not_return: not established
  note: This package carried a hand-written Accessed date of 2026-09-18 or 2026-09-19, which is later than the machine clock at the time of this correction pass. No retrieval record exists for this URL, so the date is removed and no machine timestamp is substituted for it. The values in this manifest are transcribed values with no audit-grade retrieval behind them.
Domain: air
Equipment: J-15
Configuration: Carrier-based multirole fighter derived from the Su-27 family. The baseline J-15 is a single-seat aircraft; the twin-seat derivative is the separate J-15S.
Estimation / uncertainty: Tier C specialist secondary source. It is a defence commentary publication, not an official Chinese source and not a manufacturer document. The Chinese Ministry of National Defense releases held in this tree confirm service presence only and disclose no dimensions, so these values are the best available and are labelled as secondary.
Retention: manifest and extracted parameter notes only

## Extracted parameter notes

Length 21.9 m; height 5.9 m; wingspan 14.7 m. Empty weight approximately 17,500 kg; maximum takeoff weight 33,000 kg. Powerplant two Shenyang WS-10 turbofans, 12,800 kgf thrust each in the article's units. Top speed Mach 2.4; range 3,500 km with external fuel tanks.

## Notes on this page's own content

The page gives the aircraft's dimensions, empty weight, maximum takeoff weight, engine and thrust, top speed, range with external tanks, and no hardpoint count.

Two claims on this page are contradicted or not corroborated elsewhere and must be read carefully. First, the range figure of 3,500 km is stated for external tanks, while `p5-cn-air-j15-deagel` places a 3,500 km reading on the leaf as a combat radius; the two are different quantities and the leaf records both readings. Second, the page does not state a seat configuration.

The 12-hardpoint count and the engine variant detail come from a separate Tier C database package, p5-cn-air-j15-deagel, not from this page.
