...
Content status: parameter table present with per-field source and confidence. Retrieval status is recorded per package in each manifest's `Retrieval:` block. This leaf previously carried no parameter table at all; the table below was built on 2026-09-17. The single-seat and twin-seat airframes are not separated by any source held here, and that is stated rather than papered over.
...
The single-seat and twin-seat airframes are not separated. This leaf is named for the J-11B and the J-11BS together, and no package held here states a J-11BS dimension, mass or crew figure. The encyclopedic specifications block gives crew 1, which is the single-seat B airframe, and the Mitchell entry gives one set of figures for the whole family with no per-mark split. Every geometry and mass row above is therefore a family or single-seat reading, labelled as such, and none is asserted as the twin-seat airframe's own. The twin-seat airframes of this lineage are longer than the single-seat ones, so the 21.9 m length in particular must not be read as the BS figure.
...
- `p5-cn-air-j11b-encyclopedic`: `raw/sources/wikipedia/p5-cn-air-j11b-encyclopedic/manifest.md` — metric geometry, masses, fuel, powerplant, single-seat crew
