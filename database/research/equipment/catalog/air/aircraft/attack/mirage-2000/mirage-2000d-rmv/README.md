...
| Powerplant (independent reading) | Same SNECMA M53-P2; 64.3 kN (14,455 lbf) dry and 95.1 kN (21,379 lbf) with afterburner | `p5-fr-air-mirage2000d-flugzeuginfo` | C | A second package states the same engine model and states in prose that the 2000D is the two-seat fighter-bomber variant. Its table is labelled by the page itself as Mirage 2000C, so the thrust pair is a 2000C reading. Its dry figure is 10.3 kN above the compilation's 54 kN and its afterburner figure is 0.1 kN above the compilation's 95 kN. Both readings are retained and the dry-thrust conflict is not resolved, because neither page states which rating it is describing |
...
| Maximum speed (independent reading) | 2,334 km/h | `p5-fr-air-mirage2000d-flugzeuginfo` | C | Mirage 2000C table reading, 6 km/h below the compilation's 2,340 km/h. Both are retained |
| Service ceiling (independent reading) | 17,983 m (59,000 ft) | `p5-fr-air-mirage2000d-flugzeuginfo` | C | Mirage 2000C table reading, 923 m above the compilation's 17,060 m. Both are retained |
...
A second package, `p5-fr-air-mirage2000d-flugzeuginfo`, states the same engine model and names the 2000D in prose as the two-seat fighter-bomber variant. It supplies an independent powerplant reading and two further performance readings, so the engine fit and the performance envelope now rest on two publishers rather than one. Its table is labelled by the page itself as Mirage 2000C, and it is recorded as a 2000C table rather than as a family or 2000D one. The dry-thrust split is the widest disagreement this opened: 54 kN against 64.3 kN for the same engine. It is recorded and not resolved, because the two packages may be describing different ratings and neither states which.

The performance rows remain single-seat-baseline readings from both packages: one on a 2000C baseline with a caveat, one on a 2000C table without one. The 2000D is a two-seat airframe with its own stores and mission fit, and no package held here states a 2000D-measured speed, ceiling or radius, so the leaf does not present these as such.
