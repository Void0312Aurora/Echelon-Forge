...
When a catalog leaf is shared by multiple operator rows, the domain queue keeps
one row per operator/context and the leaf keeps the concrete variant identity.
The queue `equipment_id` is not required to equal the leaf's ID; the binding is
the `catalog_path` plus an explicit operator/scope note in the leaf. This keeps
shared variants separate from operator-specific claims.
