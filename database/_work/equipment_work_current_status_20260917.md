...
| `C3` backlog vs coverage status | PASS, 75 of 75 coverage rows agree |
...
| `C6` retrieval record | PASS, 67 unretrieved citations remain advisory; 163 packages lack a retrieval block |

Measured counts:

| Metric | Value |
| --- | --- |
| Source packages (manifests) | 279 |
| Catalog leaves (carry `## Parameters`) | 109 |
| Distinct source ids referenced by leaves | 279 |
| Leaves without `Equipment ID` | 0 |
| Backlog rows | 118 |
| Coverage rows | 75 |
| Status: `cataloged` / `parameter_complete` / `held` | 24 / 86 / 8 |

## Leaf Completeness Against The Queue

The queue calls 24 rows `cataloged`, and all 24 now point at leaves carrying a
...
behave that way: its 75 rows are exactly the 75 in-scope `parameter_complete` rows of the
...
| --- | --- | --- | --- |
| `D2` | No source package records a rights field | `C4` `missing_rights_field_advisory`, 279 of 279 | open; the admission standard requires it of a ledger row |
...
| `D7` | `coverage.csv` role and status vocabulary contradict its own README | 75 of 75 rows are `parameter_complete`, not `queued` | open |
...
- The 85 `parameter_complete` rows are research drafts. They are not calibrated, not
