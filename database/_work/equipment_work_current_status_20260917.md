...
| Tracked files under `database/` | 620 |
...
| `C3` backlog vs coverage status | PASS, 85 of 85 coverage rows agree |
...
| Metric | Value |
| --- | --- |
| Source packages (manifests) | 291 |
| Catalog leaves (carry `## Parameters`) | 109 |
| Distinct source ids referenced by leaves | 291 |
| Leaves without `Equipment ID` | 0 |
| Backlog rows | 118 |
| Coverage rows | 85 |
| Status: `cataloged` / `parameter_complete` / `held` | 14 / 96 / 8 |
...
behave that way: its 85 rows are exactly the 85 in-scope `parameter_complete` rows of the
...
| --- | --- | --- | --- |
| `D2` | No source package records a rights field | `C4` `missing_rights_field_advisory`, 291 of 291 | open; the admission standard requires it of a ledger row |
...
| `D7` | `coverage.csv` role and status vocabulary contradict its own README | 85 of 85 rows are `parameter_complete`, not `queued` | open |
...
- The 96 `parameter_complete` rows are research drafts. They are not calibrated, not
