...
| Tracked files under `database/` | 601 |
...
| `C3` backlog vs coverage status | PASS, 51 of 51 coverage rows agree |
...
| `C6` retrieval record | PASS, 73 unretrieved citations remain advisory; 183 packages lack a retrieval block |

Measured counts:

| Metric | Value |
| --- | --- |
| Source packages (manifests) | 273 |
| Catalog leaves (carry `## Parameters`) | 109 |
| Distinct source ids referenced by leaves | 273 |
| Leaves without `Equipment ID` | 0 |
| Backlog rows | 118 |
| Coverage rows | 51 |
| Status: `cataloged` / `parameter_complete` / `held` | 48 / 62 / 8 |

## Leaf Completeness Against The Queue

The queue calls 48 rows `cataloged`, and all 48 now point at leaves carrying a
...
behave that way: its 51 rows are exactly the 51 `parameter_complete` rows of the
...
| --- | --- | --- | --- |
| `D2` | No source package records a rights field | `C4` `missing_rights_field_advisory`, 273 of 273 | open; the admission standard requires it of a ledger row |
...
| `D7` | `coverage.csv` role and status vocabulary contradict its own README | 51 of 51 rows are `parameter_complete`, not `queued` | open |
...
| --- | --- | --- |
| Dangling citation `p5-us-ground-m1a2sepv3-armytechnology` on the M1A2 SEP v3 leaf | An audit script that resolved source packages by declared `Source ID` under only part of the `raw/sources/` tree | The package exists at `raw/sources/army_technology/p5-us-ground-m1a2sepv3-armytechnology/manifest.md`, its `Source ID` line matches, and its own text names the geometry, crew, AGT1500, M256, FLIR, data-link and APU statements the leaf cites it for. The then-current C1 run confirmed 0 dangling ids; the present tree also resolves all 273 manifests |
| `coverage.csv` status not trustworthy | The file marked all 51 rows `parameter_complete` and held only 14 air rows against 70 in `air.csv` | The `air.csv` comparison was the observation, the conclusion was wrong. Both directions of the coverage-to-backlog comparison agree, so `C3` passes. The real defect is documentary and is now `D7` |
...
- The 51 `parameter_complete` rows are research drafts. They are not calibrated, not
