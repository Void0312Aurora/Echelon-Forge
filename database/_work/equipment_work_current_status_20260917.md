...
| `C3` backlog vs coverage status | PASS, 55 of 55 coverage rows agree |
...
| `C6` retrieval record | PASS, 73 unretrieved citations remain advisory; 179 packages lack a retrieval block |
...
| Backlog rows | 118 |
| Coverage rows | 55 |
| Status: `cataloged` / `parameter_complete` / `held` | 44 / 66 / 8 |

## Leaf Completeness Against The Queue

The queue calls 44 rows `cataloged`, and all 44 now point at leaves carrying a
...
behave that way: its 55 rows are exactly the 55 `parameter_complete` rows of the
...
| `D7` | `coverage.csv` role and status vocabulary contradict its own README | 55 of 55 rows are `parameter_complete`, not `queued` | open |
...
- The 55 `parameter_complete` rows are research drafts. They are not calibrated, not
