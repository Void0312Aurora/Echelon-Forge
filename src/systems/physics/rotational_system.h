...
inline void register_rotational_integration_system(flecs::world &ecs) {
    ecs.system<Transform, AngularVelocity, const Inertia, const ForceAccumulator>(
           "RotationalIntegrate")
        .kind(flecs::OnUpdate)
        .run([](flecs::iter &it) {
            while (it.next()) {
                ...
                for (auto i : it) {
                    ...
                    // observations finite.
                    double p = clamp_finite(ang_vel[i].p, -prm.max_rate_cross_rad_s,
                                            prm.max_rate_cross_rad_s);
                    double q = clamp_finite(ang_vel[i].q, -prm.max_rate_cross_rad_s,
                                            prm.max_rate_cross_rad_s);
                    double r = clamp_finite(ang_vel[i].r, -prm.max_rate_cross_rad_s,
                                            prm.max_rate_cross_rad_s);

                    // Also cap applied moments; control laws can generate stiff dynamics at high
                    // qbar.
                    double L =
                        clamp_finite(forces[i].torque_roll, -prm.max_torque_nm, prm.max_torque_nm);
                    double M_moment =
                        clamp_finite(forces[i].torque_pitch, -prm.max_torque_nm, prm.max_torque_nm);
                    double N =
                        clamp_finite(forces[i].torque_yaw, -prm.max_torque_nm, prm.max_torque_nm);
                    ...
                    // Integrate Rates
                    p += clamp_finite(p_dot, -prm.max_ang_accel_rad_s2, prm.max_ang_accel_rad_s2) *
                         dt;
                    q += clamp_finite(q_dot, -prm.max_ang_accel_rad_s2, prm.max_ang_accel_rad_s2) *
                         dt;
                    r += clamp_finite(r_dot, -prm.max_ang_accel_rad_s2, prm.max_ang_accel_rad_s2) *
                         dt;
                    ...
                    // In real life, aero damping (C_lp, C_mq, C_nr) would be in the "Forces"
                    // (Torques). If forces don't have damping, this spins forever. For now, let's
                    // assume "Forces" will eventually contain damping. But to start, apply a small
                    // decay if no torque? No, let's trust the physics. If it spins, it spins.
                    ...
                    transform[i].pitch = std::max(
                        -prm.pitch_limit_deg, std::min(prm.pitch_limit_deg, transform[i].pitch));
