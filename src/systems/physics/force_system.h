...
inline double canonicalize_direction_scalar(double value) {
    ...
    const double rounded =
        std::nearbyint(value / kDirectionScalarCanonicalQuantum) * kDirectionScalarCanonicalQuantum;
...
inline void register_force_system(flecs::world &ecs) {
    ecs.system<ForceAccumulator, const Transform, const Velocity, const Mass, const Propulsion,
               const FlightModel>("ComputeForces")
        .kind(flecs::OnUpdate)
        .run([](flecs::iter &it) {
            while (it.next()) {
                ...
                for (auto i : it) {
                    ...
                    double nose_x =
                        canonicalize_direction_scalar(std::cos(yaw_rad) * std::cos(pitch_rad));
                    double nose_y =
                        canonicalize_direction_scalar(std::sin(yaw_rad) * std::cos(pitch_rad));
                    double nose_z = canonicalize_direction_scalar(std::sin(pitch_rad));
                    double thrust_magnitude = propulsion[i].current_thrust_n;

                    const double thrust_fx =
                        canonicalize_projected_force_scalar(thrust_magnitude * nose_x);
                    const double thrust_fy =
                        canonicalize_projected_force_scalar(thrust_magnitude * nose_y);
                    const double thrust_fz =
                        canonicalize_projected_force_scalar(thrust_magnitude * nose_z);

                    forces[i].add_force(thrust_fx, thrust_fy, thrust_fz);
