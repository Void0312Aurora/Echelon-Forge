...
inline double canonicalize_propulsion_force_scalar(double value) {
    ...
    const double rounded =
        std::nearbyint(value / kPropulsionForceCanonicalQuantum) * kPropulsionForceCanonicalQuantum;
...
inline EngineTuning
resolve_propulsion_runtime_tuning(const Propulsion &propulsion, const EngineTuning *attached_tuning,
                                  const AircraftDamageState *aircraft_damage = nullptr) {
    ...
    if (aircraft_damage && attached_tuning && attached_tuning->mil_thrust_n > 1.0) {
        const double propulsion_scale =
            aircraft_damage_capability_floor(aircraft_damage->propulsion_integrity, 0.15);
        runtime_tuning.mil_thrust_n *= propulsion_scale;
        runtime_tuning.ab_thrust_n =
            std::max(runtime_tuning.mil_thrust_n, runtime_tuning.ab_thrust_n * propulsion_scale);
    }
    return runtime_tuning;
}

inline PropulsionAtmosphereInputs
resolve_propulsion_atmosphere_inputs(const Transform &transform, const Velocity &velocity,
...
inline void advance_propulsion_state(Propulsion &propulsion, const EngineTuning &tuning,
                                     double throttle_command, double dt,
    ...
    const double throttle_target = (throttle_command <= tuning.throttle_ab_threshold)
                                       ? throttle_command
    ...
    propulsion.throttle_state =
        first_order_step(propulsion.throttle_state, throttle_target, dt, spool_tau);
    ...
    propulsion.dry_thrust_state_n = first_order_step(
        propulsion.dry_thrust_state_n, propulsion.dry_thrust_command_n, dt, spool_tau);
    propulsion.dry_thrust_state_n =
        std::clamp(propulsion.dry_thrust_state_n, 0.0,
    ...
    propulsion.current_thrust_n =
        canonicalize_propulsion_force_scalar(std::max(0.0, propulsion.current_thrust_n));
    propulsion.afterburner_active = propulsion.ab_state >= 0.15;
    propulsion.current_tsfc =
        propulsion.afterburner_active ? tuning.tsfc_ab_kg_per_nh : tuning.tsfc_mil_kg_per_nh;
}

inline double propulsion_fuel_flow_kg_per_s(const Propulsion &propulsion,
            ...
            while (it.next()) {
                ...
                for (auto i : it) {
                    ...
                    const ResolvedAirControlInput control_input = resolve_air_control_input(
                        entity.get<PilotAction>(), entity.get<MissionCommandControlState>(),
                        nullptr, nullptr, 0.0);
                    const double throttle_command = control_input.throttle_command;
                    const EngineTuning runtime_tuning =
                        resolve_propulsion_runtime_tuning(propulsion[i], entity.get<EngineTuning>(),
                                                          entity.get<AircraftDamageState>());
                    const PropulsionAtmosphereInputs atmosphere =
                        resolve_propulsion_atmosphere_inputs(transform[i], velocity[i], &aero[i],
                                                             env_ref);
                    advance_propulsion_state(propulsion[i], runtime_tuning, throttle_command, dt,
                                             atmosphere);
                }
            }
        });
