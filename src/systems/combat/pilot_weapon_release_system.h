...
inline void register_pilot_weapon_release_system(flecs::world &ecs) {
    ecs.system<const PilotAction>("PilotWeaponRelease")
        .kind(flecs::OnUpdate)
        .each([](flecs::entity e, const PilotAction &pilot) {
            if (!pilot.active || !pilot.master_arm || !pilot.fire_weapon) {
                if (PilotWeaponReleaseState *state = e.get_mut<PilotWeaponReleaseState>()) {
                    state->fire_weapon_was_down =
                        bool(pilot.active && pilot.master_arm && pilot.fire_weapon);
            ...
            flecs::entity launched = service_ref->service->fire_weapon_from_pilot_action(
