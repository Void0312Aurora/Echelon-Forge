# `src/systems/domains/air` 边界

`systems/domains/air` 拥有 air-domain flight control、aerodynamic state、propulsion
...
- 消费 `components/domains/air/platform`、`components/physics` 以及维护中的 air command bridge。
...
旧的 `systems/physics/{aero_state,aerodynamics,control,propulsion}_system.h`
include 路径已移除。需要 air runtime system 的代码必须直接 include 本目录。
