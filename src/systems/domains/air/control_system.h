...
inline void register_control_system(flecs::world& ecs) {
    ecs.system<Velocity, Transform, const MissionCommandControlState, const FlightModel>("FlightControl")
        .kind(flecs::OnUpdate)
        .run([](flecs::iter& it) {
            while (it.next()) {
                ...
                for (auto i : it) {
                    ...
                    // We now just pass the entity. The model is responsible for fetching PilotAction/MissionCommand.
                    model_ref->model->update(it.world(), it.entity(i), v[i], p[i],
                                             fm[i], dt, env_ref ? env_ref->model : nullptr);
                }
            }
        });
}
