...
inline double ship_station_target_bearing_deg(double reference_x_m, double reference_y_m,
                                              double station_bearing_deg, double station_radius_m,
                                              double own_x_m, double own_y_m,
...
inline bool resolve_ship_station_command(flecs::world world, const Transform &own_transform,
    ...
    *commanded_heading_deg_out = ship_station_target_bearing_deg(
        reference_transform->x, reference_transform->y, stationing.station_bearing_deg,
        station_radius_m, own_transform.x, own_transform.y, fallback_heading_deg);

    const double reference_speed_mps = std::hypot(reference_velocity->vx, reference_velocity->vy);
                ...
                for (auto i : it) {
                    ...
                    if (pilot && ship_pilot_action_requests_manual_takeover(*pilot)) {
                        ...
                        commanded_heading_deg = Math::normalize_heading_deg(
                            current_heading_deg +
                            manual_turn * std::max(0.0, ship[i].max_turn_rate_deg_s));
                    ...
                    const double max_speed_mps =
                        ship[i].max_speed_mps > 0.0
                            ? ship[i].max_speed_mps
                            : std::max(commanded_speed_mps,
                                       std::hypot(velocity[i].vx, velocity[i].vy));
                    ...
                    commanded_speed_mps =
                        std::clamp(commanded_speed_mps, 0.0, effective_max_speed_mps);
                    ...
                    next_speed_mps = std::clamp(
                        next_speed_mps, 0.0, std::max(effective_max_speed_mps, current_speed_mps));
                    ...
                    if (command_active) {
                        ...
                        if (next_speed_mps > steerageway_speed_mps) {
                            ...
                            const double speed_ratio = std::clamp(
                                ...
                                std::clamp(ship[i].low_speed_turn_factor, 0.05, 1.0), 1.0);
                            ...
                            const double heading_step_deg = std::clamp(
                                heading_error_deg, -max_turn_step_deg, max_turn_step_deg);
                            next_heading_deg =
                                Math::normalize_heading_deg(next_heading_deg + heading_step_deg);
                        }
                    } else if (next_speed_mps > 0.05) {
                        next_heading_deg = Math::ground_track_deg_from_velocity(
                            velocity[i].vx, velocity[i].vy, next_heading_deg);
                    }
