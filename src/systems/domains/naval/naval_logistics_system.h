    ...
    ecs.system<ResupplyState, NavalStores, const Transform, const Velocity>("NavalUnderwayResupply")
        .run([&](flecs::iter &it) {
            while (it.next()) {
                ...
                auto transfer_axis = [](double *provider_current, double provider_rate_per_s,
                                        double *receiver_current, double receiver_max,
                                        double dt) -> double {
                ...
                for (auto i : it) {
                    ...
                    if (state[i].kind == ResupplyKind::NavalUnderway &&
                        ...
                        if (!provider.is_valid() || !provider_node || !provider_pos ||
                            !provider_vel || !provider_stores ||
                            !provider_node->underway_replenishment_enabled) {
                            ...
                            if (separation_m < provider_node->underway_min_separation_m ||
                                ...
                                if (state[i].naval_stage == NavalResupplyStage::Transferring) {
                                    ...
                                    const double transferred_fuel =
                                        transfer_axis(&provider_stores->fuel_units_current,
                                                      provider_node->transfer_rate_fuel_units_per_s,
                                                      &stores[i].fuel_units_current,
                                                      stores[i].fuel_units_max, dt);
                                    ...
                                    const double transferred_dry = transfer_axis(
                                        ...
                                        stores[i].dry_cargo_units_max, dt);
                                    state[i].time_remaining_s =
                                        std::max(0.0, state[i].time_remaining_s - dt);
                                    state[i].is_refueling = transferred_fuel > 0.0;
                                    state[i].is_rearming =
                                        (transferred_missiles > 0.0) || (transferred_dry > 0.0);

                                    const bool receiver_full =
                                        stores[i].fuel_units_current >=
                                            stores[i].fuel_units_max - 1.0e-6 &&
                                        stores[i].missile_units_current >=
                                            stores[i].missile_units_max - 1.0e-6 &&
                                        stores[i].dry_cargo_units_current >=
                                            stores[i].dry_cargo_units_max - 1.0e-6;
                                    ...
                                    const bool no_transfer_possible = transferred_fuel <= 0.0 &&
                                                                      transferred_missiles <= 0.0 &&
                                                                      transferred_dry <= 0.0;
