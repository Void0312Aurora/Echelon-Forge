# `src/systems/domains/naval` 边界

`systems/domains/naval` 保存舰艇、潜艇和舰载航空 token-level runtime 的每 tick 推进逻辑。这里消费 `components/domains/naval/platform`、`components/command` 与基础运动组件，但不拥有 mission/tasking 编排或 facade。
...
本目录可以消费 `components/domains/naval/platform`、`components/command`、`components/basic` 和必要的 `core/interfaces`。它不应依赖 `runtime/facade`、`interfaces/python` 或训练/场景 glue。
