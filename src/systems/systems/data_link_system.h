...
inline bool
data_link_pair_is_eligible(const flecs::entity &sender_entity, const DataLink &sender_link,
                           const Transform &sender_trans, double sender_effective_height_m,
                           const flecs::entity &receiver_entity, const DataLink &receiver_link,
                           const Transform &receiver_trans, double receiver_effective_height_m) {
...
inline double data_link_sender_local_support_window_s(const Sensor *sensor) {
    ...
    return std::min(track_recent_local_support_window_s(sensor->scan_period),
...
inline void register_data_link_system(flecs::world &ecs) {
    ...
    ecs.system<const DataLink, const Transform, const ContactList, const TrackDatabase,
               const Sensor>("DataLinkFusionSystem")
        .kind(flecs::OnUpdate)
        .run([](flecs::iter &it) {
            ...
            while (it.next()) {
                ...
                for (auto i : it) {
                    nodes.push_back({it.entity(i), &link[i], &trans[i], &contacts[i], &track_db[i],
                                     &sensor[i],
            ...
            for (const auto &sender : nodes) {
                ...
                if (sending_msg) {
                    for (auto &receiver : nodes) {
                        if (!data_link_pair_is_eligible(sender.entity, *sender.link, *sender.trans,
                                                        sender.effective_height_m, receiver.entity,
                                                        *receiver.link, *receiver.trans,
                                                        receiver.effective_height_m)) {
                            continue;
                        }

                        if (message_command.recipient != 0 &&
                            message_command.recipient != receiver.entity.id()) {
                            continue;
                        }
                        ...
                        q->inbox.push_back({
                            ...
                            static_cast<CommMsgType>(message_command.msg_type), message_command.arg,
                            0, // track_ref
                        ...
                        spdlog::trace("Msg delivered from {} to {}", sender.entity.id(),
                                      receiver.entity.id());
                    }
                }
                        ...
                        if (q) {
                            ...
                            for (const auto &trk : sender.track_db->tracks) {
                                ...
                                if (!track_has_recent_local_support(
                                        trk, current_time, sender_local_support_window_s)) {
                                    continue;
                                }
                                ...
                                if (receiver_db != nullptr) {
                                    for (const auto &known : receiver_db->tracks) {
                                        ...
                                        const double pos_delta_m = std::sqrt(
                                            pos_dx * pos_dx + pos_dy * pos_dy + pos_dz * pos_dz);
                                        const double vel_delta_mps = std::sqrt(
                                            vel_dx * vel_dx + vel_dy * vel_dy + vel_dz * vel_dz);
                                        const double since_last_dl =
                                            known.last_datalink_update_time < 0.0
                                                ? std::numeric_limits<double>::infinity()
                                                : (current_time - known.last_datalink_update_time);
                                        should_report = pos_delta_m > 500.0 ||
                                                        vel_delta_mps > 2.0 || since_last_dl >= 5.0;
                                        break;
                                    }
                                }
                                if (!should_report) {
                                ...
                                q->inbox.push_back(
                                    {sender.entity.id(), receiver.entity.id(),
                                     CommMsgType::ReportTrack, trk.entity_id, trk.track_id, trk.x,
                                     trk.y, trk.z, trk.vx, trk.vy, trk.vz, 0.0, trk.quality,
                                     static_cast<int>(trk.classification), current_time});
    ...
    ecs.system<CommQueue>("ClearCommInbox").kind(flecs::PreUpdate).run([](flecs::iter &it) {
        const ecs_world_info_t *info = ecs_get_world_info(it.world().c_ptr());
