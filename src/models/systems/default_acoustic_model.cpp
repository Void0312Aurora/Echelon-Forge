...
double target_source_level_db(const flecs::entity &target, double speed_mps, const Sonar &sonar) {
    ...
    if (const SubmarinePlatform *sub = target.get<SubmarinePlatform>()) {
        ...
        source_level += 16.0;
    } else if (const KeyEntity *key = target.get<KeyEntity>();
               key && key->type == UnitType::Aircraft) {
        source_level += 18.0;
    }
    return source_level;
}
...
IEnvironmentModel::MaritimeState maritime_state_for(const EnvironmentModelRef *env_ref,
...
double maritime_ambient_noise_bias_db(const EnvironmentModelRef *env_ref,
                                      const flecs::entity &owner, const Sonar &sonar) {
    ...
    const double wave_period_factor =
        std::clamp(8.0 / std::max(4.0, state.wave_period_s), 0.75, 1.35);
...
class DefaultAcousticModel : public IAcousticModel {
  public:
    void scan(flecs::world world, flecs::entity owner, const Transform &owner_transform,
              const Sonar &sonar, ContactList &out_contacts, double current_time) override {
        ...
        target_query.each([&](flecs::entity target, const KeyEntity &key,
            ...
            const double target_speed_mps =
                target_velocity ? std::sqrt(target_velocity->vx * target_velocity->vx +
