...
inline std::string make_capability_id(std::string_view type_name,
...
inline std::string make_evidence_ref(std::string_view type_name, std::string_view evidence_type) {
...
inline runtime::platform_capabilities::Capability
make_capability(std::string_view type_name, std::string_view family,
                std::string_view capability_type, std::vector<std::string> evidence_refs,
                bool required = true, bool supported = true, std::string unsupported_reason = {}) {
...
inline void append_capability(runtime::platform_capabilities::CapabilityBundle &bundle,
                              runtime::platform_capabilities::Capability capability) {
...
class DefaultUnitFactory : public IUnitFactory {
  public:
    ...
    using ResolvedPlatformSpawnPlan = runtime::platform_capabilities::ResolvedPlatformSpawnPlan;
    ...
    explicit DefaultUnitFactory(const std::string &config_path = std::string()) {
        ...
        missile.missile_tuning.max_accel_response_g_per_s =
        ...
        c2node.flight_model = {250.0, 100.0, 5.0,  5.0,  50.0,
                               2.0,   -1.0,  70.0, 60.0, 10.0}; // Slow, low G
    ...
    [[nodiscard]] CapabilityBundle
    build_platform_capability_bundle_template(std::string_view type_name,
        ...
        const auto add_sensing_capability = [&](std::string_view capability_type,
            ...
            append_capability(bundle, make_capability(type_name, kCapabilityFamilySensing,
                                                      capability_type, std::move(evidence_refs)));
        };
        const auto add_mobility_capability = [&](std::string_view capability_type,
                                                 std::vector<std::string> evidence_refs) {
            append_capability(bundle, make_capability(type_name, kCapabilityFamilyMobility,
                                                      capability_type, std::move(evidence_refs)));
        };
        const auto add_communication_capability =
            [](CapabilityBundle &bundle_ref, std::string_view type_name_ref,
               std::string_view capability_type, std::vector<std::string> evidence_refs) {
                append_capability(bundle_ref,
                                  make_capability(type_name_ref, kCapabilityFamilyCommunication,
                                                  capability_type, std::move(evidence_refs)));
            };
        const auto add_command_capability =
            [](CapabilityBundle &bundle_ref, std::string_view type_name_ref,
               std::string_view capability_type, std::vector<std::string> evidence_refs) {
                append_capability(bundle_ref,
                                  make_capability(type_name_ref, kCapabilityFamilyCommand,
                                                  capability_type, std::move(evidence_refs)));
            };
        const auto add_launching_capability = [&](std::string_view capability_type,
                                                  std::vector<std::string> evidence_refs) {
            append_capability(bundle, make_capability(type_name, kCapabilityFamilyLaunching,
                                                      capability_type, std::move(evidence_refs)));
        };
        const auto add_survivability_capability = [&](std::string_view capability_type,
                                                      std::vector<std::string> evidence_refs) {
            append_capability(bundle, make_capability(type_name, kCapabilityFamilySurvivability,
                                                      capability_type, std::move(evidence_refs)));
        };
        const auto add_doctrine_capability = [&](std::string_view capability_type,
                                                 std::vector<std::string> evidence_refs) {
            append_capability(bundle, make_capability(type_name, kCapabilityFamilyDoctrine,
                                                      capability_type, std::move(evidence_refs)));
        };

        if (!def.sensor_refs.empty()) {
            add_sensing_capability("sensor_refs", {
                                                      make_evidence_ref(type_name, "sensor_refs"),
                                                      make_evidence_ref(type_name, "sensor_ref"),
                                                  });
        }
        if (!def.sensor_ref.empty()) {
            add_sensing_capability("sensor_ref", {
                                                     make_evidence_ref(type_name, "sensor_ref"),
                                                 });
        }
        if (def.has_sensor) {
            add_sensing_capability("inline_sensor",
                                   {
        ...
        if (def.has_ship_platform) {
            add_mobility_capability("ship_platform_mobility",
                                    {
                                        make_evidence_ref(type_name, "ship_platform"),
                                        make_evidence_ref(type_name, "mobility"),
                                    });
            add_survivability_capability("ship_platform_survivability",
                                         {
                                             make_evidence_ref(type_name, "ship_platform"),
        ...
        if (def.has_submarine_platform) {
            add_mobility_capability("submarine_platform_mobility",
                                    {
                                        make_evidence_ref(type_name, "submarine_platform"),
                                        make_evidence_ref(type_name, "mobility"),
                                    });
            add_survivability_capability("submarine_platform_survivability",
                                         {
                                             make_evidence_ref(type_name, "submarine_platform"),
                                             make_evidence_ref(type_name, "survivability"),
                                         });
        ...
        if (def.has_embarked_air_ops) {
            add_doctrine_capability("embarked_air_ops",
                                    {
        ...
        if (def.has_ship_platform || def.has_submarine_platform || def.has_naval_weapon_system) {
            add_doctrine_capability("naval_platform_doctrine",
                                    {
        ...
        if (def.damage_model.hitboxes.empty() == false || def.health.current_hp > 0.0) {
            add_survivability_capability("health_and_damage_model",
                                         {
        ...
        if (def.has_aircraft_vulnerability) {
            add_survivability_capability(
                    ...
                    : "aircraft_vulnerability_synthetic_profile",
                {
                    ...
                    aircraft_vulnerability_has_calibrated_evidence(def.aircraft_vulnerability)
                        ? make_evidence_ref(type_name,
                                            "damage_model.vulnerability.calibrated_dataset")
                        : make_evidence_ref(type_name,
                                            "damage_model.vulnerability.synthetic_scaffold"),
                });
        }
    ...
    [[nodiscard]] PlatformCapabilityValidationResult
    validate_platform_capability_bundle_template(std::string_view type_name,
                                                 const UnitDefinition &def) const {
        return runtime::platform_capabilities::validate_capability_bundle(
            build_platform_capability_bundle_template(type_name, def));
    }

    [[nodiscard]] ResolvedPlatformSpawnPlan
    resolve_platform_spawn_plan(std::string_view type_name, const UnitDefinition &def) const {
        using namespace runtime::platform_capabilities;
        using namespace default_unit_factory_detail;
    ...
    [[nodiscard]] PlatformCapabilityValidationResult
    validate_resolved_platform_spawn_plan(std::string_view type_name,
                                          const UnitDefinition &def) const {
    ...
    [[nodiscard]] ResolvedPlatformSpawnPlan
    resolve_platform_spawn_plan_for_type_name(std::string_view type_name) const {
        using namespace runtime::platform_capabilities;
        using namespace default_unit_factory_detail;
    ...
    [[nodiscard]] PlatformCapabilityValidationResult
    validate_resolved_platform_spawn_plan_for_type_name(std::string_view type_name) const {
        return runtime::platform_capabilities::validate_resolved_platform_spawn_plan(
    ...
    flecs::entity spawn(flecs::world &ecs, const std::string &unit_name,
                        const SpawnParams &params) override {
        const auto resolved_spawn_plan = resolve_platform_spawn_plan_for_type_name(unit_name);
        ...
        if (!plan_validation.valid || !resolved_spawn_plan.admitted) {
            const std::string rejection_reason = !plan_validation.valid
                                                     ? plan_validation.rejection_reason
                                                     : resolved_spawn_plan.rejection_reason;
            spdlog::error("Spawn gate rejected type_name {} via resolved platform spawn plan: {}",
                          unit_name,
        ...
        if (it == definitions_.end()) {
            spdlog::error("Spawn gate admitted type_name {} but definition lookup failed during "
                          "materialization",
                          unit_name);
            return flecs::entity::null();
        }
        ...
        if (std::abs(heading_init) < 1e-6 && std::abs(pitch_init) < 1e-6 &&
        ...
        if (!def.sensor_refs.empty()) {
            for (const auto &sensor_ref_name : def.sensor_refs) {
                ...
                if (s_it == definitions_.end()) {
                    spdlog::warn("Unit {} references unknown sensor {}", unit_name,
                                 sensor_ref_name);
                    continue;
                }
        ...
        if (!def.default_loadout.empty()) {
            for (const auto &[station, weapon_name] : def.default_loadout) {
                ...
                if (w_it != definitions_.end()) {
                    ...
                    // tagging is enough. The WeaponSystem will look at these children later.
                } else {
        ...
        } else if (def.has_submarine_platform &&
                   def.submarine_platform.submerged_displacement_kg > 0.0) {
                ...
                if (propulsion->mil_thrust_n > 0.0) {
                    if (const EngineTuning *tuning = e.get<EngineTuning>()) {
                        if (tuning->tsfc_mil_kg_per_nh > 0.0) {
                            mil_flow_rate =
                                (propulsion->mil_thrust_n * tuning->tsfc_mil_kg_per_nh) / 3600.0;
                        }
                        if (tuning->tsfc_mil_kg_per_nh > 1.0e-9) {
                            ab_mult = std::max(1.0, tuning->tsfc_ab_kg_per_nh /
                                                        tuning->tsfc_mil_kg_per_nh);
                        }
                    }
                }
            }
            e.set<FuelSystem>({internal_fuel, // Current
                               internal_fuel, // Max
                               0.0, 0.0,      // External
                               0.0, false,    // State
        ...
        if (!def.rcs_profile_ref.empty()) {
            ...
            if (rcs_it != definitions_.end()) {
                ...
                spdlog::warn("Unit {} references unknown RCS profile {}", unit_name,
                             def.rcs_profile_ref);
            }
        } else {
            // Fallback
        ...
        double drag_coef =
            (def.airframe.drag_coefficient > 0) ? def.airframe.drag_coefficient : 0.02;
        ...
        if (def.has_ship_platform && def.ship_platform.beam_m > 0.0) {
            ...
            ref_area =
                std::max(1.0, def.submarine_platform.length_m * def.submarine_platform.draft_m);
        ...
        // The previous fixed inertia was far too small for fighter-sized masses, producing
        // unrealistically high yaw/roll accelerations on the ground (e.g., excessive weathervaning
        // in crosswind).
        ...
        if (def.type == UnitType::Aircraft) {
            ...
            // Box inertia scaled down to reflect centralized mass distribution (engines/fuel near
            // CG). Scale chosen to keep fighter Izz on the order of 1e5 kg*m^2.
        ...
        if (def.has_naval_stores) {
            e.set<NavalStores>(
                {def.naval_stores.fuel_units_current, def.naval_stores.fuel_units_max,
                 def.naval_stores.missile_units_current, def.naval_stores.missile_units_max,
                 def.naval_stores.dry_cargo_units_current, def.naval_stores.dry_cargo_units_max,
                 def.naval_stores.can_receive_underway, def.naval_stores.can_provide_underway});
            e.set<ResupplyState>(
                {0.0, false, false, ResupplyKind::BaseRefuel, 0, NavalResupplyStage::None});
        }
        if (def.has_command_link) {
        ...
        if (def.has_flight_model) {
            ...
            e.set<ActionSpaceConfig>({def.flight_model.max_turn_rate, def.flight_model.max_accel,
                                      ...
                                      def.flight_model.max_speed, 0.0, 20000.0});
        ...
        if (def.has_data_link) {
            ...
            // Blue=1, Red=2
            int net_id = (params.side == Side::Blue) ? 1 : (params.side == Side::Red) ? 2 : 0;
            e.set<DataLink>({true, net_id, LinkType::Link16,
        ...
        if (def.has_embarked_air_ops && def.embarked_air_ops.enabled &&
            ...
            if (helo_it != definitions_.end()) {
                ...
                helo_params.x +=
                    std::sin(heading_rad) * def.embarked_air_ops.launch_offset_forward_m +
                    std::sin(right_rad) * def.embarked_air_ops.launch_offset_starboard_m;
                helo_params.y +=
                    std::cos(heading_rad) * def.embarked_air_ops.launch_offset_forward_m +
                ...
                if (helo.is_valid()) {
                    ...
                    spdlog::warn("Unit {} failed to pre-spawn embarked helo {}", unit_name,
                                 def.embarked_air_ops.helo_unit_name);
                }
            } else {
                spdlog::warn("Unit {} references unknown embarked helo {}", unit_name,
                             def.embarked_air_ops.helo_unit_name);
            }
        }
        ...
        if (def.type == UnitType::Facility || def.name.find("Airbase") != std::string::npos) {
            ...
            e.set<LogisticsNode>({0.0, false, def.naval_logistics.underway_replenishment_enabled,
  ...
  private:
    ...
    HitboxConfig generate_default_hitboxes(const Airframe &af) {
        ...
        if (af.configuration == "Flanker") {
            ...
            config.hitboxes.push_back(
                {0, L * 0.4, 0, 0, L * 0.2, 0.8, 0.8, 5.0, {"radar", "cockpit"}});
            ...
            config.hitboxes.push_back(
                {2, -L * 0.35, -1.0, -0.5, L * 0.25, 0.8, 0.8, 15.0, {"engine_left"}});

            // 4. Right Engine Nacelle
            config.hitboxes.push_back(
                {3, -L * 0.35, 1.0, -0.5, L * 0.25, 0.8, 0.8, 15.0, {"engine_right"}});

            // 5. Wings (Fuel, Flight Control)
            config.hitboxes.push_back(
                {4, -L * 0.1, 0, 0, L * 0.2, W, 0.2, 3.0, {"wings", "flight_control"}});

        } else {
            // "Conventional" (Default F-16 style)
            // 1. Nose
            config.hitboxes.push_back(
                {0, L * 0.4, 0, 0, L * 0.25, 0.8, 0.8, 5.0, {"radar", "cockpit"}});
            ...
            config.hitboxes.push_back(
                {1,
                 ...
                 1.0,
                 10.0,
                 {"fuel", "ammo", "engine"}}); // Single engine usually embedded
            ...
            // 4. Wings
            config.hitboxes.push_back(
                {3, -L * 0.05, 0, 0, L * 0.2, W, 0.2, 3.0, {"wings", "flight_control"}});
        }
