...
void apply_default_effects_spatial_projection(
    ...
    if (structured_air_target && (!scratch.structure_hit || broad_spatial_projection) &&
        ...
        const auto component_has_direct_hit_row = [&](const DamageComponent &component) {
            const std::string component_name = damage_component_key(component);
            const std::string redundancy_group_id =
                damage_component_redundancy_group_key(component);
            for (const ComponentMechanismLoadRow &row : scratch.component_mechanism_load_rows) {
                if (row.direct_hit && row.component_name == component_name &&
                    row.component_system == component.system &&
                    row.component_redundancy_group_id == redundancy_group_id) {
                    return true;
                }
            }
            return false;
        };
        const auto make_component_projection_candidate =
            [&](const Hitbox &parent_box, const DamageComponent &component,
                SpatialProjectionCandidate *out_candidate) {
                if (component_has_direct_hit_row(component)) {
                    return false;
                }
                ...
                const Hitbox component_box = component_as_hitbox(component, parent_box);
        ...
        const auto collect_component_projection_candidates = [&](const Hitbox &parent_box) {
            ...
            for (const auto &component : parent_box.components) {
                ...
                if (!make_component_projection_candidate(parent_box, component, &candidate)) {
                    continue;
                }
                component_candidates.push_back(candidate);
            }
            std::sort(
                component_candidates.begin(), component_candidates.end(),
                [&](const SpatialProjectionCandidate &lhs, const SpatialProjectionCandidate &rhs) {
                    const double lhs_score =
                        component_projection_priority_score(missile.warhead_profile, lhs);
                    const double rhs_score =
                        component_projection_priority_score(missile.warhead_profile, rhs);
        ...
        const auto select_closest_component_projection_candidate =
            [&](const Hitbox &parent_box, SpatialProjectionCandidate *out_candidate) {
                bool found = false;
                SpatialProjectionCandidate best_candidate{};
                for (const auto &component : parent_box.components) {
                    SpatialProjectionCandidate candidate{};
                    if (!make_component_projection_candidate(parent_box, component, &candidate)) {
                        continue;
                    }
                    if (!found || candidate.distance_m < best_candidate.distance_m ||
                        (candidate.distance_m == best_candidate.distance_m &&
                         candidate.effect_scale > best_candidate.effect_scale)) {
                        best_candidate = candidate;
                        found = true;
                    }
                }
                if (found && out_candidate) {
        ...
        for (const auto &box : hitboxes.hitboxes) {
            if (broad_spatial_projection) {
                SpatialProjectionCandidate candidate{};
                if (make_box_projection_candidate(box, &candidate)) {
            ...
            if (!broad_spatial_projection && !box.components.empty()) {
                for (const auto &component : box.components) {
                    ...
                    if (make_component_projection_candidate(box, component, &component_candidate)) {
                        candidates.push_back(component_candidate);
                    }
                }
        ...
        for (std::size_t candidate_index = 0; candidate_index < projected_count;
             ++candidate_index) {
            ...
            record_default_effects_warhead_effect_sample(
                ...
                make_default_effects_spatial_sample(candidate), candidate.mechanism_load);
            ++scratch.projected_hitbox_count;
            scratch.structure_hit = true;
            spdlog::info("PROXIMITY FIELD >>> Box {}{} NEAR HIT at {:.2f} m, quality {:.2f}, armor "
                         "{:.2f}, exposure {:.2f}",
                         projected_box->id, projected_component ? " component" : "",
                         candidate.distance_m, candidate.effect_scale, candidate.armor_scale,
                         candidate.exposure_scale);

            const double projected_system_severity = std::clamp(
                resolve_system_severity(candidate.effect_scale, false, candidate.mechanism_load) *
                    candidate.effect_scale,
                0.02, 0.95);
            if (!projected_component && broad_spatial_projection &&
                !projected_box->components.empty()) {
                std::vector<SpatialProjectionCandidate> component_candidates;
                if (candidate_index == 0) {
                    component_candidates = collect_component_projection_candidates(*projected_box);
                    if (component_candidates.size() > 2U) {
                        component_candidates.resize(2U);
                    }
                } else {
                    SpatialProjectionCandidate closest_candidate{};
                    if (select_closest_component_projection_candidate(*projected_box,
                                                                      &closest_candidate)) {
                ...
                for (const SpatialProjectionCandidate &component_candidate : component_candidates) {
                    record_default_effects_warhead_effect_sample(
                        ...
                        component_candidate.armor_scale, component_candidate.exposure_scale,
                        make_default_effects_spatial_sample(component_candidate),
                        component_candidate.mechanism_load);

                    const double component_projected_system_severity =
                        std::clamp(resolve_system_severity(component_candidate.effect_scale, false,
                                                           component_candidate.mechanism_load) *
                                       component_candidate.effect_scale,
                                   0.02, 0.95);
                    const double component_scale = resolve_default_effects_component_scale(
                        missile.warhead_profile, *component_candidate.component);
                    scratch.sampled_component_threshold_scale =
                        std::max(scratch.sampled_component_threshold_scale, component_scale);
                    ComponentMechanismLoadRow *component_row = record_default_effects_component_hit(
                        scratch, *component_candidate.component, component_candidate.effect_scale,
                        component_scale, false, component_candidate.distance_m,
                        component_candidate.mechanism_load);
                    scratch.processed_air_systems.insert(component_candidate.component->system);
                    apply_system_effect(
                        component_candidate.component->system, component_projected_system_severity,
                        component_candidate.effect_scale, component_scale, false,
                        component_candidate.mechanism_load, component_candidate.component->critical,
                        component_candidate.component->redundancy_group,
                        component_candidate.component, component_row);
                    spdlog::info("   - projected component {}:{} Status: {:.2f}",
                                 component_candidate.component->name.empty()
                                     ? component_candidate.component->system
                                     : component_candidate.component->name,
                                 component_candidate.component->system,
                                 sys_health->systems[component_candidate.component->system]);
                }
            }
            if (projected_component) {
                const double component_scale = resolve_default_effects_component_scale(
                    missile.warhead_profile, *projected_component);
                scratch.sampled_component_threshold_scale =
                    std::max(scratch.sampled_component_threshold_scale, component_scale);
                ComponentMechanismLoadRow *component_row = record_default_effects_component_hit(
                    scratch, *projected_component, candidate.effect_scale, component_scale, false,
                    candidate.distance_m, candidate.mechanism_load);
                apply_system_effect(
                    projected_component->system, projected_system_severity, candidate.effect_scale,
                    component_scale, false, candidate.mechanism_load, projected_component->critical,
                    projected_component->redundancy_group, projected_component, component_row);
                spdlog::info("   - component {}:{} Status: {:.2f}",
                             projected_component->name.empty() ? projected_component->system
                ...
                for (const auto &system : projected_box->protected_systems) {
                    if (!scratch.processed_air_systems.insert(system).second) {
                        continue;
                    }
                    ...
                    scratch.sampled_component_threshold_scale =
                        std::max(scratch.sampled_component_threshold_scale, component_scale);
                    apply_system_effect(system, projected_system_severity, candidate.effect_scale,
                                        component_scale, false, candidate.mechanism_load);
