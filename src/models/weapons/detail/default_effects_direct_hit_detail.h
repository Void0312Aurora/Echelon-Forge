        ...
        if (check_hitbox(local_imp, box)) {
            ...
            if (structured_air_target && !box.components.empty()) {
                for (const auto &component : box.components) {
                    ...
                    const double armor_scale =
                        warhead_mechanism_armor_scale(missile, component_box, 0.0, 1.0, 1.0, true);
                    ...
                    const WarheadMechanismLoadEvidence mechanism_load = with_surface_incidence(
                        estimate_warhead_mechanism_load(missile, component_box, 0.0, 1.0, 1.0,
                                                        orientation_weight, exposure_scale, true,
                                                        closure_mps, spatial_sample),
                        hitbox_surface_incidence_cos(local_imp, component_box, missile_axis_body));
                    ...
                    record_default_effects_warhead_effect_sample(
                        scratch, direct_mechanism_scale, direct_mechanism_scale, armor_scale,
                        exposure_scale, spatial_sample, mechanism_load);

                    const double component_scale =
                        resolve_default_effects_component_scale(missile.warhead_profile, component);
                    ...
                    ComponentMechanismLoadRow *component_row = record_default_effects_component_hit(
                        scratch, component, direct_mechanism_scale, component_scale, true, 0.0,
                        mechanism_load);
                    const double direct_system_severity =
                        resolve_system_severity(direct_mechanism_scale, true, mechanism_load);
                    apply_system_effect(component.system, direct_system_severity,
                                        direct_mechanism_scale, component_scale, true,
                                        mechanism_load, component.critical,
                                        component.redundancy_group, &component, component_row);
                    spdlog::info("   - component {}:{} Status: {:.2f} component_scale={:.2f}",
                                 component.name.empty() ? component.system : component.name,
                                 component.system, sys_health->systems[component.system],
                                 component_scale);
                }
            }
            ...
            if (!structured_air_target || box.components.empty() || !component_direct_hit) {
                const double armor_scale =
                    structured_air_target
                        ? warhead_mechanism_armor_scale(missile, box, 0.0, 1.0, 1.0, true)
                        : 1.0;
                const double exposure_scale =
                    structured_air_target ? hitbox_projected_exposure_scale(local_imp, box) : 1.0;
                ...
                if (structured_air_target) {
                    ...
                    mechanism_load = with_surface_incidence(
                        estimate_warhead_mechanism_load(missile, box, 0.0, 1.0, 1.0,
                                                        orientation_weight, exposure_scale, true,
                                                        closure_mps, spatial_sample),
                        hitbox_surface_incidence_cos(local_imp, box, missile_axis_body));
                    const double direct_mechanism_scale =
                        std::clamp(armor_scale * exposure_scale, 0.05, 1.10);
                    record_default_effects_warhead_effect_sample(
                        scratch, direct_mechanism_scale, direct_mechanism_scale, armor_scale,
                ...
                for (const auto &system : box.protected_systems) {
                    ...
                    const double direct_system_severity =
                        resolve_system_severity(direct_mechanism_scale, true, mechanism_load);
                    apply_system_effect(system, direct_system_severity, direct_mechanism_scale,
                                        component_scale, true, mechanism_load);
                    spdlog::info("   - {} Status: {:.2f} component_scale={:.2f}", system,
                                 sys_health->systems[system], component_scale);
                }
            }
        }
    }
