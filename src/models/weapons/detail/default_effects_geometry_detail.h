...
Vec3 world_to_body(const Transform &t, double wx, double wy, double wz) {
    return math_body_to_local_right_frame(Math::world_to_body({wx - t.x, wy - t.y, wz - t.z}, t));
}
...
bool check_hitbox(const Vec3 &local_p, const Hitbox &box) {
    ...
    return (local_p.x >= min_x && local_p.x <= max_x && local_p.y >= min_y && local_p.y <= max_y &&
...
double hitbox_surface_incidence_cos(const Vec3 &local_p, const Hitbox &box,
    ...
    if (!vec3_is_finite(local_p) || !vec3_is_finite(axis) || vec3_norm(axis) <= 1.0e-9 ||
        ...
        !std::isfinite(box.offset_z) || !std::isfinite(box.dim_l) || !std::isfinite(box.dim_w) ||
        !std::isfinite(box.dim_h) || box.dim_l <= 0.0 || box.dim_w <= 0.0 || box.dim_h <= 0.0) {
        return 0.0;
    }
    ...
    const auto consider_face = [&](const Vec3 &normal) {
        const double candidate = std::clamp(std::abs(vec3_dot(axis, normal)), 0.0, 1.0);
    ...
    const double distances[] = {
        std::abs(local_p.x - min_x), std::abs(max_x - local_p.x), std::abs(local_p.y - min_y),
        std::abs(max_y - local_p.y), std::abs(local_p.z - min_z), std::abs(max_z - local_p.z),
    };
...
Vec3 component_geometry_half_extents(const DamageComponent &component) {
    return {
        component.geometry_half_extents_m[0] > 0.0 ? component.geometry_half_extents_m[0]
                                                   : component.dim_l * 0.5,
        component.geometry_half_extents_m[1] > 0.0 ? component.geometry_half_extents_m[1]
                                                   : component.dim_w * 0.5,
        component.geometry_half_extents_m[2] > 0.0 ? component.geometry_half_extents_m[2]
                                                   : component.dim_h * 0.5,
    };
}

bool component_uses_obb_like_geometry(const DamageComponent &component) {
    return component.geometry_primitive == "obb" || component.geometry_primitive == "thin_prism";
}

Vec3 component_obb_local_coordinates(const Vec3 &local_p, const DamageComponent &component) {
    const Vec3 delta{
...
Vec3 component_obb_world_coordinates(const Vec3 &local_p, const DamageComponent &component) {
    ...
    return {
        component.offset_x + axis_x.x * local_p.x + axis_y.x * local_p.y + axis_z.x * local_p.z,
        component.offset_y + axis_x.y * local_p.x + axis_y.y * local_p.y + axis_z.y * local_p.z,
        component.offset_z + axis_x.z * local_p.x + axis_y.z * local_p.y + axis_z.z * local_p.z,
    };
}

ComponentAxisBounds component_axis_aligned_bounds(const DamageComponent &component) {
    if (component.geometry_primitive == "convex_hull" && !component.geometry_vertices_m.empty()) {
        Vec3 minimum{
    ...
    if (component_uses_obb_like_geometry(component)) {
        ...
        const Vec3 axis_z = component_geometry_axis(component, 2);
        const Vec3 span{
            2.0 * (std::abs(axis_x.x) * half.x + std::abs(axis_y.x) * half.y +
                   std::abs(axis_z.x) * half.z),
            2.0 * (std::abs(axis_x.y) * half.x + std::abs(axis_y.y) * half.y +
                   std::abs(axis_z.y) * half.z),
            2.0 * (std::abs(axis_x.z) * half.x + std::abs(axis_y.z) * half.y +
                   std::abs(axis_z.z) * half.z),
        };
        return {
            {component.offset_x - span.x * 0.5, component.offset_y - span.y * 0.5,
    ...
    return {
        {component.offset_x - component.dim_l * 0.5, component.offset_y - component.dim_w * 0.5,
...
bool system_is_air_control_surface(const std::string &system) {
    return system_name_matches(system, "flight_control") ||
           system_name_matches(system, "control") || system_name_matches(system, "hydraulic");
}

bool system_is_air_sensor(const std::string &system) {
    return system_name_matches(system, "radar") || system_name_matches(system, "sensor") ||
           system_name_matches(system, "rwr") || system_name_matches(system, "esm");
}

bool system_is_air_propulsion_or_fuel(const std::string &system) {
    return system_name_matches(system, "engineering") || system_name_matches(system, "engine") ||
           system_name_matches(system, "fuel") || system_name_matches(system, "propeller") ||
           system_name_matches(system, "transmission");
}

bool system_is_air_propulsion(const std::string &system) {
    return system_name_matches(system, "engineering") || system_name_matches(system, "engine") ||
           system_name_matches(system, "propeller") || system_name_matches(system, "transmission");
}
...
bool system_is_air_fire_suppression(const std::string &system) {
    return system_name_matches(system, "fire_suppression") ||
           ...
           system_name_matches(system, "suppression") || system_name_matches(system, "extinguish");
}

bool system_is_crew_or_cockpit(const std::string &system) {
    return system_name_matches(system, "cockpit") || system_name_matches(system, "pilot") ||
           system_name_matches(system, "crew");
}

bool system_is_command_navigation(const std::string &system) {
    return system_name_matches(system, "command") || system_name_matches(system, "navigation");
}

bool system_is_mission_crew_station(const std::string &system) {
    return system_name_matches(system, "operator") || system_name_matches(system, "crew");
}
...
CrewConsequenceKind classify_crew_consequence(const std::string &system,
                                              const std::string &component_name) {
    const std::string &name = component_name.empty() ? system : component_name;
    if (system_name_matches(system, "cockpit") || system_name_matches(system, "pilot") ||
        system_name_matches(name, "flight_deck") || system_name_matches(name, "cockpit") ||
        system_name_matches(name, "pilot")) {
        return CrewConsequenceKind::Pilot;
    }
    if (system_name_matches(system, "command") || system_name_matches(system, "navigation") ||
        system_name_matches(name, "command") || system_name_matches(name, "navigation")) {
        return CrewConsequenceKind::CommandNavigation;
    }
    if (system_name_matches(name, "mission_operator") || system_name_matches(name, "operator") ||
...
void apply_aircraft_crew_consequence(AircraftDamageState &aircraft_damage, CrewConsequenceKind kind,
                                     double delta) {
...
bool system_is_mission_or_combat(const std::string &system) {
    return system_name_matches(system, "combat") || system_name_matches(system, "command") ||
           ...
           system_name_matches(system, "avionics") || system_name_matches(system, "navigation") ||
           system_name_matches(system, "mission");
}

bool system_is_air_structure(const std::string &system) {
    return system_name_matches(system, "wing") || system_name_matches(system, "airframe") ||
           system_name_matches(system, "fuselage") || system_name_matches(system, "structure") ||
           system_name_matches(system, "rotor") || system_name_matches(system, "tail");
}
...
bool component_is_engine_fuel_feed_path(const DamageComponent &component) {
    ...
    return system_name_matches(name, "fuel_feed") || system_name_matches(name, "fuel_control") ||
...
bool component_is_lateral_fuel_storage_path(const DamageComponent &component) {
    if (!system_is_air_fuel(component.system) || component_is_engine_fuel_feed_path(component) ||
        component_is_fire_suppression_path(component)) {
        return false;
    }
    const std::string name = damage_component_key(component);
    return system_name_matches(name, "left") || system_name_matches(name, "right");
}
...
bool component_is_hydraulic_supply_path(const DamageComponent &component) {
    ...
    return system_name_matches(component.system, "hydraulic") ||
           (system_name_matches(name, "hydraulic") &&
            (system_name_matches(name, "pump") || system_name_matches(name, "reservoir") ||
             system_name_matches(name, "supply") || system_name_matches(name, "line") ||
             system_name_matches(name, "module"))) ||
           (system_name_matches(group, "hydraulic") &&
            (system_name_matches(group, "supply") || system_name_matches(group, "pump")));
}
...
bool component_depends_on_hydraulic_power(const DamageComponent &component) {
    for (const auto &dependency : component.dependencies) {
        const bool hydraulic_target = system_name_matches(dependency.target_system, "hydraulic") ||
                                      system_name_matches(dependency.system, "hydraulic");
        const bool hydraulic_edge =
            dependency.edge_type == "hydraulic_power" || dependency.edge_type == "hydraulic-power";
...
bool component_is_hydraulic_consumer_path(const DamageComponent &component) {
    ...
    return component_depends_on_hydraulic_power(component) ||
           ((system_is_air_control_surface(component.system) ||
            ...
            (system_name_matches(name, "actuator") || system_name_matches(name, "servo") ||
             system_name_matches(name, "cyclic") || system_name_matches(name, "collective")));
}

enum class AircraftFireZone { None, EngineBay, Wing, Fuselage, MissionBay };

AircraftFireZone classify_aircraft_fire_zone(const std::string &system,
    ...
    if (system_name_matches(key, "engine") || system_name_matches(key, "propulsion") ||
        system_name_matches(key, "afterburner") || system_name_matches(key, "nozzle") ||
        system_name_matches(key, "fuel_feed") || system_name_matches(key, "fuel_control") ||
        system_name_matches(key, "transmission")) {
        return AircraftFireZone::EngineBay;
    }
    if (system_name_matches(key, "wing") || system_name_matches(key, "aileron") ||
        system_name_matches(key, "elevon") || system_name_matches(key, "flap") ||
        system_name_matches(key, "spar")) {
        return AircraftFireZone::Wing;
    }
    if (system_name_matches(key, "radar") || system_name_matches(key, "sensor") ||
        system_name_matches(key, "avionics") || system_name_matches(key, "data_link") ||
        system_name_matches(key, "mission") || system_name_matches(key, "command") ||
        system_name_matches(key, "navigation") || system_name_matches(key, "operator") ||
        system_name_matches(key, "power") || system_name_matches(key, "electrical")) {
        return AircraftFireZone::MissionBay;
    }
    if (system_name_matches(key, "fuselage") || system_name_matches(key, "fuel") ||
        system_name_matches(key, "cockpit") || system_name_matches(key, "crew") ||
...
double component_mechanism_threshold_scale(const WarheadProfile &profile,
...
double component_authored_mechanism_threshold_scale(const WarheadProfile &profile,
                                                    const DamageComponent &component) {
    const std::string family = warhead_effect_family(profile);
    ...
    if ((family == "fragmentation" || family == "blast_fragmentation") &&
        ...
        return std::clamp(component.mechanism_threshold_scales.at("fragmentation"), 0.35, 2.40);
    }
    if (family == "blast_fragmentation" && component.mechanism_threshold_scales.find("blast") !=
                                               component.mechanism_threshold_scales.end()) {
        return std::clamp(component.mechanism_threshold_scales.at("blast"), 0.35, 2.40);
    }
    return 1.0;
}
