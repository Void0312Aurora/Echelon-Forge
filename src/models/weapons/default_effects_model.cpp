...
public:
    EffectsResult on_proximity_hit(flecs::world world,
        ...
        if (hitboxes && sys_health && t_tgt && t_msl) {
            ...
            DefaultEffectsSystemSeverityResolver resolve_system_severity{
                system_severity_context};
            ...
            DefaultEffectsSystemEffectApplicator apply_system_effect{
                system_effect_context};
            const auto populate_result = [&]() {
                populate_default_effects_result(result, scratch, warhead_orientation_axis_body);
            };
            apply_default_effects_direct_hitboxes(
                scratch,
                *hitboxes,
