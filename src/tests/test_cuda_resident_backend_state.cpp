...
TEST_CASE("RB4 fixed-air setup input barriers and export reconstruct exact device state") {
    ...
    CHECK(snapshot.barrier.materialized_shards == std::vector<std::string>{"identity", "clock",
                                                                           "snapshot", "kinematics",
                                                                           "export_envelope"});
    ...
    CHECK(snapshot.envelope.schema_version == std::string(kCudaResidentSnapshotSchemaV1));
    CHECK(snapshot.envelope.visibility_label == "export");
    CHECK(snapshot.envelope.provenance == std::string(kCudaResidentSnapshotProvenance));
    ...
    CHECK(semantic_export.kinematics[0].state.x == doctest::Approx(fixture.spawns[1].x));
...
TEST_CASE("RB4 fixed-air boundary rejects undeclared setup input advance and export features") {
    ...
    CHECK_THROWS_AS(backend.advance({.kind = runtime::backend::AdvanceKind::StepExecutionResults}),
                    std::logic_error);
    CHECK_THROWS_AS(backend.export_state({.include_agent_observations = true}), std::logic_error);
