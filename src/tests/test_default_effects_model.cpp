...
std::pair<flecs::entity, flecs::entity> spawn_structured_f16_pair(SimulationKernel &kernel) {
    auto attacker = kernel.spawn_unit(Side::Blue, "F-16C_Block50", 0.0, 0.0, 5000.0, 0.0, 0.0, 0.0,
                                      0.0, 250.0, 0.0);
    auto target = kernel.spawn_unit(Side::Red, "F-16C_Block50", 0.0, 500.0, 5000.0, 180.0, 0.0, 0.0,
                                    0.0, -250.0, 0.0);
...
RecentEngagementEvents run_profiled_hit(const std::string &family) {
    ...
    CHECK(kernel.debug_apply_profiled_local_proximity_hit_with_velocity(
        static_cast<std::uint64_t>(attacker.id()), static_cast<std::uint64_t>(target.id()), -0.8,
        4.1, -0.985, profile, 900.0, -250.0, 0.0));
...
TEST_SUITE("default_effects_model") {
    ...
    TEST_CASE("profiled proximity hit fails closed for invalid entities") {
        ...
        CHECK_FALSE(kernel.debug_apply_profiled_local_proximity_hit(
            0, static_cast<std::uint64_t>(target.id()), -0.8, 4.1, -0.985, profile));
        CHECK_FALSE(kernel.debug_apply_profiled_local_proximity_hit(
            static_cast<std::uint64_t>(attacker.id()), 0, -0.8, 4.1, -0.985, profile));
    ...
    TEST_CASE("continuous rod profile emits rod mechanism facts through default effects") {
        ...
        REQUIRE(events.component_load_events.size() ==
        ...
        for (std::size_t index = 0; index < events.component_load_events.size(); ++index) {
            ...
            const bool known_load_source = load.load_source == "direct_component_hit" ||
            ...
            saw_primary_load =
                saw_primary_load || (load.component_name == effects.component_primary_name &&
