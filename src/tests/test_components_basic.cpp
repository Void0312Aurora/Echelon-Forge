...
TEST_CASE("transform_body_to_world_identity") {
    Transform t{};
    t.x = 100.0; t.y = 200.0; t.z = 300.0;
    t.heading = 0.0; t.pitch = 0.0; t.roll = 0.0;

    // Body-frame offset at origin → world = body + transform position.
    auto w = Math::body_to_world(Math::Vector3{10.0, 5.0, -2.0},
                                  t.heading, t.pitch, t.roll);
...
TEST_CASE("transform_body_to_world_heading_90") {
    ...
    auto w = Math::body_to_world(Math::Vector3{10.0, 0.0, 0.0},
                                  90.0, 0.0, 0.0);
