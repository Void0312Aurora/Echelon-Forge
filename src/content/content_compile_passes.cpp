...
const std::unordered_set<std::string>& recognized_top_level_keys() {
    static const std::unordered_set<std::string> keys = {
        ...
        "type", "name", "mass_kg", "engine_ref", "engine", "mil_thrust_n",
        "ab_thrust_n", "sfc_mil", "sfc_ab", "bypass_ratio", "engine_tuning",
        "hardpoints", "default_loadout", "health", "sensor_ref", "sensor_refs",
        "sensor", "has_sensor", "mounted_sensors", "sonar", "mounted_sonars",
        "has_flight_model", "flight_model", "has_landing_gear", "landing_gear",
        "has_score", "score", "airframe", "aero_tuning", "stall_state",
        "ship_platform", "submarine_platform", "naval_stores", "naval_logistics",
        "naval_weapon_system", "embarked_air_ops", "damage_model", "has_ammo",
        "ammo", "missile_tuning", "guidance", "warhead", "fuze", "fuse",
        "has_command_link", "command_link", "has_data_link", "data_link_network_id",
        "data_link_max_reports_per_update", "data_link_max_messages_per_update",
        "rwr", "jammer", "countermeasures", "esm",
        // --- 52 missile-tuning helper keys (parse_missile_tuning_json_fields) ---
        "max_speed", "turn_rate", "fuse_distance", "damage", "seeker_fov_deg",
        "seeker_lock_range", "guidance_delay_s", "guidance_update_period_s",
        "max_flight_time_s", "nav_gain", "apn_target_accel_gain", "sensor_max_range",
        "sensor_fov_deg", "sensor_scan_period", "sensor_detection_prob",
        "sensor_bearing_noise_std", "sensor_range_noise_std", "sensor_track_memory_s",
        "seeker_type", "seeker_activation_range_m", "seeker_gimbal_limit_deg",
        "seeker_ifov_deg", "bearing_filter_tau_s", "elevation_filter_tau_s",
        "range_filter_tau_s", "track_break_time_s", "boost_time_s", "sustain_time_s",
        "boost_thrust_n", "sustain_thrust_n", "reference_area_m2", "cd0_subsonic",
        "cd0_supersonic", "induced_drag_k", "cd0_mach_breakpoints", "cd0_mach_values",
        "induced_drag_k_mach_breakpoints", "induced_drag_k_mach_values",
        "propellant_mass_kg", "max_lateral_g", "autopilot_tau_s", "autopilot_damping",
        "autopilot_order", "max_accel_response_g_per_s", "mach_transonic_start",
        "mach_transonic_end", "cd0_power_on_ratio", "min_launch_range_m",
        "max_launch_off_boresight_deg", "lobl_required", "midcourse_datalink_supported",
        "use_kalman_seeker",
        // --- 3 semantic present-but-unread top-level keys (survey section 3) ---
        "ew_suite_ref", "rcs", "rcs_profile_ref",
    };
    return keys;
}
...
std::vector<ContentDiagnostic> validate_pass(const std::vector<nlohmann::json>& entries,
    ...
    for (const auto& entry : entries) {
        ...
        diagnostics.insert(diagnostics.end(),
                           std::make_move_iterator(entry_diagnostics.begin()),
