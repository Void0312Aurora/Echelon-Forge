...
constexpr const char *kVulnerabilityEvidenceSchemaVersion = "a2.vulnerability_evidence.v1";
...
std::array<double, 3> parse_vec3_array(const nlohmann::json &value,
...
void parse_damage_component_geometry_json_fields(const nlohmann::json &component_json,
    ...
    if (geometry_json) {
        ...
        if (geometry_json->contains("vertices_m")) {
            component.geometry_vertices_m = parse_vec3_array_rows((*geometry_json)["vertices_m"]);
        }
        if (geometry_json->contains("thin_prism") && (*geometry_json)["thin_prism"].is_object()) {
            ...
            component.geometry_nominal_thickness_m =
                thin_json.value("nominal_thickness_m", component.geometry_nominal_thickness_m);
        }
        ...
        if (obb_json->contains("half_extents_m")) {
            component.geometry_half_extents_m =
                parse_vec3_array((*obb_json)["half_extents_m"], component.geometry_half_extents_m);
        }
    }
    ...
    if (component_json.contains("geometry_half_extents_m")) {
        component.geometry_half_extents_m = parse_vec3_array(
            component_json["geometry_half_extents_m"], component.geometry_half_extents_m);
    }
}

void parse_optional_evidence_row_number(const nlohmann::json &row_json, const char *key,
...
void parse_sensor_json_fields(const nlohmann::json &s, Sensor *out_sensor,
    ...
    sensor.sea_clutter_sensitivity =
        s.value("sea_clutter_sensitivity", sensor.sea_clutter_sensitivity);
    sensor.sea_state_loss_per_level =
        s.value("sea_state_loss_per_level", sensor.sea_state_loss_per_level);
...
void parse_missile_tuning_json_fields(const nlohmann::json &src,
...
void parse_fuze_json_fields(const nlohmann::json &src, MissileTuningDefinition *out_tuning) {
    if (!out_tuning || !src.is_object()) return;
    ...
    profile.trigger_logic =
        src.value("trigger_logic", src.value("trigger_mode", profile.trigger_logic));
...
void parse_warhead_json_fields(const nlohmann::json &src, MissileTuningDefinition *out_tuning) {
    ...
    profile.synthetic = src.value("synthetic", false);
    profile.provenance = src.value("provenance", profile.damage_scalar_synthetic
    ...
    if (!out_tuning->has_fuze_profile && std::isfinite(profile.lethal_radius_m)) {
        out_tuning->fuze_profile = make_synthetic_fuze_profile(profile.lethal_radius_m,
...
void parse_sonar_json_fields(const nlohmann::json &s, Sonar *out_sonar) {
    ...
    sonar.self_noise_per_speed_db =
        s.value("self_noise_per_speed_db", sonar.self_noise_per_speed_db);
    sonar.ambient_noise_db = s.value("ambient_noise_db", sonar.ambient_noise_db);
    sonar.source_level_reference_db =
        s.value("source_level_reference_db", sonar.source_level_reference_db);
    sonar.source_level_speed_factor_db =
        s.value("source_level_speed_factor_db", sonar.source_level_speed_factor_db);
    ...
    sonar.convergence_zone_bonus_m =
        s.value("convergence_zone_bonus_m", sonar.convergence_zone_bonus_m);
    sonar.baffle_exclusion_deg = s.value("baffle_exclusion_deg", sonar.baffle_exclusion_deg);
    sonar.ownship_quieting_speed_mps =
        s.value("ownship_quieting_speed_mps", sonar.ownship_quieting_speed_mps);
...
bool parse_unit_type(const std::string &value, UnitType *out_type) {
    ...
    if (value == "Aircraft") {
        *out_type = UnitType::Aircraft;
        return true;
    }
    if (value == "Ship") {
        *out_type = UnitType::Ship;
        return true;
    }
    if (value == "Missile") {
        *out_type = UnitType::Missile;
        return true;
    }
    if (value == "Facility") {
        *out_type = UnitType::Facility;
        return true;
    }
    if (value == "C2Node") {
        *out_type = UnitType::C2Node;
        return true;
    }
    if (value == "Sensor") {
        *out_type = UnitType::Sensor;
        return true;
    }
    if (value == "Engine") {
        *out_type = UnitType::Engine;
        return true;
    }
    if (value == "EWSuite") {
        *out_type = UnitType::EWSuite;
        return true;
    }
    if (value == "RCSProfile") {
        *out_type = UnitType::RCSProfile;
        return true;
    }
    if (value == "Submarine") {
        *out_type = UnitType::Submarine;
        return true;
    }
    if (value == "Ground") {
        *out_type = UnitType::Ground;
        return true;
    }
...
bool validated_physics_surrogate_has_auditable_manifest(
    ...
    if (descriptor.validation_artifact_ref.empty() ||
        ...
        descriptor.validation_benchmark_ref.empty() || descriptor.validation_metrics_ref.empty() ||
    ...
    return descriptor.validation_scope_target_type == descriptor.target_type &&
           ...
           descriptor.validation_scope_miss_distance_bucket == descriptor.miss_distance_bucket;
}
...
bool vulnerability_evidence_descriptor_is_calibrated_match(
    ...
    const AircraftVulnerabilityProfile &profile, const std::string &target_type) {
    if (!descriptors || profile.evidence_dataset_ref.empty()) {
        return false;
    }
    if (profile.synthetic || !profile.calibrated || profile.calibration_status != "calibrated") {
        return false;
    }
    ...
    if (descriptor.schema_version != kVulnerabilityEvidenceSchemaVersion ||
        descriptor.source_ref.empty() || descriptor.provenance.empty()) {
        return false;
    }
    if (descriptor.weapon_family.empty() || descriptor.aspect_bucket.empty() ||
        descriptor.closure_bucket.empty() || descriptor.miss_distance_bucket.empty()) {
        return false;
    }
    return vulnerability_evidence_descriptor_has_authoritative_source(descriptor);
}

const VulnerabilityEvidenceDescriptor *
find_vulnerability_evidence_descriptor(const VulnerabilityEvidenceDescriptorMap *descriptors,
...
void copy_vulnerability_descriptor_metadata(const VulnerabilityEvidenceDescriptor *descriptor,
    ...
    profile->evidence_validation_artifact_ref = descriptor->validation_artifact_ref;
    profile->evidence_validation_manifest_schema_version =
        descriptor->validation_manifest_schema_version;
    profile->evidence_validation_status = descriptor->validation_status;
    profile->evidence_validation_artifact_sha256 = descriptor->validation_artifact_sha256;
    profile->evidence_validated_surrogate_model_ref = descriptor->validated_surrogate_model_ref;
    profile->evidence_validation_benchmark_ref = descriptor->validation_benchmark_ref;
    profile->evidence_validation_metrics_ref = descriptor->validation_metrics_ref;
    profile->evidence_validation_acceptance_criteria_ref =
        descriptor->validation_acceptance_criteria_ref;
}

bool vulnerability_row_matches_descriptor(const AircraftVulnerabilityEvidenceRow &row,
                                          const VulnerabilityEvidenceDescriptor &descriptor) {
...
bool vulnerability_row_has_authority_metadata(const AircraftVulnerabilityEvidenceRow &row) {
    return !row.row_id.empty() && !row.source_ref.empty() && !row.provenance.empty();
}

void copy_authoritative_vulnerability_rows(const VulnerabilityEvidenceDescriptor *descriptor,
                                           AircraftVulnerabilityProfile *profile) {
    ...
    profile->effect_scale_authority =
        descriptor->effect_scale_authority && !profile->evidence_rows.empty();
...
bool parse_unit_json(
    ...
    if (entry.contains("sensor_refs") && entry["sensor_refs"].is_array()) {
        ...
        const std::string default_sensor_type =
            (def.type == UnitType::Aircraft || def.type == UnitType::C2Node) ? "Radar" : "Radar";
    ...
    if (entry.contains("flight_model")) {
        ...
        def.flight_model.max_climb_rate =
            fm.value("max_climb_rate", def.flight_model.max_climb_rate);
    ...
    if (entry.contains("landing_gear")) {
        ...
        def.landing_gear.can_use_unpaved =
            lg.value("can_use_unpaved", def.landing_gear.can_use_unpaved);
        def.landing_gear.rolling_friction_coeff =
            lg.value("rolling_friction_coeff", def.landing_gear.rolling_friction_coeff);
        def.landing_gear.max_load_factor =
            lg.value("max_load_factor", def.landing_gear.max_load_factor);
    ...
    if (entry.contains("naval_logistics") && entry["naval_logistics"].is_object()) {
        ...
        def.naval_logistics.underway_replenishment_enabled = logistics.value(
            "underway_replenishment_enabled", def.naval_logistics.underway_replenishment_enabled);
        ...
        def.naval_logistics.max_relative_speed_mps =
            logistics.value("max_relative_speed_mps", def.naval_logistics.max_relative_speed_mps);
        def.naval_logistics.transfer_rate_fuel_units_per_s = logistics.value(
            "transfer_rate_fuel_units_per_s", def.naval_logistics.transfer_rate_fuel_units_per_s);
        def.naval_logistics.transfer_rate_missile_units_per_s =
            logistics.value("transfer_rate_missile_units_per_s",
                            def.naval_logistics.transfer_rate_missile_units_per_s);
    ...
    if (def.type == UnitType::Missile) {
        ...
        if (entry.contains("sensor") && entry["sensor"].is_object()) {
            ...
            const std::string default_sensor_type =
                missile_tuning.seeker_type == static_cast<int>(SensorType::Infrared) ? "Infrared"
                                                                                     : "Radar";
...
bool load_file(const std::string &path, std::vector<UnitDefinition> &out_definitions,
    ...
    // Case 2: Root IS the unit object (single file per unit)
        ...
        if (error)
            *error =
                "Invalid unit definition JSON: expected 'units' array or a single unit object.";
        return false;
    }
...
load_vulnerability_evidence_descriptors(const std::string &root_path) {
    ...
    for (const auto &entry : fs::directory_iterator(evidence_dir)) {
        ...
        try {
            ...
            descriptor.weapon_family = normalize_warhead_family(root.value("weapon_family", ""));
            ...
            descriptor.validation_artifact_ref = root.value("validation_artifact_ref", "");
            if (root.contains("validation_manifest") && root["validation_manifest"].is_object()) {
                ...
                descriptor.validation_metrics_ref = manifest.value("validation_metrics_ref", "");
                descriptor.validation_acceptance_criteria_ref =
                ...
                if (manifest.contains("validation_scope") &&
                    ...
                    descriptor.validation_scope_target_type = scope.value("target_type", "");
                    descriptor.validation_scope_weapon_family =
                        normalize_warhead_family(scope.value("weapon_family", ""));
                    descriptor.validation_scope_aspect_bucket = scope.value("aspect_bucket", "");
                    descriptor.validation_scope_closure_bucket = scope.value("closure_bucket", "");
            ...
            descriptor.effect_scale_authority = root.value("effect_scale_authority", false);
            descriptor.component_failure_probability_authority =
                root.value("component_failure_probability_authority", false);
            descriptor.pk_authority = root.value("pk_authority", false);
            descriptor.deterministic_fuze_authority = false;
            if (root.contains("rows") && root["rows"].is_array()) {
                for (const auto &row_json : root["rows"]) {
                    ...
                    row.aspect_bucket = row_json.value("aspect_bucket", descriptor.aspect_bucket);
                    row.closure_bucket =
                        row_json.value("closure_bucket", descriptor.closure_bucket);
                    row.miss_distance_bucket =
                        row_json.value("miss_distance_bucket", descriptor.miss_distance_bucket);
                    ...
                    row.miss_distance_scale =
                        row_json.value("miss_distance_scale", row.miss_distance_scale);
                    ...
                    parse_optional_evidence_row_number(row_json, "min_fragment_energy_j",
                                                       &row.has_min_fragment_energy_j,
                                                       &row.min_fragment_energy_j);
                    parse_optional_evidence_row_number(row_json, "max_fragment_energy_j",
                                                       &row.has_max_fragment_energy_j,
                                                       &row.max_fragment_energy_j);
                    parse_optional_evidence_row_number(row_json,
                    ...
                    parse_optional_evidence_row_number(row_json, "min_penetration_margin",
                                                       &row.has_min_penetration_margin,
                                                       &row.min_penetration_margin);
                    parse_optional_evidence_row_number(row_json, "max_penetration_margin",
                                                       &row.has_max_penetration_margin,
                                                       &row.max_penetration_margin);
                    parse_optional_evidence_row_number(row_json, "min_blast_overpressure_kpa",
                                                       &row.has_min_blast_overpressure_kpa,
                                                       &row.min_blast_overpressure_kpa);
                    parse_optional_evidence_row_number(row_json, "max_blast_overpressure_kpa",
                                                       &row.has_max_blast_overpressure_kpa,
                                                       &row.max_blast_overpressure_kpa);
                    parse_optional_evidence_row_number(row_json, "min_blast_impulse_kpa_ms",
                                                       &row.has_min_blast_impulse_kpa_ms,
                                                       &row.min_blast_impulse_kpa_ms);
                    parse_optional_evidence_row_number(row_json, "max_blast_impulse_kpa_ms",
                                                       &row.has_max_blast_impulse_kpa_ms,
                                                       &row.max_blast_impulse_kpa_ms);
                    parse_optional_evidence_row_number(row_json, "min_blast_scaled_distance_m_kg13",
                                                       &row.has_min_blast_scaled_distance_m_kg13,
                                                       &row.min_blast_scaled_distance_m_kg13);
                    parse_optional_evidence_row_number(row_json, "max_blast_scaled_distance_m_kg13",
                                                       &row.has_max_blast_scaled_distance_m_kg13,
                                                       &row.max_blast_scaled_distance_m_kg13);
                    parse_optional_evidence_row_number(row_json, "min_rod_cut_margin",
                                                       &row.has_min_rod_cut_margin,
                                                       &row.min_rod_cut_margin);
                    parse_optional_evidence_row_number(row_json, "max_rod_cut_margin",
                                                       &row.has_max_rod_cut_margin,
                                                       &row.max_rod_cut_margin);
                    parse_optional_evidence_row_number(row_json, "min_surface_incidence_cos",
                                                       &row.has_min_surface_incidence_cos,
                                                       &row.min_surface_incidence_cos);
                    parse_optional_evidence_row_number(row_json, "max_surface_incidence_cos",
