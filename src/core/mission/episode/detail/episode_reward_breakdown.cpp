...
void apply_flight_shaping_breakdown_terms(const FlightShapingRuntimeProducts &products,
                                          bool include_roll_stability, nlohmann::json *breakdown) {
...
std::string build_episode_reward_breakdown_json(const ExecutionEpisodeRuntimeInputs &runtime_inputs,
                                                const ExecutionEpisodeRuntimeProducts &products,
                                                double reward_total, bool waypoint_arrived,
    ...
    if (double(safety_terms.crash_penalty) != 0.0) {
        ...
        if (products.flight_shaping_evaluated) {
            apply_flight_shaping_breakdown_terms(products.flight_shaping,
                                                 runtime_inputs.include_roll_stability, &breakdown);
        }
        ...
        if (double(safety_terms.overload_penalty) != 0.0) {
            add_breakdown_term(&breakdown, "overload_penalty",
                               double(safety_terms.overload_penalty));
        }
        if (double(safety_terms.failfast_penalty) != 0.0) {
            add_breakdown_term(&breakdown, "failfast_penalty",
                               double(safety_terms.failfast_penalty));
        }
        if (double(safety_terms.gear_collapse_penalty) != 0.0) {
            add_breakdown_term(&breakdown, "gear_collapse_penalty",
                               double(safety_terms.gear_collapse_penalty));
        }
        if (double(safety_terms.off_runway_penalty) != 0.0) {
            add_breakdown_term(&breakdown, "off_runway_penalty",
                               double(safety_terms.off_runway_penalty));
        }
        if (double(safety_terms.gear_stress_penalty) != 0.0) {
            add_breakdown_term(&breakdown, "gear_stress_penalty",
                               double(safety_terms.gear_stress_penalty));
        }
        if (double(safety_terms.off_runway_terminate_penalty) != 0.0) {
            add_breakdown_term(&breakdown, "off_runway_terminate_penalty",
        ...
        if (runtime_inputs.execution_step.has_approach && execution_step.approach_evaluated) {
            ...
            if (double(approach_terms.approach_localizer) != 0.0) {
                add_breakdown_term(&breakdown, "approach_localizer",
                                   double(approach_terms.approach_localizer));
            }
            if (double(approach_inputs.localizer_improve_weight) != 0.0 &&
                bool(approach_inputs.has_prev_loc)) {
                add_breakdown_term(&breakdown, "approach_localizer_improve",
                                   double(approach_terms.approach_localizer_improve));
            }
            if (double(approach_terms.approach_glideslope) != 0.0) {
                add_breakdown_term(&breakdown, "approach_glideslope",
                                   double(approach_terms.approach_glideslope));
            }
            if (double(approach_inputs.glideslope_improve_weight) != 0.0 &&
                bool(approach_inputs.has_prev_gs)) {
                add_breakdown_term(&breakdown, "approach_glideslope_improve",
                                   double(approach_terms.approach_glideslope_improve));
            }
            if (double(approach_inputs.dme_progress_weight) != 0.0 &&
                bool(approach_inputs.has_prev_dme) &&
                std::isfinite(double(approach_inputs.ils_dme_m))) {
                add_breakdown_term(&breakdown, "approach_dme_progress",
                                   double(approach_terms.approach_dme_progress));
            }
            if (double(approach_terms.approach_capture_bonus) != 0.0) {
                add_breakdown_term(&breakdown, "approach_capture_bonus",
                                   double(approach_terms.approach_capture_bonus));
            }
            if (double(approach_terms.landing_sink_rate_penalty) != 0.0) {
                add_breakdown_term(&breakdown, "landing_sink_rate_penalty",
        ...
        if (runtime_inputs.execution_step.has_waypoint && execution_step.waypoint_evaluated) {
            ...
            if (double(waypoint_inputs.progress_weight) != 0.0 &&
                bool(waypoint_inputs.has_prev_dist)) {
                add_breakdown_term(&breakdown, "waypoint_progress",
                                   double(waypoint_terms.waypoint_progress));
            }
            if (double(waypoint_inputs.distance_weight) != 0.0) {
                add_breakdown_term(&breakdown, "waypoint_distance",
                                   double(waypoint_terms.waypoint_distance));
            }
            if (double(waypoint_terms.waypoint_cross_track) != 0.0) {
                add_breakdown_term(&breakdown, "waypoint_cross_track",
                                   double(waypoint_terms.waypoint_cross_track));
            }
            if (double(waypoint_terms.waypoint_proximity) != 0.0) {
                add_breakdown_term(&breakdown, "waypoint_proximity",
                                   double(waypoint_terms.waypoint_proximity));
            }
            if (waypoint_arrived) {
                add_breakdown_term(&breakdown, "waypoint_reached_bonus",
                                   double(waypoint_terms.waypoint_reached_bonus));
                if (!had_post_waypoint_transition_before &&
                    execution_step.waypoint_episode_success) {
                    add_breakdown_term(&breakdown, "waypoint_success_bonus",
        ...
        if (execution_step.objective_evaluated && execution_step.matched_objective_index >= 0) {
            if (double(execution_step.objective.success_runway_cross_penalty) != 0.0) {
                add_breakdown_term(&breakdown, "success_runway_cross_penalty",
            ...
            if (double(execution_step.objective.success_ground_track_error_penalty) != 0.0) {
                add_breakdown_term(
                    &breakdown, "success_ground_track_error_penalty",
                    double(execution_step.objective.success_ground_track_error_penalty));
            }
            add_breakdown_term(&breakdown, "objective_bonus",
