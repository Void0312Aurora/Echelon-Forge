...
bool mission_commands_equal(const MissionCommand &lhs, const MissionCommand &rhs) {
    ...
    const auto lhs_ground_static_task = mission_command_ground_static_task_directive(lhs);
    const auto rhs_ground_static_task = mission_command_ground_static_task_directive(rhs);

    return lhs_core == rhs_core && lhs_stationing == rhs_stationing &&
           lhs_embarked_helo == rhs_embarked_helo &&
           lhs_ground_static_task == rhs_ground_static_task && lhs_recovery == rhs_recovery &&
           lhs_takeoff == rhs_takeoff && lhs_formation == rhs_formation;
}

bool spatial_route_waypoints_equal(const SpatialRouteWaypoint &lhs,
                                   const SpatialRouteWaypoint &rhs) {
    return lhs.x_m == rhs.x_m && lhs.y_m == rhs.y_m && lhs.z_m == rhs.z_m &&
           lhs.radius_m == rhs.radius_m && lhs.altitude_m == rhs.altitude_m &&
           lhs.speed_mps == rhs.speed_mps && lhs.waypoint_mode == rhs.waypoint_mode;
}

bool spatial_route_waypoint_vectors_equal(const std::vector<SpatialRouteWaypoint> &lhs,
...
bool execution_episode_states_equivalent(const ExecutionEpisodeState &lhs,
                                         const ExecutionEpisodeState &rhs) {
    return lhs.agent_id == rhs.agent_id && lhs.step_count == rhs.step_count &&
           ...
           lhs.prev_altitude_m == rhs.prev_altitude_m && lhs.prev_ias_mps == rhs.prev_ias_mps &&
