...
enum class MissionObservationMode {
    ...
    NavV2 = 2,
    NavV2FormationV1 = 3,
    NavV2FormationRoleV1 = 4,
    NavV2CooperativeTakeoffV1 = 5,
};
...
MissionObservationMode parse_mission_observation_mode(int mode_code) {
    switch (mode_code) {
        ...
        case 3:
            return MissionObservationMode::NavV2FormationV1;
        case 4:
            return MissionObservationMode::NavV2FormationRoleV1;
        case 5:
            return MissionObservationMode::NavV2CooperativeTakeoffV1;
        default:
...
size_t mission_observation_nav_fields(MissionObservationMode mode) {
    switch (mode) {
        ...
        case MissionObservationMode::NavV2FormationV1:
            return 10;
        case MissionObservationMode::NavV2FormationRoleV1:
            return 10;
        case MissionObservationMode::NavV2CooperativeTakeoffV1:
            return 10;
        default:
            return 0;
    }
...
size_t mission_observation_formation_fields(MissionObservationMode mode) {
    switch (mode) {
        case MissionObservationMode::NavV2FormationV1:
            return 3;
        case MissionObservationMode::NavV2FormationRoleV1:
...
MissionObservationProducts compute_mission_observation(const MissionObservationInputs& inputs) {
    ...
    const size_t formation_fields = mission_observation_formation_fields(mode);

    out.valid = true;
    out.mode_code = inputs.mode_code;
    out.values.reserve(static_cast<size_t>(4) + nav_fields + formation_fields);
    ...
    if (nav_fields > 0) {
        ...
        } else if (mode == MissionObservationMode::NavV1) {
    ...
    if (formation_fields > 0) {
        if (mode == MissionObservationMode::NavV2CooperativeTakeoffV1) {
            out.values.push_back(static_cast<float>(inputs.takeoff_procedure_code));
            out.values.push_back(static_cast<float>(inputs.takeoff_clearance_code));
            out.values.push_back(static_cast<float>(inputs.takeoff_interval_s));
            out.values.push_back(static_cast<float>(inputs.runway_slot_code));
            out.values.push_back(static_cast<float>(inputs.form_offset_x));
            out.values.push_back(static_cast<float>(inputs.form_offset_y));
            out.values.push_back(static_cast<float>(inputs.form_offset_z));
            out.values.push_back(static_cast<float>(inputs.self_role_code));
            out.values.push_back(static_cast<float>(inputs.self_formation_role_code));
            out.values.push_back(static_cast<float>(inputs.relative_slot_code));
            out.values.push_back(static_cast<float>(inputs.reference_relative_slot_code));
        } else {
            out.values.push_back(static_cast<float>(inputs.form_offset_x));
            out.values.push_back(static_cast<float>(inputs.form_offset_y));
            out.values.push_back(static_cast<float>(inputs.form_offset_z));
            if (mode == MissionObservationMode::NavV2FormationRoleV1) {
                out.values.push_back(static_cast<float>(inputs.self_role_code));
                out.values.push_back(static_cast<float>(inputs.self_formation_role_code));
                out.values.push_back(static_cast<float>(inputs.relative_slot_code));
                out.values.push_back(static_cast<float>(inputs.reference_relative_slot_code));
