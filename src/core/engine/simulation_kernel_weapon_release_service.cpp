...
bool has_explicit_global_missile_tuning(const MissileTuning &tuning) {
    return std::isfinite(tuning.max_speed) || std::isfinite(tuning.turn_rate) ||
           ...
           std::isfinite(tuning.target_tracker_gamma) ||
...
flecs::entity SimulationKernelWeaponReleaseService::fire_missile(uint64_t attacker_id,
    ...
    const double missile_target_tracker_beta = std::clamp(
        ...
        0.0, 2.0);
    const double missile_target_tracker_gamma = std::clamp(
        finite_or_default(resolved_tuning.target_tracker_gamma,
    ...
    missile.proximity_last_sample_time_s = std::numeric_limits<double>::quiet_NaN();
    missile.proximity_last_missile_x_m = std::numeric_limits<double>::quiet_NaN();
    missile.proximity_last_missile_y_m = std::numeric_limits<double>::quiet_NaN();
    missile.proximity_last_missile_z_m = std::numeric_limits<double>::quiet_NaN();
    missile.proximity_last_target_x_m = std::numeric_limits<double>::quiet_NaN();
    missile.proximity_last_target_y_m = std::numeric_limits<double>::quiet_NaN();
    missile.proximity_last_target_z_m = std::numeric_limits<double>::quiet_NaN();
