...
SimulationKernel::resolve_missile_launch_definition(flecs::entity attacker, const PilotAction* pilot) const {
    ...
    ecs_iter_t it = ecs_children(ecs.c_ptr(), attacker.id());
    while (ecs_children_next(&it)) {
        for (int i = 0; i < it.count; ++i) {
            const ecs_entity_t child_id = it.entities[i];
            auto child = ecs.entity(child_id);
            ...
            const auto platform_name = platform_definition_name_from_munition_name(
                ecs_get_name(ecs.c_ptr(), child_id),
                munition->station_id);
...
flecs::entity SimulationKernel::fire_missile(uint64_t attacker_id, uint64_t target_id) {
    ...
    if (resolved_launch_definition.has_value() && resolved_launch_definition->munition_entity_id != 0) {
        auto munition = ecs.entity(resolved_launch_definition->munition_entity_id);
