...
std::uint64_t current_source_frame(flecs::world &ecs) {
    ...
    return info && info->frame_count_total > 0 ? static_cast<std::uint64_t>(info->frame_count_total)
                                               : 0;
}

std::uint64_t find_launch_event_id_for_munition(const RecentEngagementEvents &events,
...
void complete_lethality_header(LethalityChainHeader &header, const std::string &stage,
                               const std::string &default_status, double event_time_s,
                               std::uint64_t event_id, std::uint64_t chain_id,
                               std::uint64_t parent_event_id, std::uint64_t munition_entity_id,
                               std::uint64_t shooter_id, std::uint64_t target_id,
...
SimulationKernelEngagementEventStore::capture_engagement_damage_state(uint64_t target_id) const {
...
std::uint64_t SimulationKernelEngagementEventStore::record_legacy_launch_event(
    uint64_t shooter_id, uint64_t, uint64_t spawned_munition_id,
    const std::string &selected_launcher, const std::string &selected_munition, int ammo_delta,
    double cooldown_delta_s, double event_time_s) {
    ...
    while (recent_engagement_events_.launch_events.size() > kMaxRecentEngagementEvents) {
        recent_engagement_events_.launch_events.erase(
            recent_engagement_events_.launch_events.begin());
    }
    ...
    while (recent_engagement_events_.diagnostics_traces.size() > kMaxRecentEngagementEvents) {
        recent_engagement_events_.diagnostics_traces.erase(
            recent_engagement_events_.diagnostics_traces.begin());
    }
    (void)shooter_id;
    return event_id;
}
...
std::uint64_t SimulationKernelEngagementEventStore::record_nearest_approach_event(
    ...
    if (record.munition_entity_id != 0) {
        for (auto it = recent_engagement_events_.launch_events.rbegin();
             it != recent_engagement_events_.launch_events.rend(); ++it) {
...
std::uint64_t SimulationKernelEngagementEventStore::record_fuze_evaluation_event(
    ...
    if (record.munition_entity_id != 0) {
        for (auto it = recent_engagement_events_.nearest_approach_events.rbegin();
             it != recent_engagement_events_.nearest_approach_events.rend(); ++it) {
...
std::uint64_t SimulationKernelEngagementEventStore::record_effects_damage_event(
    ...
    if (effects_reached_warhead_loads) {
        ...
        warhead_event.fragment_density_per_m2 = effects.mechanism_fragment_areal_density_per_m2;
        ...
        warhead_event.blast_scaled_distance_m_kg13 = effects.mechanism_blast_scaled_distance_m_kg13;
        ...
        for (const ComponentMechanismLoadRow &row : effects.component_mechanism_load_rows) {
            ...
            component_event.component_redundancy_group_id = row.component_redundancy_group_id;
    ...
    const auto min_damage_capability = [](const EngagementDamageStateSnapshot &snapshot) {
        return std::min(std::min(snapshot.mission_capability, snapshot.mobility_capability),
                        std::min(snapshot.sensor_capability, snapshot.survivability_margin));
    };

    char damage_delta[160];
    std::snprintf(damage_delta, sizeof(damage_delta),
...
RecentEngagementEvents SimulationKernelEngagementEventStore::export_recent_events_sorted() const {
    ...
    std::sort(
        out.launch_events.begin(), out.launch_events.end(),
        [](const LaunchEvent &lhs, const LaunchEvent &rhs) { return lhs.event_id < rhs.event_id; });
    std::sort(out.effects_events.begin(), out.effects_events.end(),
              [](const EffectsEvent &lhs, const EffectsEvent &rhs) {
                  return lhs.event_id < rhs.event_id;
              });
    std::sort(out.nearest_approach_events.begin(), out.nearest_approach_events.end(),
              [](const NearestApproachEvent &lhs, const NearestApproachEvent &rhs) {
                  return lhs.header.event_id < rhs.header.event_id;
              });
    std::sort(out.fuze_evaluation_events.begin(), out.fuze_evaluation_events.end(),
              [](const FuzeEvaluationEvent &lhs, const FuzeEvaluationEvent &rhs) {
                  return lhs.header.event_id < rhs.header.event_id;
              });
    std::sort(out.warhead_mechanism_events.begin(), out.warhead_mechanism_events.end(),
              [](const WarheadMechanismEvent &lhs, const WarheadMechanismEvent &rhs) {
                  return lhs.header.event_id < rhs.header.event_id;
              });
    std::sort(out.spatial_coverage_events.begin(), out.spatial_coverage_events.end(),
    ...
    std::sort(out.diagnostics_traces.begin(), out.diagnostics_traces.end(),
