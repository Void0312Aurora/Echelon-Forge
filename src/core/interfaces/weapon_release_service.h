...
class IWeaponReleaseService {
  public:
    ...
    virtual flecs::entity fire_missile(std::uint64_t attacker_id, std::uint64_t target_id) = 0;
    virtual bool fire_naval_weapon(std::uint64_t attacker_id, std::uint64_t target_id,
...
struct WeaponReleaseServiceRef {
    IWeaponReleaseService *service = nullptr;
};
