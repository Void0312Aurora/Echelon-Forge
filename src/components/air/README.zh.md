# `src/components/air` 边界

`components/air` 保存 air-domain ECS value type 与 tuning state。它是 fixed-wing
...
- 被 `systems/air` 和 air model 消费的 flight dynamics tuning。
...
旧的 `components/physics/flight_dynamics_tuning.h` 头文件保留为 include-only
wrapper。新增代码应直接 include `components/air/flight_dynamics_tuning.h`。
