...
static_assert(kTaskOrderAirOwnedDomainSlice && kTaskOrderNavalOwnedDomainSlice &&
...
task_order_air_recovery_directive(const TaskOrderCompatibilityTransportShell &order) noexcept {
    return task_order_air_recovery_directive(task_order_air_owner_slice(order));
}

[[nodiscard]] inline TaskOrderAir::TakeoffDirective
task_order_air_takeoff_directive(const TaskOrderCompatibilityTransportShell &order) noexcept {
...
task_order_naval_command_authority(const TaskOrderCompatibilityTransportShell &order) noexcept {
    return task_order_naval_command_authority(task_order_naval_owner_slice(order));
}

[[nodiscard]] inline TaskOrderGround::StaticTaskDirective task_order_ground_static_task_directive(
