...
static_assert(
    kTaskOrderAirOwnedDomainSlice &&
        kTaskOrderNavalOwnedDomainSlice &&
...
task_order_ground_static_status_directive(
    ...
    return task_order_ground_static_status_directive(
        task_order_ground_owner_slice(order),
        order.supported_node_id,
        order.supporting_node_id
    );
}
