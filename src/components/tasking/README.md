...
maintained shape is a common C2/tasking foundation with air and naval
extensions: `common` carries shared semantics, `air` carries the mature air
mission-organization surface, and `naval` carries the first-stage maritime
tasking slice. Ground tasking/native-schema evidence is still bootstrap-only
and has not landed here as a maintained `ground/` subdomain.
...
- `ground tasking` remains bootstrap-only in this C++ component layer. Do not represent land movement, sensing, fires, damage, or terrain-control semantics as generic `common` fields just to avoid a later dedicated ground schema.
...
- Ground-aware setup exists elsewhere through unit type/capability evidence; maintained ground tasking/runtime remains held beyond bootstrap evidence.
