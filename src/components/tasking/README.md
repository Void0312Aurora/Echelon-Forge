...
slice, and `ground` carries the early G0/G1 static task/status owner slices.
Ground runtime movement, sensing, fires, terrain, and damage remain held.
...
- Task status objects such as `TaskOrder`, `LeaderIntent`, `PilotReport`, and their `common` / `air` / `naval` / `ground` layered versions.
...
- `leader_intent.h`
- `pilot_report.h`
- `common/*`, `air/*`, `naval/*`, and `ground/*` owner slices for task order, leader intent, and pilot report.
...
- `tasking_enums.h` is retained as a compatibility umbrella; new code should preferentially explicitly depend on `common/core_tasking_enums.h`, `air/air_tasking_enums.h`, `naval/naval_tasking_enums.h`, or `ground/ground_tasking_enums.h`.
