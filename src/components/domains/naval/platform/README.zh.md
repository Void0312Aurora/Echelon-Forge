# `src/components/domains/naval/platform` 边界

`components/domains/naval/platform` 保存舰艇、潜艇和舰载航空运作的静态/缓变平台状态组件。
...
- 舰载直升机/甲板运作所需的轻量状态组件。
- 可被 `systems/domains/naval`、`models/`、`core/mission` 读取的 naval platform DTO。

## 禁止

- 舰艇/潜艇运动积分、海况推进或舰载机调度逻辑；这些属于 `systems/domains/naval`。
...
本目录是数据层。`systems/domains/naval`、`core/mission`、`runtime/facade` 和
