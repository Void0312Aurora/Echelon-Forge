...
inline EulerRotationCoefficients euler_rotation_coefficients(double heading_deg, double pitch_deg,
                                                             double roll_deg) {
    ...
    return {
        std::cos(psi),   std::sin(psi), std::cos(theta),
        std::sin(theta), std::cos(phi), std::sin(phi),
    };
}

inline Vector3 world_to_body(const Vector3 &v_world, double heading_deg, double pitch_deg,
                             double roll_deg) {
    const EulerRotationCoefficients rot =
        euler_rotation_coefficients(heading_deg, pitch_deg, roll_deg);
    return {
        rot.c_psi * rot.c_theta * v_world.x + rot.s_psi * rot.c_theta * v_world.y -
...
inline Vector3 body_to_world(const Vector3 &v_body, double heading_deg, double pitch_deg,
                             double roll_deg) {
    const EulerRotationCoefficients rot =
        euler_rotation_coefficients(heading_deg, pitch_deg, roll_deg);
    return {
        ...
        -rot.s_theta * v_body.x + rot.c_theta * rot.s_phi * v_body.y +
...
enum class Side : uint8_t { Unknown = 0, Blue, Red, Neutral };
