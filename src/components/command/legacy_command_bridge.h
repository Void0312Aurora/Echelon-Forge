...
inline double resolved_compatibility_damage_evasion(const ActionCommand *legacy_action) {
...
inline double resolved_compatibility_damage_evasion(const EntityT &entity) {
    return resolved_compatibility_damage_evasion(entity.template get<ActionCommand>());
}

inline ResolvedCompatibilityCountermeasureCommand
resolve_compatibility_countermeasure_command(const PilotAction *pilot,
...
resolve_compatibility_message_command(const ActionCommand *legacy_action) {
    const ActionCommand *action = active_legacy_action_command(legacy_action);
    if (!action) {
        return {};
    }
...
template <typename EntityT>
    requires(!std::is_pointer_v<std::remove_reference_t<EntityT>>)
inline ResolvedCompatibilityMessageCommand
resolve_compatibility_message_command(const EntityT &entity) {
    return resolve_compatibility_message_command(entity.template get<ActionCommand>());
}

inline void set_compatibility_autopilot_movement_command(MovementCommand &legacy_movement,
                                                         double heading_deg, double speed_mps,
                                                         double altitude_m) {
    legacy_movement = make_legacy_autopilot_movement_command(heading_deg, speed_mps, altitude_m);
}

template <typename EntityT>
    requires(!std::is_pointer_v<std::remove_reference_t<EntityT>>)
inline MissionCommandControlState &ensure_mission_command_control_state(EntityT &entity) {
    if (MissionCommandControlState *state = entity.template get_mut<MissionCommandControlState>()) {
        return *state;
    }
...
template <typename EntityT>
    requires(!std::is_pointer_v<std::remove_reference_t<EntityT>>)
inline MovementCommand &ensure_compatibility_movement_command(EntityT &entity) {
    if (MovementCommand *legacy_movement = entity.template get_mut<MovementCommand>()) {
...
template <typename EntityT>
    requires(!std::is_pointer_v<std::remove_reference_t<EntityT>>)
inline LaggedCommand &ensure_compatibility_lagged_command(EntityT &entity) {
...
inline void refresh_compatibility_autopilot_movement_command_from_control_state(
    MovementCommand &legacy_movement, const MissionCommandControlState &state) {
...
inline void refresh_compatibility_lagged_command_mirror_from_control_state(
    LaggedCommand &lagged, const MissionCommandControlState &state) {
...
inline void refresh_optional_compatibility_autopilot_movement_command_from_control_state(
    ...
    refresh_compatibility_autopilot_movement_command_from_control_state(*legacy_movement, state);
}
...
inline void refresh_optional_compatibility_lagged_command_mirror_from_control_state(
    ...
    refresh_compatibility_lagged_command_mirror_from_control_state(*lagged, state);
}
...
inline MissionCommandControlState make_compatibility_control_state_seed(const Transform &transform,
                                                                        const Velocity &velocity) {
    return make_mission_command_control_state(compatibility_wrap_angle_360(transform.heading),
                                              compatibility_speed_from_velocity(velocity),
                                              transform.z);
}

inline MissionCommandTypedAirControlState
make_compatibility_typed_air_control_from_legacy_movement(const MovementCommand &legacy_movement) {
...
inline MissionCommandTypedAirControlState
make_compatibility_typed_air_control_from_legacy_action(const ActionCommand &legacy_action) {
    MissionCommandTypedAirControlState typed_air_control{};
    typed_air_control.throttle_command =
        std::clamp((legacy_action.accel_cmd + 1.0) * 0.5, 0.0, 1.0);
...
inline void refresh_compatibility_typed_air_control_from_legacy_movement(
    MissionCommandControlState &state, const MovementCommand &legacy_movement) {
    MissionCommandTypedAirControlState typed_air_control = state.typed_air_control;
...
inline void
refresh_compatibility_typed_air_control_from_legacy_action(MissionCommandControlState &state,
                                                           const ActionCommand &legacy_action) {
    MissionCommandTypedAirControlState typed_air_control = state.typed_air_control;
    const MissionCommandTypedAirControlState legacy_projection =
...
inline void refresh_compatibility_typed_air_control_from_pending_action_bridge(
    ...
    typed_air_control.action_semantics_active = pending_action_bridge.action_semantics_active;
...
inline void refresh_compatibility_control_mirrors_from_state(EntityT &entity) {
    const MissionCommandControlState &state = ensure_mission_command_control_state(entity);
    refresh_compatibility_autopilot_movement_command_from_control_state(
        ensure_compatibility_movement_command(entity), state);
    refresh_compatibility_lagged_command_mirror_from_control_state(
        ensure_compatibility_lagged_command(entity), state);
}

template <typename EntityT>
    requires(!std::is_pointer_v<std::remove_reference_t<EntityT>>)
inline void set_compatibility_autopilot_control_target(EntityT &entity, double heading_deg,
                                                       double speed_mps, double altitude_m) {
    MissionCommandControlState &state = ensure_mission_command_control_state(entity);
    set_mission_command_control_target(state, heading_deg, speed_mps, altitude_m, true);
    if (!state.lagged_active) {
        set_mission_command_control_lagged(state, heading_deg, speed_mps, altitude_m, true);
    }
...
template <typename EntityT>
    requires(!std::is_pointer_v<std::remove_reference_t<EntityT>>)
inline void set_compatibility_autopilot_movement_command(EntityT &entity, double heading_deg,
                                                         double speed_mps, double altitude_m) {
    MissionCommandControlState &state = ensure_mission_command_control_state(entity);
    set_mission_command_control_target(state, heading_deg, speed_mps, altitude_m, true);
    set_mission_command_control_lagged(state, heading_deg, speed_mps, altitude_m, true);
...
template <typename EntityT>
    requires(!std::is_pointer_v<std::remove_reference_t<EntityT>>)
inline void set_quarantined_compatibility_stick_movement_command(EntityT &entity, double stick_roll,
                                                                 double stick_pitch,
                                                                 double throttle_cmd,
                                                                 bool gear_down) {
    ...
    legacy_movement =
        make_legacy_stick_movement_command(stick_roll, stick_pitch, throttle_cmd, gear_down);
    refresh_compatibility_typed_air_control_from_legacy_movement(
        ensure_mission_command_control_state(entity), legacy_movement);
}
...
template <typename EntityT>
    requires(!std::is_pointer_v<std::remove_reference_t<EntityT>>)
inline void
refresh_compatibility_typed_air_control_from_action_command(EntityT &entity,
                                                            const ActionCommand &legacy_action) {
    refresh_compatibility_typed_air_control_from_legacy_action(
        ensure_mission_command_control_state(entity), legacy_action);
}

template <typename EntityT>
    requires(!std::is_pointer_v<std::remove_reference_t<EntityT>>)
inline void refresh_optional_compatibility_typed_air_control_from_action_command(
    EntityT &entity, const ActionCommand &legacy_action) {
    if (MissionCommandControlState *state = entity.template get_mut<MissionCommandControlState>()) {
        refresh_compatibility_typed_air_control_from_legacy_action(*state, legacy_action);
    }
}
...
inline void refresh_optional_pending_action_typed_air_control_bridge(
    EntityT &entity, const PendingActionCommand &pending_action) {
    if (MissionCommandControlState *state = entity.template get_mut<MissionCommandControlState>()) {
        refresh_compatibility_typed_air_control_from_pending_action_bridge(
            *state, pending_action.typed_air_control_bridge);
    }
}
