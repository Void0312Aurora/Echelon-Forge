...
inline bool runtime_window_has_payload(const RuntimeWindowActionRequest &request) {
    return request.action_intent.has_pilot_action || request.action_intent.has_mission_command;
}
...
inline bool runtime_window_is_supported_clock_merge_policy(std::string_view policy) {
    return policy == "nested_slot" || policy == "hold_last" || policy == "interpolate" ||
           policy == "enqueue_event" || policy == "defer_to_next_window" ||
           policy == "reject_on_ambiguous_order";
}

inline bool runtime_window_is_independent_clock_domain_relation(std::string_view relation) {
    return relation == "independent";
}

inline bool
runtime_window_has_selected_barrier_order(const std::vector<std::string> &barrier_order) {
    ...
    for (const auto &barrier_id : barrier_order) {
        const auto it =
            std::find(selected_barriers.begin() + static_cast<std::ptrdiff_t>(next_index),
                      selected_barriers.end(), barrier_id);
        if (it == selected_barriers.end()) {
            return false;
        }
        next_index = static_cast<std::size_t>(std::distance(selected_barriers.begin(), it) + 1);
    }
    return true;
}
...
runtime_window_partial_barrier_order(std::initializer_list<std::string_view> barrier_ids) {
...
inline std::string
runtime_window_input_source_snapshot_version(const RuntimeWindowActionRequest &request) {
    if (!runtime_window_is_blank(request.clock_domain_metadata.source_snapshot_version)) {
        return request.clock_domain_metadata.source_snapshot_version;
    }
    return request.input_snapshot_version;
}

inline double runtime_window_input_source_time_s(const RuntimeWindowActionRequest &request,
                                                 double fallback_source_time_s) {
    if (request.clock_domain_metadata.has_source_time &&
        runtime_window_has_finite_time(request.clock_domain_metadata.source_time_s)) {
...
inline std::string
runtime_window_input_target_window_id(const RuntimeWindowActionRequest &request,
...
inline std::vector<std::string>
runtime_window_input_barrier_order(const RuntimeWindowActionRequest &request,
...
inline std::string
runtime_window_input_clock_merge_policy(const RuntimeWindowActionRequest &request,
...
inline RuntimeWindowCadence runtime_window_make_cadence(std::string_view domain,
                                                        std::uint32_t tick_count, double interval_s,
...
inline RuntimeWindowCadenceConfig runtime_window_default_selected_slice_cadence_config() {
    RuntimeWindowCadenceConfig config{};
    config.window_duration_s = 0.1;
    config.domains = {
        runtime_window_make_cadence(kRuntimeWindowCadenceDomainPolicy, 1U, 0.1, "nested_slot",
                                    kRuntimeWindowBarrierInputInjection),
        runtime_window_make_cadence(kRuntimeWindowCadenceDomainControl, 2U, 0.05, "hold_last",
                                    kRuntimeWindowBarrierInputInjection),
        runtime_window_make_cadence(kRuntimeWindowCadenceDomainPhysics, 6U, 1.0 / 60.0,
                                    "enqueue_event", kRuntimeWindowBarrierWindowCommit),
        runtime_window_make_cadence(kRuntimeWindowCadenceDomainExport, 1U, 0.1, "nested_slot",
                                    kRuntimeWindowBarrierExport),
    };
    return config;
}
...
inline std::size_t
runtime_window_find_cadence_domain_index(const RuntimeWindowCadenceConfig &config,
                                         std::string_view domain) {
    const auto it = std::find_if(
        config.domains.begin(), config.domains.end(),
        [domain](const RuntimeWindowCadence &cadence) { return cadence.domain == domain; });
    if (it == config.domains.end()) {
        return config.domains.size();
    }
    return static_cast<std::size_t>(std::distance(config.domains.begin(), it));
}

inline const RuntimeWindowCadence *
runtime_window_find_cadence_domain(const RuntimeWindowCadenceConfig &config,
                                   std::string_view domain) {
    const std::size_t index = runtime_window_find_cadence_domain_index(config, domain);
...
inline void runtime_window_append_default_cadence_domain_if_missing(
    RuntimeWindowCadenceConfig *config, const RuntimeWindowCadence &default_cadence) {
    if (config == nullptr) {
        return;
    }
...
normalize_runtime_window_cadence_config(const RuntimeWindowRequest &request) {
    ...
    runtime_window_append_default_cadence_domain_if_missing(&config, defaults.domains[0]);
    runtime_window_append_default_cadence_domain_if_missing(&config, defaults.domains[1]);
    runtime_window_append_default_cadence_domain_if_missing(&config, defaults.domains[2]);
    runtime_window_append_default_cadence_domain_if_missing(&config, defaults.domains[3]);
    ...
    for (auto &cadence : config.domains) {
        ...
        if (cadence.tick_count == 0U) {
            cadence.tick_count = default_cadence != nullptr ? default_cadence->tick_count : 1U;
        }
        if (!runtime_window_has_finite_time(cadence.interval_s) || cadence.interval_s <= 0.0) {
            cadence.interval_s = config.window_duration_s / static_cast<double>(cadence.tick_count);
        }
        if (runtime_window_is_blank(cadence.merge_policy) && default_cadence != nullptr) {
            cadence.merge_policy = default_cadence->merge_policy;
        }
        if (runtime_window_is_blank(cadence.barrier_id) && default_cadence != nullptr) {
...
inline ActionHoldPolicy
runtime_window_normalized_hold_policy(const RuntimeWindowActionRequest &request) {
    return normalize_action_hold_policy(request.cadence_control.hold_policy);
}

inline double runtime_window_resolve_hold_expiry_time_s(const RuntimeWindowActionRequest &request,
                                                        double fallback_source_time_s) {
    ...
    const ActionHoldPolicy hold_policy = runtime_window_normalized_hold_policy(request);
...
inline bool runtime_window_hold_candidate_expired_at_tick(const RuntimeWindowActionRequest &request,
                                                          double fallback_source_time_s,
                                                          double tick_time_s) {
...
inline bool runtime_window_has_requested_export(const RuntimeWindowRequest &request) {
    return request.export_observation || request.export_engagement || request.export_diagnostics;
}
