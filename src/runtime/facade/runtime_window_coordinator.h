...
inline constexpr std::string_view kRuntimeWindowBarrierInputInjection = "input_injection";
inline constexpr std::string_view kRuntimeWindowBarrierStagePublish = "stage_publish";
inline constexpr std::string_view kRuntimeWindowBarrierWindowCommit = "window_commit";
...
struct RuntimeWindowCoordinatorCallbacks {
    std::function<void(const std::vector<WorldPilotActionAssignment> &)> apply_pilot_actions;
...
inline bool runtime_window_requests_conflict(const RuntimeWindowActionRequest &lhs,
...
inline RuntimeWindowSchedulingContext
classify_runtime_window_inputs(const RuntimeWindowRequest &request) {
    ...
    for (const auto &action_request : request.action_requests) {
        ...
        if (!runtime_window_is_blank(action_request.clock_domain_metadata.clock_merge_policy) &&
            !runtime_window_is_supported_clock_merge_policy(
                action_request.clock_domain_metadata.clock_merge_policy)) {
            record.reason = "clock_domain_metadata.clock_merge_policy is not supported by the "
                            "WP16-B cadence gate";
            context.rejected_inputs.push_back(record);
            continue;
        }
        if (runtime_window_is_independent_clock_domain_relation(
            ...
            if (action_request.clock_domain_metadata.diagnostics_only) {
                record.reason = action_request.clock_domain_metadata.diagnostics_reason.empty()
                                    ? "independent_clock_domain diagnostics-only inputs cannot "
                                      "enter the maintained slice"
                                    : action_request.clock_domain_metadata.diagnostics_reason;
                context.rejected_inputs.push_back(record);
                continue;
            }
            if (runtime_window_is_blank(action_request.clock_domain_metadata.clock_merge_policy)) {
                record.reason = "independent_clock_domain inputs require clock_merge_policy";
                context.rejected_inputs.push_back(record);
                continue;
            }
            if (!runtime_window_has_finite_time(
                ...
                record.reason = "independent_clock_domain inputs require finite source_time_s";
                context.rejected_inputs.push_back(record);
                continue;
            }
            if (runtime_window_is_blank(
                    action_request.clock_domain_metadata.source_snapshot_version)) {
                record.reason = "independent_clock_domain inputs require source_snapshot_version";
                context.rejected_inputs.push_back(record);
                continue;
            }
            if (runtime_window_is_blank(action_request.clock_domain_metadata.target_window_id)) {
                record.reason = "independent_clock_domain inputs require target_window_id";
                context.rejected_inputs.push_back(record);
                continue;
            }
            if (!runtime_window_has_selected_barrier_order(
                    action_request.clock_domain_metadata.barrier_order)) {
                record.reason = "independent_clock_domain inputs require deterministic "
                                "selected-slice barrier_order metadata";
                context.rejected_inputs.push_back(record);
                continue;
            }
            if (action_request.clock_domain_metadata.target_window_id != context.window_id) {
                record.reason =
        ...
        if (intent.valid_until_s != 0.0 && intent.valid_until_s < request.source_time_s) {
    ...
    for (std::size_t index = 0; index < context.accepted_inputs.size(); ++index) {
        ...
        for (std::size_t other = 0; other < context.accepted_inputs.size(); ++other) {
            ...
            if (runtime_window_requests_conflict(context.accepted_inputs[index].request,
...
inline RuntimeWindowResult
execute_runtime_window(const RuntimeWindowRequest &request,
    ...
    result.cadence_trace = build_runtime_window_cadence_trace(request, result.context,
                                                              result.cadence_config, callbacks);
    ...
    for (const auto &record : result.context.accepted_inputs) {
        ...
        if (intent.has_mission_command) {
            mission_assignments.push_back(WorldMissionCommandMaintainedAssignment{
                ...
                .entity_id = intent.target.entity_id,
                .mission_command =
                    mission_command_maintained_batch_contract(intent.mission_command),
            });
        }
    }

    runtime_window_advance_barrier(&result.context, &result.barrier_trace,
    ...
    if (fire_control_manifest != nullptr) {
        result.executed_nodes.push_back(runtime_window_fire_control_launch_record(
            *fire_control_manifest, result.context, result.cadence_trace));
    }
    ...
    if (effects_damage_manifest != nullptr && !result.executed_nodes.empty()) {
        result.executed_nodes.push_back(runtime_window_effects_damage_record(
            *effects_damage_manifest, result.context, result.executed_nodes.front(),
    ...
    for (const auto *manifest : manifests) {
        ...
        const bool stage_publish_required =
            manifest->write_commit_policy == runtime::scheduler::kWriteCommitPolicyStagePublish ||
            runtime::scheduler::contains_value(manifest->required_barriers,
                                               std::string(kRuntimeWindowBarrierStagePublish));
        if (!stage_publish_required) {
            continue;
        }
        runtime_window_advance_barrier(&result.context, &result.barrier_trace,
                                       kRuntimeWindowBarrierStagePublish, manifest->node_id);
    }
    ...
    if (export_manifest != nullptr) {
        export_record = runtime_window_observation_export_record(
            *export_manifest, request, result.context, callbacks, result.cadence_trace);
    ...
    if (export_record_present && export_record.execution_state == "executed" &&
        ...
        result.observation_packet = callbacks.export_observation_packet(observation_request);
    }
    if (export_record_present && export_record.execution_state == "executed" &&
        request.export_engagement && callbacks.export_engagement_event_packet) {
        result.engagement_packet = callbacks.export_engagement_event_packet(engagement_request);
    }
    if (export_record_present && export_record.execution_state == "executed" &&
        request.export_diagnostics && callbacks.export_diagnostics_traces) {
        result.diagnostics_traces = callbacks.export_diagnostics_traces(engagement_request);
    }

    if (export_record_present) {
        export_record.source_snapshot_version = runtime_window_export_snapshot_evidence(result);
