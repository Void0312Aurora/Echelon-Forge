...
inline void
runtime_window_append_cadence_trace(std::vector<RuntimeWindowCadenceTraceRecord> *cadence_trace,
...
inline const RuntimeWindowCadenceTraceRecord *runtime_window_preferred_cadence_trace_record(
    const std::vector<RuntimeWindowCadenceTraceRecord> &cadence_trace, std::string_view domain,
    ...
    for (const auto &trace : cadence_trace) {
        ...
        const int priority = runtime_window_cadence_decision_priority(trace.decision);
...
inline void runtime_window_append_policy_cadence_trace(
    const RuntimeWindowSchedulingContext &context, const RuntimeWindowCadenceConfig &cadence_config,
    std::vector<RuntimeWindowCadenceTraceRecord> *cadence_trace) {
    const RuntimeWindowCadence *policy =
        runtime_window_find_cadence_domain(cadence_config, kRuntimeWindowCadenceDomainPolicy);
    if (policy == nullptr) {
        return;
    }
    ...
    if (!context.accepted_inputs.empty()) {
        ...
        record.clock_merge_policy = runtime_window_input_clock_merge_policy(request, "nested_slot");
        ...
        record.decision_reason = "policy boundary rejected the selected-slice trigger";
        record.source = request.action_intent.source_id;
        record.clock_domain = request.clock_domain_metadata.source_clock_domain;
        record.clock_merge_policy =
            runtime_window_input_clock_merge_policy(request, "reject_on_ambiguous_order");
        record.relation = request.clock_domain_metadata.relation;
        ...
        record.source = request.action_intent.source_id;
        record.clock_domain = request.clock_domain_metadata.source_clock_domain;
        record.clock_merge_policy = runtime_window_input_clock_merge_policy(request, "drop");
        record.relation = request.clock_domain_metadata.relation;
        record.expired = true;
    } else {
...
inline void runtime_window_append_control_cadence_trace(
    ...
    const RuntimeWindowCadence *control =
        runtime_window_find_cadence_domain(cadence_config, kRuntimeWindowCadenceDomainControl);
    if (control == nullptr) {
        return;
    }
    ...
    for (std::uint32_t tick = 0; tick < control->tick_count; ++tick) {
        ...
        const double tick_time_s =
            context.source_time_s + static_cast<double>(tick) * control->interval_s;

        if (primary_request == nullptr) {
            record.decision = "skipped";
            record.decision_reason = "control cadence had no trigger or hold candidate";
            record.source = "none";
            runtime_window_append_cadence_trace(cadence_trace, std::move(record));
            continue;
        }
        ...
        if (!runtime_window_is_blank(primary_request->clock_domain_metadata.source_clock_domain)) {
            record.clock_domain = primary_request->clock_domain_metadata.source_clock_domain;
        }
        record.relation = primary_request->clock_domain_metadata.relation;

        if (!context.accepted_inputs.empty()) {
            record.clock_merge_policy =
                runtime_window_input_clock_merge_policy(*primary_request, control->merge_policy);
            if (tick == 0U) {
                record.decision = "triggered";
                record.decision_reason = "control cadence consumed the accepted policy trigger";
            } else if (hold_policy.hold_mode == kActionHoldModeHoldLast) {
                if (runtime_window_hold_candidate_expired_at_tick(
                        *primary_request, context.source_time_s, tick_time_s)) {
                    record.decision = "expired";
                    record.decision_reason =
            ...
            record.clock_merge_policy = "nested_slot";
            record.decision = "skipped";
            record.decision_reason = "control cadence had no visible trigger";
        }
...
inline void runtime_window_append_physics_cadence_trace(
    ...
    const RuntimeWindowCadence *physics =
        runtime_window_find_cadence_domain(cadence_config, kRuntimeWindowCadenceDomainPhysics);
    if (physics == nullptr) {
        return;
    }
    ...
    for (std::uint32_t tick = 0; tick < physics->tick_count; ++tick) {
        ...
        if (upstream_ready) {
            record.decision = "triggered";
            record.decision_reason = "physics cadence advanced inside the selected 100ms window";
            ...
            record.decision = "skipped";
            record.decision_reason = "physics cadence had no upstream launch/effects trigger";
            record.source = "none";
        }
...
inline void runtime_window_append_export_cadence_trace(
    const RuntimeWindowRequest &request, const RuntimeWindowCadenceConfig &cadence_config,
    const RuntimeWindowCoordinatorCallbacks &callbacks,
    std::vector<RuntimeWindowCadenceTraceRecord> *cadence_trace) {
    const RuntimeWindowCadence *export_cadence =
        runtime_window_find_cadence_domain(cadence_config, kRuntimeWindowCadenceDomainExport);
    ...
    if (!runtime_window_has_requested_export(request)) {
        record.decision = "skipped";
        record.decision_reason = "export cadence skipped because no facade export was requested";
        runtime_window_append_cadence_trace(cadence_trace, std::move(record));
        return;
    }
    ...
    record.decision = "triggered";
    record.decision_reason = "export cadence emitted the selected-slice facade packets";
    runtime_window_append_cadence_trace(cadence_trace, std::move(record));
}

inline std::vector<RuntimeWindowCadenceTraceRecord>
build_runtime_window_cadence_trace(const RuntimeWindowRequest &request,
                                   const RuntimeWindowSchedulingContext &context,
                                   const RuntimeWindowCadenceConfig &cadence_config,
                                   const RuntimeWindowCoordinatorCallbacks &callbacks) {
    std::vector<RuntimeWindowCadenceTraceRecord> cadence_trace;
    cadence_trace.reserve(16U);
    runtime_window_append_policy_cadence_trace(context, cadence_config, &cadence_trace);
    runtime_window_append_control_cadence_trace(context, cadence_config, &cadence_trace);
    runtime_window_append_physics_cadence_trace(context, cadence_config, &cadence_trace);
    runtime_window_append_export_cadence_trace(request, cadence_config, callbacks, &cadence_trace);
