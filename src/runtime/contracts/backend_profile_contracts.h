...
inline constexpr std::string_view kBackendProfileIdCpuExactReference = "cpu_exact.reference";
...
inline constexpr std::string_view kBackendProfileClassAcceleratedExact = "accelerated_exact";
...
inline constexpr std::string_view kBackendProfileClassDiagnosticsOnly = "diagnostics_only";
...
inline constexpr std::string_view kBackendProfileSyncPolicyObservationOnly = "observation-only";
inline constexpr std::string_view kBackendProfileSyncPolicyExportOnly = "export-only";
inline constexpr std::string_view kBackendProfileSyncPolicyUndeclaredBlocked = "undeclared_blocked";

inline constexpr std::string_view kBackendProfileRejectionReasonProfileIdNotFound =
    "backend_profile_id_not_found";

inline constexpr std::string_view kBackendProfileRejectionReasonMaintainedBaselineExactGpu =
    "backend_profile_maintained_exact_baseline_cannot_authorize_exact_gpu_support";
inline constexpr std::string_view kBackendProfileRejectionReasonDiagnosticsOnlyExactGpu =
    "backend_profile_diagnostics_only_cannot_authorize_exact_gpu_support";
inline constexpr std::string_view kBackendProfileRejectionReasonUnmaintainedCandidateExactGpu =
    "backend_profile_unmaintained_candidate_cannot_authorize_exact_gpu_support";

inline constexpr std::string_view kBackendProfileRejectionReasonMaintainedBaselineResidentState =
    "backend_profile_maintained_exact_baseline_cannot_authorize_resident_state_support";
inline constexpr std::string_view kBackendProfileRejectionReasonDiagnosticsOnlyResidentState =
    "backend_profile_diagnostics_only_cannot_authorize_resident_state_support";
inline constexpr std::string_view kBackendProfileRejectionReasonUnmaintainedCandidateResidentState =
    "backend_profile_unmaintained_candidate_cannot_authorize_resident_state_support";

inline constexpr std::string_view kBackendProfileRejectionReasonMaintainedBaselineShadowCompare =
    "backend_profile_maintained_exact_baseline_cannot_authorize_shadow_compare_support";
inline constexpr std::string_view kBackendProfileRejectionReasonDiagnosticsOnlyShadowCompare =
    "backend_profile_diagnostics_only_cannot_authorize_shadow_compare_support";
inline constexpr std::string_view kBackendProfileRejectionReasonUnmaintainedCandidateShadowCompare =
...
inline constexpr std::string_view
    kBackendProfileRejectionReasonMaintainedBaselineDeviceObservationView =
        "backend_profile_maintained_exact_baseline_cannot_authorize_device_observation_view_"
        "support";
inline constexpr std::string_view
...
[[nodiscard]] inline bool contains_value(const std::vector<std::string> &items,
...
[[nodiscard]] inline bool is_known_backend_profile_class(std::string_view value) {
    return value == kBackendProfileClassReference ||
           ...
           value == kBackendProfileClassResidentState || value == kBackendProfileClassApproximate ||
           value == kBackendProfileClassDiagnosticsOnly;
}

[[nodiscard]] inline bool is_known_backend_profile_maintained_status(std::string_view value) {
...
[[nodiscard]] inline bool is_maintained_backend_profile(const BackendProfileContract &profile) {
    return profile.maintained_status == kBackendProfileMaintainedStatusMaintainedExactBaseline;
}

[[nodiscard]] inline bool
backend_profile_authorizes_capability_claim(const BackendProfileContract &profile,
...
rejection_reason_for_backend_profile_capability(const BackendProfileContract &profile,
                                                BackendProfileCapabilityClaim claim) {
    const std::string_view maintained_status = profile.maintained_status;
    switch (claim) {
    case BackendProfileCapabilityClaim::exact_gpu_backend:
        if (maintained_status == kBackendProfileMaintainedStatusMaintainedExactBaseline) {
...
validate_backend_profile_contract(const BackendProfileContract &profile) {
    ...
    if (profile.projection_eligibility.maintained_cpu_exact_baseline &&
        ...
        result.add_error("maintained_cpu_exact_baseline may only be true for cpu_exact.reference");
    }
    if (profile.projection_eligibility.maintained_cpu_exact_baseline &&
        profile.maintained_status != kBackendProfileMaintainedStatusMaintainedExactBaseline) {
        result.add_error("maintained_cpu_exact_baseline requires maintained_exact_baseline status");
    }
    if (profile.projection_eligibility.maintained_cpu_exact_baseline &&
        profile.profile_class != kBackendProfileClassReference) {
        result.add_error("maintained_cpu_exact_baseline requires the reference profile class");
    }
    if (!is_maintained_backend_profile(profile) &&
        ...
        result.add_error("non-maintained profiles must not authorize "
                         "exact_gpu/resident_state/shadow/device_observation_view support");
    }
...
validate_maintained_backend_profile_contract(const BackendProfileContract &profile) {
    ...
    if (!profile.projection_eligibility.maintained_cpu_exact_baseline) {
        result.add_error("maintained backend profile must declare maintained_cpu_exact_baseline");
    }
    return result;
}

[[nodiscard]] inline BackendProfileValidationResult
validate_backend_profile_registry(const std::vector<BackendProfileContract> &registry) {
    BackendProfileValidationResult result{};
...
[[nodiscard]] inline const std::vector<BackendProfileContract> &backend_profile_registry_seed() {
    static const std::vector<BackendProfileContract> registry = {
        BackendProfileContract{
            ...
            .host_state_owner = "Host owns committed scheduler state, world state, observation "
                                "envelopes, and diagnostics ancestry.",
            .backend_state_owner = "None for maintained truth.",
            .sync_policy = std::string(kBackendProfileSyncPolicyHostOwned),
            .state_scope = "Maintained CPU exact execution, event order, snapshots, observations, "
                           "and diagnostics ancestry exposed through facade contracts.",
            .parity_budget_ref = "parity_budget.cpu_exact.reference.v1",
            .observability_scope = "Maintained facade outputs and structured diagnostics ancestry; "
                                   "diagnostics prose remains diagnostics-only.",
            .compatibility_rule =
                "Default fallback and comparison anchor for other profiles; no accelerated, "
                "resident-state, shadow, or device-observation support is implied.",
            .deprecation_rule =
                "Deprecate only through a replacement reference profile that preserves scheduler "
                "event order, snapshot identity, and validation obligations.",
            .validation_gate = "WP6-A registry review plus WP6-B reference budget acceptance; no "
                               "GPU/resident/shadow promotion implied.",
            .maintained_status =
            ...
            .source_doc_provenance =
                BackendProfileSourceProvenance{
                    ...
                    .accepted_by = "docs/task/review/archive/wp-acceptance/"
                                   "wp6_backend_profile_policy_acceptance_review_20260519.md",
                },
        },
        BackendProfileContract{
            .backend_profile_id = std::string(kBackendProfileIdGpuHelpersDiagnosticsOnly),
            ...
            .backend_state_owner = "GPU/helper-local diagnostics buffers or probes only when "
                                   "labeled diagnostics-only.",
            .sync_policy = std::string(kBackendProfileSyncPolicyExportOnly),
            .state_scope = "GPU availability checks, helper traces, probe outputs, or debug "
                           "artifacts that do not affect committed state.",
            .parity_budget_ref = "parity_budget.gpu_helpers.diagnostics_only.v1",
            .observability_scope = "Diagnostics traces, probe summaries, build/runtime "
                                   "availability facts; never maintained state.",
            .compatibility_rule =
                "Probeable deployment facts may be reported, but exact GPU, resident-state, "
                "shadow, and device-observation-view support stay false.",
            .deprecation_rule = "Remove or narrow if a helper starts influencing committed state; "
                                "promote only through a separate maintained profile.",
            .validation_gate = "Diagnostics labeling review; tests may assert report-only behavior "
                               "but cannot accept it as maintained parity.",
            .maintained_status = std::string(kBackendProfileMaintainedStatusDiagnosticsOnly),
            .projection_eligibility =
                BackendProfileProjectionEligibility{
            ...
            .source_doc_provenance =
                BackendProfileSourceProvenance{
                    .path = "docs/task/simulation_architecture/archive/wp6_backend_profile_policy/"
                            "wp6_backend_profile_registry_20260519.md",
                    .section = "3. Initial Registry",
                    .row_label = std::string(kBackendProfileIdGpuHelpersDiagnosticsOnly),
                    .accepted_by = "docs/task/review/archive/wp-acceptance/"
                                   "wp6_backend_profile_policy_acceptance_review_20260519.md",
                },
        },
        BackendProfileContract{
            .backend_profile_id = std::string(kBackendProfileIdGpuExactUnmaintainedCandidate),
            .profile_class = std::string(kBackendProfileClassAcceleratedExact),
            .comparison_reference = std::string(kBackendProfileIdCpuExactReference),
            .host_state_owner = "Host ownership of committed state is assumed until a maintained "
                                "profile states otherwise.",
            .backend_state_owner = "No backend-owned maintained state declared; GPU execution "
                                   "internals are not authoritative.",
            .sync_policy = std::string(kBackendProfileSyncPolicyUndeclaredBlocked),
            .state_scope = "Placeholder for a possible exact GPU world-step or accelerated exact "
                           "path; no maintained exact GPU support is claimed.",
            .parity_budget_ref = "parity_budget.gpu_exact.unmaintained_candidate.v1",
            .observability_scope = "Candidate diagnostics only, such as mismatch evidence or "
                                   "performance notes, when explicitly labeled.",
            .compatibility_rule = "Deployment facts may explain GPU helper availability, but "
                                  "maintained exact GPU support must remain false.",
            .deprecation_rule = "Delete if no exact promotion plan remains; replace only after "
                                "ownership, sync, parity, and validation gates pass.",
            .validation_gate =
                "Blocked until exact event order, snapshot identity, ownership split, sync "
                "barriers, parity budget, and replay/validation gates are accepted.",
            .maintained_status = std::string(kBackendProfileMaintainedStatusUnmaintainedCandidate),
            .projection_eligibility =
                BackendProfileProjectionEligibility{
                    .maintained_cpu_exact_baseline = false,
                    .exact_gpu_supported = false,
            ...
            .source_doc_provenance =
                BackendProfileSourceProvenance{
                    .path = "docs/task/simulation_architecture/archive/wp6_backend_profile_policy/"
                            "wp6_backend_profile_registry_20260519.md",
                    .section = "3. Initial Registry",
                    .row_label = std::string(kBackendProfileIdGpuExactUnmaintainedCandidate),
                    .accepted_by = "docs/task/review/archive/wp-acceptance/"
                                   "wp6_backend_profile_policy_acceptance_review_20260519.md",
                },
        },
        BackendProfileContract{
            .backend_profile_id = std::string(kBackendProfileIdResidentStateUnmaintainedCandidate),
            .profile_class = std::string(kBackendProfileClassResidentState),
            .comparison_reference = std::string(kBackendProfileIdCpuExactReference),
            .host_state_owner = "Host remains owner of maintained committed state until a profile "
                                "declares host-visible reconstruction or export rules.",
            .backend_state_owner =
                "Candidate backend-resident operational shards are not maintained truth.",
            .sync_policy = std::string(kBackendProfileSyncPolicyUndeclaredBlocked),
            .state_scope = "Placeholder for backend-resident observation, physics, or operational "
                           "state; no maintained resident-state support is claimed.",
            .parity_budget_ref = "parity_budget.resident_state.unmaintained_candidate.v1",
            .observability_scope = "Candidate diagnostics only; unsynced backend-local state must "
                                   "stay outside maintained parity.",
            .compatibility_rule = "Probeable backend presence does not imply resident-state "
                                  "ownership or device-observation-view support.",
            .deprecation_rule = "Remove or split if the resident scope cannot be reconstructed, "
                                "exported, or synchronized under resident-state boundary rules.",
            .validation_gate =
                "Blocked until ownership split, sync cadence/trigger, barriers, host-visible "
                "reconstruction/export, parity budget, and validation gates are accepted.",
            .maintained_status = std::string(kBackendProfileMaintainedStatusUnmaintainedCandidate),
            .projection_eligibility =
                BackendProfileProjectionEligibility{
                    .maintained_cpu_exact_baseline = false,
            ...
            .source_doc_provenance =
                BackendProfileSourceProvenance{
                    .path = "docs/task/simulation_architecture/archive/wp6_backend_profile_policy/"
                            "wp6_backend_profile_registry_20260519.md",
                    .section = "3. Initial Registry",
                    .row_label = std::string(kBackendProfileIdResidentStateUnmaintainedCandidate),
                    .accepted_by = "docs/task/review/archive/wp-acceptance/"
                                   "wp6_backend_profile_policy_acceptance_review_20260519.md",
                },
        },
        BackendProfileContract{
            .backend_profile_id = std::string(kBackendProfileIdShadowCompareUnmaintainedCandidate),
            .profile_class = std::string(kBackendProfileClassDiagnosticsOnly),
            .comparison_reference = std::string(kBackendProfileIdCpuExactReference),
            ...
            .sync_policy = std::string(kBackendProfileSyncPolicyExportOnly),
            .state_scope = "Placeholder for shadow comparison reports or offline A/B evidence; no "
                           "maintained shadow support is claimed.",
            .parity_budget_ref = "parity_budget.shadow_compare.unmaintained_candidate.v1",
            .observability_scope = "Comparison reports, mismatch summaries, and replay evidence "
                                   "labeled diagnostics-only.",
            .compatibility_rule = "Shadow output cannot affect committed state, fallback control "
                                  "flow, or maintained support.",
            .deprecation_rule =
                "Remove if reports are not used; promote only by defining shadow scope, "
                "non-interference, diagnostics separation, and validation evidence.",
            .validation_gate =
                "Blocked until shadow scope, non-interference rule, diagnostics separation, parity "
                "budget if maintained, and validation review are accepted.",
            .maintained_status = std::string(kBackendProfileMaintainedStatusUnmaintainedCandidate),
            .projection_eligibility =
                BackendProfileProjectionEligibility{
                    .maintained_cpu_exact_baseline = false,
            ...
            .source_doc_provenance =
                BackendProfileSourceProvenance{
                    .path = "docs/task/simulation_architecture/archive/wp6_backend_profile_policy/"
                            "wp6_backend_profile_registry_20260519.md",
                    .section = "3. Initial Registry",
                    .row_label = std::string(kBackendProfileIdShadowCompareUnmaintainedCandidate),
                    .accepted_by = "docs/task/review/archive/wp-acceptance/"
                                   "wp6_backend_profile_policy_acceptance_review_20260519.md",
                },
        },
...
[[nodiscard]] inline const BackendProfileContract *
find_backend_profile_contract(std::string_view backend_profile_id) {
...
validate_backend_profile_for_capability(const BackendProfileContract &profile,
                                        BackendProfileCapabilityClaim claim) {
    ...
    if (!result.allowed) {
        result.rejection_reason =
            std::string(rejection_reason_for_backend_profile_capability(profile, claim));
    }
    return result;
}

[[nodiscard]] inline BackendProfileCapabilityGateResult
validate_backend_profile_for_capability(std::string_view backend_profile_id,
                                        BackendProfileCapabilityClaim claim) {
    const BackendProfileContract *profile = find_backend_profile_contract(backend_profile_id);
    if (profile == nullptr) {
        BackendProfileCapabilityGateResult result{};
        result.backend_profile_id = std::string(backend_profile_id);
        result.allowed = false;
        result.rejection_reason = std::string(kBackendProfileRejectionReasonProfileIdNotFound);
        return result;
    }
...
validate_backend_profile_for_exact_gpu_support(std::string_view backend_profile_id) {
    return validate_backend_profile_for_capability(
        backend_profile_id, BackendProfileCapabilityClaim::exact_gpu_backend);
}

[[nodiscard]] inline BackendProfileCapabilityGateResult
validate_backend_profile_for_resident_state_support(std::string_view backend_profile_id) {
    return validate_backend_profile_for_capability(backend_profile_id,
                                                   BackendProfileCapabilityClaim::resident_state);
}

[[nodiscard]] inline BackendProfileCapabilityGateResult
validate_backend_profile_for_shadow_compare_support(std::string_view backend_profile_id) {
    return validate_backend_profile_for_capability(backend_profile_id,
                                                   BackendProfileCapabilityClaim::shadow_compare);
}

[[nodiscard]] inline BackendProfileCapabilityGateResult
validate_backend_profile_for_device_observation_view_support(std::string_view backend_profile_id) {
    return validate_backend_profile_for_capability(
        backend_profile_id, BackendProfileCapabilityClaim::device_observation_view);
}
