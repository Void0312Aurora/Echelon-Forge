...
inline constexpr std::string_view kFidelityProfileLabelExactEvaluation = "exact_evaluation";
inline constexpr std::string_view kFidelityProfileLabelFastTraining = "fast_training";
inline constexpr std::string_view kFidelityProfileLabelSensorHeavy = "sensor_heavy";
inline constexpr std::string_view kFidelityProfileLabelWeaponEffectsHeavy = "weapon_effects_heavy";
inline constexpr std::string_view kFidelityProfileLabelLargeScaleSwarm = "large_scale_swarm";
...
[[nodiscard]] inline FidelityProfileRequest make_exact_evaluation_cpu_reference_request() {
    return FidelityProfileRequest{
        ...
        .validation_gate = "exact-evaluation baseline admission; no accelerated support implied",
...
[[nodiscard]] inline FidelityProfileAdmissionResult
reject_fidelity_request(const FidelityProfileRequest &request, std::string_view reason,
                        std::string error) {
...
[[nodiscard]] inline FidelityProfileAdmissionResult
admit_fidelity_profile_request(const FidelityProfileRequest &request) {
    if (is_blank(request.request_label)) {
        return reject_fidelity_request(request, kFidelityProfileRejectionMissingLabel,
                                       "request_label is required");
    }
    if (!is_known_fidelity_profile_label(request.request_label)) {
        return reject_fidelity_request(request, kFidelityProfileRejectionUnsupportedLabel,
    ...
    if (request.requests_adaptive_scheduling) {
        return reject_fidelity_request(
            request, kFidelityProfileRejectionAdaptiveScheduling,
            "adaptive fidelity scheduling is outside the exact-evaluation baseline");
    }
    if (request.requests_learned_model_provider) {
        return reject_fidelity_request(
            request, kFidelityProfileRejectionLearnedProvider,
            "learned ModelProvider runtime is outside the exact-evaluation baseline");
    }
    if (request.requests_approximate_execution) {
        return reject_fidelity_request(request, kFidelityProfileRejectionApproximateExecution,
                                       "approximate fidelity execution is not maintained");
    }
    if (request.requests_exact_gpu_backend) {
        return reject_fidelity_request(request, kFidelityProfileRejectionExactGpu,
                                       "exact GPU fidelity requires a maintained backend profile");
    }
    if (request.requests_resident_state) {
        return reject_fidelity_request(
            request, kFidelityProfileRejectionResidentState,
            "resident-state fidelity requires a maintained backend profile");
    }
    if (request.requests_shadow_compare) {
        return reject_fidelity_request(
            request, kFidelityProfileRejectionShadowCompare,
            "shadow-backed fidelity requires a maintained backend profile");
    }
    if (is_blank(request.backend_profile_id)) {
        return reject_fidelity_request(request, kFidelityProfileRejectionMissingBackendProfile,
    ...
    if (profile == nullptr || !backend_profiles::is_maintained_backend_profile(*profile)) {
        return reject_fidelity_request(
            request, kFidelityProfileRejectionRequiresMaintainedBackendProfile,
    ...
    if (!profile_result.valid) {
        ...
        result.errors.insert(result.errors.end(), profile_result.errors.begin(),
                             profile_result.errors.end());
        return result;
    }

    if (is_blank(request.parity_budget_ref)) {
        return reject_fidelity_request(request, kFidelityProfileRejectionMissingBudget,
    ...
    const parity::ParityBudgetValidationResult budget_result =
        parity::validate_profile_owned_parity_budget(
            profile->backend_profile_id, profile->profile_class, request.parity_budget_ref);
    if (!budget_result.valid || !budget_result.accepted_for_maintained_use) {
        FidelityProfileAdmissionResult result = reject_fidelity_request(
            request, kFidelityProfileRejectionRequiresAcceptedBudget,
    ...
    if (request.model_family_scope.empty() || contains_blank_value(request.model_family_scope)) {
        return reject_fidelity_request(
            request, kFidelityProfileRejectionMissingModelScope,
            "model_family_scope is required and cannot contain blank entries");
    }
    if (is_blank(request.validation_gate)) {
        return reject_fidelity_request(request, kFidelityProfileRejectionMissingValidationGate,
                                       "validation_gate is required");
    }
    if (request.facade_evidence_refs.empty() ||
        contains_blank_value(request.facade_evidence_refs)) {
        return reject_fidelity_request(
            request, kFidelityProfileRejectionMissingFacadeEvidence,
    ...
    if (request.request_label != kFidelityProfileLabelExactEvaluation) {
        return reject_fidelity_request(
            request, kFidelityProfileRejectionUnsupportedLabel,
            "only exact_evaluation is admitted by the exact-evaluation baseline gate");
    }
