...
manifest_json(const contracts::SimulationCompositionManifest &manifest) {
    ...
    for (const auto *plugin : plugin_rows) {
        ...
        Json artifact = {
            ...
            {"sha256",
             plugin->artifact.sha256.has_value() ? Json(*plugin->artifact.sha256) : Json(nullptr)},
        };
        plugins.push_back({
    ...
    std::sort(binding_rows.begin(), binding_rows.end(), [](const auto *left, const auto *right) {
        return std::tie(left->consumer_kind, left->consumer_id, left->service_key,
                        left->provider_id) < std::tie(right->consumer_kind, right->consumer_id,
                                                      right->service_key, right->provider_id);
    });
    ...
    std::sort(scope_rows.begin(), scope_rows.end(), [](const auto *left, const auto *right) {
        return static_cast<std::uint8_t>(left->scope) < static_cast<std::uint8_t>(right->scope);
    });
    for (const auto *scope : scope_rows) {
        if (!contracts::is_valid_scope(scope->scope) ||
            (scope->parent_scope.has_value() && !contracts::is_valid_scope(*scope->parent_scope))) {
            return CompositionResult<Json>::failure({
                std::string(contracts::kErrorInvalidScopePolicy),
    ...
    Json result = {
        ...
        {"reconfiguration_policy",
         {
             {"active_episode_change", manifest.reconfiguration_policy.active_episode_change},
             {"allowed_barriers", sorted_strings(manifest.reconfiguration_policy.allowed_barriers)},
             {"truth_affecting_change", manifest.reconfiguration_policy.truth_affecting_change},
         }},
        {"requested_profile",
         {
