...
def compare_exact_world_step_aircraft_tail_slice(
    ...
    *,
    ...
    if loaded.get("trace_kind") != "cpu_exact_system_stage_trace_v1":
        raise ValueError(
            "compare_exact_world_step_aircraft_tail_slice expects trace_kind=cpu_exact_system_stage_trace_v1"
        )

    stage_records = list(loaded.get("stage_records", []))
    target_record = next((record for record in stage_records if record.get("stage_name") == "MassUpdate"), None)
    if target_record is None:
        raise ValueError("system trace does not contain a MassUpdate stage record")
    ...
    after_force_ground = bytes(ef_py.step_exact_world_step_force_ground_reference_cpu_packed(after_control_aero))
    stepped_packed = bytes(ef_py.step_exact_world_step_aircraft_tail_cuda_packed(after_force_ground, bool(use_gpu)))
    ...
    aircraft_tail_cuda_stats = ef_py.last_exact_world_step_aircraft_tail_cuda_stats()
    ...
    return {
        ...
        "target_stage_name": "MassUpdate",
        "world_count": int(loaded.get("world_count", 0)),
        "use_gpu_requested": bool(use_gpu),
        "used_cuda": bool(getattr(aircraft_tail_cuda_stats, "used_cuda", False)),
        ...
        "aircraft_tail_total_ms": float(getattr(aircraft_tail_stats, "total_ms", 0.0)),
        "aircraft_tail_cuda_state_count": int(getattr(aircraft_tail_cuda_stats, "state_count", 0)),
        "aircraft_tail_cuda_host_to_device_ms": float(getattr(aircraft_tail_cuda_stats, "host_to_device_ms", 0.0)),
        "aircraft_tail_cuda_kernel_ms": float(getattr(aircraft_tail_cuda_stats, "kernel_ms", 0.0)),
        "aircraft_tail_cuda_device_to_host_ms": float(getattr(aircraft_tail_cuda_stats, "device_to_host_ms", 0.0)),
        "aircraft_tail_cuda_cpu_fallback_ms": float(getattr(aircraft_tail_cuda_stats, "cpu_fallback_ms", 0.0)),
        "aircraft_tail_cuda_total_ms": float(getattr(aircraft_tail_cuda_stats, "total_ms", 0.0)),
    }
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replay the exact CPU aircraft-tail executor slice (chained after command-lane, control/aero, "
            "and force/ground slices) and compare it against the MassUpdate stage in a system trace."
        )
    )
    ...
    parser.add_argument(
        "--gpu",
        action="store_true",
        help="Run the aircraft-tail CUDA backend instead of the CPU reference wrapper.",
    )
...
def main() -> int:
    args = _parse_args()
    report = compare_exact_world_step_aircraft_tail_slice(
        args.trace,
        ...
        json.dumps(
            {
                ...
                "aircraft_tail_cuda_state_count": report["aircraft_tail_cuda_state_count"],
                "aircraft_tail_cuda_total_ms": report["aircraft_tail_cuda_total_ms"],
            },
            indent=2,
            sort_keys=True,
        )
