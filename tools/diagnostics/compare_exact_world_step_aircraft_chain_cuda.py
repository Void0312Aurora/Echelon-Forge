...
from tools.diagnostics.compare_exact_world_step_aircraft_tail_slice import (
...
def compare_exact_world_step_aircraft_chain_cuda(
    ...
    *,
    ...
    if loaded.get("trace_kind") != "cpu_exact_system_stage_trace_v1":
        raise ValueError(
            "compare_exact_world_step_aircraft_chain_cuda expects trace_kind=cpu_exact_system_stage_trace_v1"
        )
    ...
    stepped_packed = bytes(ef_py.step_exact_world_step_aircraft_chain_cuda_packed(initial_packed, bool(use_gpu)))

    chain_stats = ef_py.last_exact_world_step_aircraft_chain_cuda_stats()
    ...
    return {
        ...
        "aircraft_chain_state_count": int(getattr(chain_stats, "state_count", 0)),
        "aircraft_chain_command_lane_ms": float(getattr(chain_stats, "command_lane_ms", 0.0)),
        "aircraft_chain_host_to_device_ms": float(getattr(chain_stats, "host_to_device_ms", 0.0)),
        "aircraft_chain_kernel_ms": float(getattr(chain_stats, "kernel_ms", 0.0)),
        "aircraft_chain_device_to_host_ms": float(getattr(chain_stats, "device_to_host_ms", 0.0)),
        "aircraft_chain_cpu_post_command_ms": float(getattr(chain_stats, "cpu_post_command_ms", 0.0)),
        "aircraft_chain_total_ms": float(getattr(chain_stats, "total_ms", 0.0)),
    }
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replay the aircraft-only resident CUDA chain (CommandLane on CPU, FlightControl through "
            "MassUpdate on CUDA) against the archived MassUpdate stage in a system trace."
        )
    )
    ...
    parser.add_argument("--gpu", action="store_true", help="Run the resident CUDA chain instead of the CPU fallback.")
    return parser.parse_args()


def main() -> int:
    args = _parse_args()
    report = compare_exact_world_step_aircraft_chain_cuda(
        args.trace,
        ...
        json.dumps(
            {
                ...
                "aircraft_chain_state_count": report["aircraft_chain_state_count"],
                "aircraft_chain_total_ms": report["aircraft_chain_total_ms"],
            },
            indent=2,
            sort_keys=True,
        )
