...
def compare_exact_world_step_control_aero_slice(
    ...
    *,
    ...
    if loaded.get("trace_kind") != "cpu_exact_system_stage_trace_v1":
        raise ValueError(
            "compare_exact_world_step_control_aero_slice expects trace_kind=cpu_exact_system_stage_trace_v1"
        )

    stage_records = list(loaded.get("stage_records", []))
    target_record = next((record for record in stage_records if record.get("stage_name") == "ComputeAeroState"), None)
    if target_record is None:
        raise ValueError("system trace does not contain a ComputeAeroState stage record")
    ...
    stepped_packed = bytes(ef_py.step_exact_world_step_control_aero_reference_cpu_packed(after_command_lane))
    ...
    actual_record["stage_name"] = "ComputeAeroState"
    ...
    return {
        ...
        "target_stage_name": "ComputeAeroState",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replay the exact CPU control/aero executor slice (chained after the command-lane slice) "
            "and compare it against the ComputeAeroState stage in a system trace."
        )
    )
...
def main() -> int:
    args = _parse_args()
    report = compare_exact_world_step_control_aero_slice(
        args.trace,
        ...
        json.dumps(
            {
                ...
                "command_lane_state_count": report["command_lane_state_count"],
                "command_lane_total_ms": report["command_lane_total_ms"],
                "control_aero_state_count": report["control_aero_state_count"],
                "control_aero_total_ms": report["control_aero_total_ms"],
            },
            indent=2,
            sort_keys=True,
        )
