#!/usr/bin/env python3
"""
Cooperative takeoff-to-cruise bridge trajectory diagnostic for one world episode.

This script replays one cooperative bridge world and exports:
  - a PNG with both aircraft ground tracks, altitude/speed traces,
    takeoff-clearance timeline, and waypoint-progress timeline
  - a JSON payload with per-step traces and compact per-slot summaries

Use cases:
  - Verify that interval-takeoff semantics still work once the mission command
    already carries a cruise route
  - Inspect where both aircraft lose time before the first waypoint
  - Compare a trained shared-execution policy against the scripted residual-zero
    bridge baseline on the exact same cooperative scenario
"""
...
class SlotEpisodeSummary:
    ...
    first_waypoint_capture_step: int | None
    max_altitude_agl_m: float
    max_ground_speed_mps: float
    final_waypoint_index: float | None
    final_waypoint_count: float | None
    final_distance_to_active_waypoint_m: float | None
...
def _cooperative_action_wrapper_kwargs(train_config: dict[str, Any], *, scripted: bool) -> dict[str, Any] | None:
...
def _capture_slot_sample(
    ...
    *,
    ...
    distance_to_wp_m = None
    waypoint_index = None
    waypoint_count = None
    if len(mission_status) >= 1:
        distance_to_wp_m = float(mission_status[0])
    if len(mission_status) >= 2:
        waypoint_index = float(mission_status[1])
    if len(mission_status) >= 3:
        waypoint_count = float(mission_status[2])
...
def _build_slot_summary(slot_meta: dict[str, Any], samples: list[dict[str, Any]], final_info: dict[str, Any]) -> SlotEpisodeSummary:
    ...
    max_altitude_agl_m = float(np.nanmax(np.asarray([sample["altitude_agl_m"] for sample in samples], dtype=np.float64)))
    max_ground_speed_mps = float(np.nanmax(np.asarray([sample["ground_speed_mps"] for sample in samples], dtype=np.float64)))
    ...
    final_distance_to_wp_m = None if len(mission_status) < 1 else float(mission_status[0])
    return SlotEpisodeSummary(
        ...
        first_waypoint_capture_step=_first_step(samples, lambda sample: float(sample.get("waypoint_index", 0.0) or 0.0) >= 1.0),
        max_altitude_agl_m=float(max_altitude_agl_m),
        max_ground_speed_mps=float(max_ground_speed_mps),
        final_waypoint_index=final_waypoint_index,
        final_waypoint_count=final_waypoint_count,
        final_distance_to_active_waypoint_m=final_distance_to_wp_m,
...
def _plot_waypoint_trace(ax, traces: list[list[dict[str, Any]]], slot_meta: list[dict[str, Any]]) -> None:
    for slot_index, samples in enumerate(traces):
        ...
        t = np.asarray([sample["sim_time_s"] for sample in samples], dtype=np.float64)
        idx = np.asarray(
            [0.0 if sample["waypoint_index"] is None else float(sample["waypoint_index"]) for sample in samples],
            dtype=np.float64,
        )
        dist = np.asarray(
            [
                np.nan if sample["distance_to_active_waypoint_m"] is None else float(sample["distance_to_active_waypoint_m"])
                for sample in samples
            ],
            dtype=np.float64,
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Export one cooperative takeoff-to-cruise bridge trajectory diagnostic episode.")
    ...
    try:
        slot_meta, traces, final_infos, terminated = _run_episode(
            env,
            model,
        ...
        summary = WorldEpisodeSummary(
            ...
            trace_note=(
                ...
                if terminated
                else "Trace stopped at max_world_steps before termination; use a higher limit for full-episode export."
            ),
