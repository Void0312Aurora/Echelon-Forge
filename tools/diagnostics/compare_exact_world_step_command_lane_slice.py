...
def compare_exact_world_step_command_lane_slice(
    ...
    *,
    ...
    if loaded.get("trace_kind") != "cpu_exact_system_stage_trace_v1":
        raise ValueError(
            "compare_exact_world_step_command_lane_slice expects trace_kind=cpu_exact_system_stage_trace_v1"
        )

    stage_records = list(loaded.get("stage_records", []))
    target_record = next((record for record in stage_records if record.get("stage_name") == "CommandLag"), None)
    if target_record is None:
        raise ValueError("system trace does not contain a CommandLag stage record")
    ...
    stepped_packed = bytes(ef_py.step_exact_world_step_command_lane_reference_cpu_packed(initial_packed))
    stats = ef_py.last_exact_world_step_command_lane_stats()
    ...
    actual_record["stage_name"] = "CommandLag"
    ...
    return {
        ...
        "command_lane_state_count": int(getattr(stats, "state_count", 0)),
        "command_lane_total_ms": float(getattr(stats, "total_ms", 0.0)),
    }


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Replay the exact CPU command-lane executor slice and compare it against the CommandLag stage in a system trace."
    )
...
def main() -> int:
    args = _parse_args()
    report = compare_exact_world_step_command_lane_slice(
        args.trace,
