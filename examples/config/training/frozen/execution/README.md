...
- The maintained `p5` configs now default to the GPU-assisted world-batch path:
  `runtime.world_batch_vec_env=true`, `batch_observation_backend=compiled`,
  `batch_visual_backend=gpu_host`, and `env.execution_step_runtime_mode=compiled`.
- `batch_visual_backend=gpu_host` stays enabled by default because the maintained world-batch visual helper already has a real rollout consumer and can produce a measurable wall-clock gain without changing exact world-step semantics.
- `batch_observation_backend` intentionally stays on `compiled` instead of `gpu_host` because the learner-facing path still consumes host NumPy tensors; until a device-resident consumer exists, GPU observation packing is not the strongest maintained default.
- Expected build/runtime matrix for the maintained `p5` configs:
  - CPU-only build:
    `world_batch_vec_env` stays available, and the visual/observation helpers fall back to maintained CPU paths.
  - CUDA build without an available runtime device:
    the same config remains launchable; the helper path still preserves CPU fallback semantics.
  - CUDA build with an available runtime device:
    `batch_visual_backend=gpu_host` can use the maintained CUDA visual helper, while exact world stepping remains on the CPU simulation core.
