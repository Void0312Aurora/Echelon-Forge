...
def test_g0_j_static_environment_fixture_validates_deterministically() -> None:
    ...
    branch_ids = {branch["branch_id"] for branch in fixture_a.to_metadata()["branch_registry"]}
...
def test_g0_j_manifest_requires_the_shared_environment_branch_registry() -> None:
    ...
    metadata["branch_registry"] = [
        branch for branch in metadata["branch_registry"] if branch["branch_id"] != "hydrology"
    ]
...
def test_g0_j_manifest_rejects_missing_required_component_attributes() -> None:
    ...
    assert validation.fail_closed
    assert (
        validation.rejection_reason == "environment_substrate_missing_required_component_attribute"
    )


def test_g0_j_required_component_attributes_accept_false_and_zero_only_when_present() -> None:
    metadata = build_deterministic_environment_fixture().to_metadata()
    metadata["objects"][0]["components"].extend(
        (
            {
                "component_id": "component:test-held-anchor",
            ...
            {
                "component_id": "component:test-zero-confidence",
                "family": "terrain_morphology",
                "schema_version": "1",
    ...
    assert validate_environment_manifest(_manifest_from_metadata(metadata)).valid

    for missing_value in (None, "", "   "):
        invalid = deepcopy(metadata)
        invalid["objects"][0]["components"][-2]["attributes"]["source"] = missing_value
        validation = validate_environment_manifest(_manifest_from_metadata(invalid))
        assert not validation.valid
        assert (
...
def test_g0_k_catalog_template_attributes_accept_false_and_zero_only_when_present() -> None:
    def catalog_with_source(source: object) -> EnvironmentCatalogDescriptor:
        return EnvironmentCatalogDescriptor(
            catalog_id="catalog:presence-regression",
            schema_version="1",
            branch_membership=(
            ...
            required_components=("elevation_anchor", "terrain_morphology"),
            component_templates=(
                EnvironmentComponent(
                    component_id="component-template:held-anchor",
                    family="elevation_anchor",
                    attributes={
                        "mode": "elevated_profile",
                        "resolved": False,
                ...
                EnvironmentComponent(
                    component_id="component-template:zero-confidence",
                    family="terrain_morphology",
                    attributes={"confidence": 0},
                ),
            ),
    ...
    assert validate_environment_catalog_descriptors(
        (catalog_with_source("contract-regression"),)
    ).valid

    for missing_value in (None, "", "   "):
        validation = validate_environment_catalog_descriptors((catalog_with_source(missing_value),))
        assert not validation.valid
        assert (
            validation.rejection_reason
...
def test_g0_k_request_rejects_catalog_outside_branch_scope() -> None:
    ...
    assert not validation.valid
    assert validation.fail_closed
    assert validation.rejection_reason == "environment_substrate_generator_branch_scope_mismatch"
...
def test_g0_k_catalog_descriptors_validate_and_reject_feature_schema_roots() -> None:
    ...
    assert rejected.rejection_reason == "environment_substrate_catalog_feature_schema_root_rejected"
...
def test_g0_k_catalog_admission_rejects_branch_mismatch() -> None:
    ...
    assert not validation.valid
    assert validation.fail_closed
    assert validation.rejection_reason == "environment_substrate_catalog_branch_layer_mismatch"
...
def test_g0_k_generated_manifest_is_canonical_and_deterministic() -> None:
    ...
    for item in manifest_a.objects:
        assert item.provenance["catalog_id"] == item.catalog_ref
...
def test_g0_k_catalog_admission_rejects_missing_required_provenance() -> None:
    catalogs = default_environment_catalog_descriptors()
    metadata = build_deterministic_generated_environment_manifest(
        _request(), catalogs
    ).to_metadata()
    del metadata["objects"][0]["provenance"]["catalog_id"]

    validation = validate_environment_catalog_admission(EnvironmentManifest(**metadata), catalogs)

    assert not validation.valid
    assert validation.fail_closed
    assert validation.rejection_reason == "environment_substrate_catalog_provenance_required"


def test_g0_k_catalog_admission_rejects_mismatched_provenance() -> None:
    catalogs = default_environment_catalog_descriptors()
    metadata = build_deterministic_generated_environment_manifest(
        _request(), catalogs
    ).to_metadata()
    metadata["objects"][0]["provenance"]["catalog_schema_version"] = "wrong"

    validation = validate_environment_catalog_admission(EnvironmentManifest(**metadata), catalogs)

    assert not validation.valid
    assert validation.fail_closed
    assert validation.rejection_reason == "environment_substrate_catalog_provenance_mismatch"
...
def test_g0_k_catalog_rejects_missing_required_component_template() -> None:
    invalid_catalog = EnvironmentCatalogDescriptor(
        ...
        schema_version="1",
        branch_membership=(EnvironmentBranchMembership(branch_id="terrain", role="projectable"),),
    ...
    assert not validation.valid
    assert validation.fail_closed
    assert validation.rejection_reason == "environment_substrate_catalog_required_component_missing"
...
def test_g0_m_surface_zone_index_requires_projection_profile() -> None:
    ...
    assert result.fail_closed
    assert result.bundle is None
    assert result.rejection_reason == "environment_substrate_derived_projection_profile_required"
