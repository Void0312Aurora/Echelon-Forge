...
from tools.diagnostics.compare_exact_world_step_aircraft_tail_slice import (  # noqa: E402
    compare_exact_world_step_aircraft_tail_slice,
)
...
def _single_world_runtime():
    ...
    runtime.reset_batch([43])
    runtime.set_time_step(0.05)
    return runtime


def _setup_aircraft_tail_world():
    ...
    world.set_mission_command(entity_id, _default_command(0))
    world.set_action(entity_id, 0.35, -0.25, 0.18, 0.0, False, False, False)
    world.set_action_space_config(entity_id, 18.0, 9.0, 34.0, 160.0, 330.0, 500.0, 3400.0)
    world.set_command_lag(entity_id, 0.45, 0.60, 0.85)
    pilot = ef_py.PilotAction()
    pilot.active = True
    pilot.stick_roll = 0.18
    pilot.stick_pitch = -0.11
    pilot.rudder = 0.27
    pilot.throttle = 0.82
    pilot.gear_handle = 1.0
    pilot.flaps = 0.2
    pilot.speedbrake = 0.1
...
class ExactWorldStepAircraftTailSliceTests(unittest.TestCase):
    def test_aircraft_tail_slice_matches_massupdate_stage_in_system_trace(self) -> None:
        trace = generate_cpu_exact_world_step_system_trace(seeds=[11, 17], time_step_s=0.05)
        report = compare_exact_world_step_aircraft_tail_slice(trace, use_gpu=False, abs_tol=1e-6, max_examples=4)
    ...
    def test_aircraft_tail_cuda_backend_runs_against_massupdate_stage(self) -> None:
        trace = generate_cpu_exact_world_step_system_trace(seeds=[11, 17], time_step_s=0.05)
        report = compare_exact_world_step_aircraft_tail_slice(trace, use_gpu=True, abs_tol=1e-6, max_examples=4)

        self.assertEqual(report["trace_kind"], "cpu_exact_system_stage_trace_v1")
        self.assertEqual(report["target_stage_name"], "MassUpdate")
        self.assertEqual(int(report["aircraft_tail_cuda_state_count"]), 2)
        ...
        if bool(info.cuda_runtime_available):
            ...
            self.assertGreaterEqual(float(report["aircraft_tail_cuda_kernel_ms"]), 0.0)
        else:
    ...
    def test_aircraft_tail_slice_matches_live_manual_pipeline(self) -> None:
        runtime_live, ref_live = _setup_aircraft_tail_world()
        runtime_exec, ref_exec = _setup_aircraft_tail_world()
        ...
        packed_after_force_ground = bytes(
            ef_py.step_exact_world_step_force_ground_reference_cpu_packed(packed_after_control)
        )
        packed_exec = bytes(ef_py.step_exact_world_step_aircraft_tail_reference_cpu_packed(packed_after_force_ground))
