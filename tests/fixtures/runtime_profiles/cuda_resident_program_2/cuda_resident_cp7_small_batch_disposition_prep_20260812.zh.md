...
1. **每窗口一次同步 + 合并状态数组。** 给每个阶段一个独立的设备端状态字
   ...
   **已作为 CP-7b 落地（2026-08-12），形态经更正。** 设计评审发现推迟检查
   的形态不成立：两个状态槽、三个阶段之下，推迟全部检查会摧毁 inject 阶段
   失败的回滚点（下一阶段的暂存拷贝会覆盖唯一干净的槽）。落地形态改为把
   stage_publish 与 window_commit 屏障折入各自阶段 kernel 作逐 world 尾声：
   每窗 launch、同步、状态回读、memset 各 5→3，且每个阶段的宿主检查、翻转、
   重试契约与故障钩子的可观测行为全部保持。实测：world-1 warmed e2e p50 降
   20-30%，全部 30 行导出状态摘要与冻结 CR2-6b 逐位一致；证据世代 v4 记录
   launch 吸收。候选 1 的残余切片（把输入注入阶段并入同一纪律）与 mapped
   pinned 状态字留给未来迭代，视 CP-8 是否仍需要。
...
5. **改为冻结显式选择规则。** 把 CR2-6b 的建议（world 1 走 CPU）升级为
   ...
   它同样满足门禁，因为退化不再是静默的。
   **已作为 CP-7a 落地（2026-08-12）：** 性能契约中的
   `cp7.small_batch_selection_rule.v1` 把低于 4 的 world 数冻结为路由 CPU
   参考，文档级策略（架构门禁强制零运行时消费者），交叉点数值列为 CP-8
   复核项。上文的 world-1 时间线归因把它从「路由偏好」升级为 world 1 的
   唯一诚实处置：单线程设备地板（约 65.5 us）在计入任何宿主成本之前就已
   超过 CPU 整步（约 18-31 us）。所有者选定「规则 + 候选 1」作为 CP-7
   处置；候选 1 作为 CP-7b 继续。
