...
| CP-9 | Promotion decision: all gates + independent review, or a recorded hold with the exact missing authority | **DECIDED 2026-08-13: scoped promotion.** All six gates verified green; independent review (separate context, zero implementation edits, zero blocking findings) recommended `scoped_promote` and the repository owner chose it. The resident backend is promoted to a selectable, explicit opt-in maintained backend on the fixture surface with advisory minimum 4 worlds; CPU stays the maintained default everywhere; correctness claims are maintained-grade, performance claims stay host-specific experimental. No runtime behavior changes until the authorized exposure scope lands. Record: [cuda_resident_cp9_promotion_decision_20260813.md](cuda_resident_cp9_promotion_decision_20260813.md) |
