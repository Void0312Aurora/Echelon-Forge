...
第二轮评审(同日)在上述机制中发现四处缺口,均已关闭:surface↔flag 门禁
改为解析 configure 步骤的 run 命令本体(注释无法满足它,并有变异门禁佐证);
learner 报告校验器按包声明的世代分发(每个注册世代恰好对应一个 schema id,
重贴标签的报告无法混入声明其他世代的包);CP-6 包钉定捕获时源出处
(`source_commit` 加该 commit 下学习者契约、消费者实现、会话与探针的规范
哈希,循 CP-8 manifest 先例);计数器收集器对 v3+ 捕获出具运行时提权记录,
并有生成器-校验器往返门禁,v4 采集不再会被自己的校验器拒收。
