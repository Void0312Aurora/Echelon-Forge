...
from tools.maintenance import (  # noqa: E402
    a2_blastfrag_res002_scoped_release_identity_gate as gate,
)


def test_a2_blastfrag_res002_scoped_release_identity_gate_passes_scoped_surface() -> None:
    artifact = gate.generate_res002_scoped_release_identity_gate(repo_root=REPO_ROOT)
    ...
    assert gate.TEMP_ANCHOR not in json.dumps(artifact, sort_keys=True)
    ...
    assert len(artifact["relevant_file_hash_inventory"]) == len(gate.DOC_REFS)
...
def test_a2_blastfrag_res002_scoped_release_identity_gate_fails_closed_when_global_clean_required() -> None:
    artifact = gate.generate_res002_scoped_release_identity_gate(
        repo_root=REPO_ROOT,
...
def test_a2_blastfrag_res002_scoped_release_identity_gate_writes_retained_bundle(
    tmp_path: Path,
) -> None:
    manifest = gate.write_retained_scoped_identity_artifact(
        repo_root=REPO_ROOT,
        retained_output_dir=tmp_path,
    )

    artifact_path = tmp_path / gate.GATE_FILENAME
    ...
    assert manifest["relevant_file_hash_count"] == len(gate.DOC_REFS)
    assert manifest["temporary_anchor_scan"]["scoped_surface_anchor_count"] == 0
    assert manifest["artifacts"][0]["sha256"] == gate._sha256_file(artifact_path)
    assert not any(manifest["authority_guards"].values())


def test_a2_blastfrag_res002_scoped_release_identity_gate_cli_default_writes_manifest(
    tmp_path: Path,
) -> None:
    ...
    assert (tmp_path / gate.GATE_FILENAME).exists()
    assert (tmp_path / gate.MANIFEST_FILENAME).exists()
