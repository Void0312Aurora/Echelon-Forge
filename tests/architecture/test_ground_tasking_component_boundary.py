...
def test_ground_tasking_headers_declare_narrow_g0_g1_owner_slices() -> None:
    ...
    assert "enum class GroundTaskGrade" in enum_text
    assert "TaskingStatus = 1" in enum_text
    assert "StaticOccupySupport = 2" in enum_text
    assert "enum class GroundTaskKind" in enum_text
    assert "Move = 1" in enum_text
    assert "Occupy = 2" in enum_text
    assert "Support = 3" in enum_text
    assert "enum class GroundSchemaBoundary" in enum_text
    assert "CompatibilityShell = 1" in enum_text
    assert "NativeStaticSchema = 2" in enum_text

    for text, owner in (
        (task_text, "TaskOrderGround"),
        (intent_text, "LeaderIntentGround"),
        (report_text, "PilotReportGround"),
    ):
        assert f"struct {owner}" in text
        assert f"using {owner}OwnerSlice = {owner};" in text
        assert f"k{owner}OwnedDomainSlice = true" in text
        assert "StaticStatusDirective" in text
        assert "ground_task_grade" in text
        assert "ground_task_kind" in text
        assert "ground_schema_boundary" in text
        assert "ground_objective_area_id" in text
        assert "ground_movement_released = false" in text
        assert "ground_observation_export_released = false" in text
        assert "ground_fires_released = false" in text
...
def test_tasking_compatibility_shells_project_ground_owner_slice() -> None:
    ...
    assert "task_order_ground_static_status_directive(" in task_text
    assert "order.supported_node_id" in task_text
    assert "order.supporting_node_id" in task_text
...
def test_ground_tasking_boundary_docs_keep_packet_and_runtime_surfaces_held() -> None:
    ...
    for required in (
        "formal C++ component boundary",
        "no maintained movement",
        "observation/action packet boundary",
        "below a G2 movement release",
        "must not depend on",
    ):
    ...
    for held_surface in (
        ...
        assert held_surface in readme

    assert "src/components/tasking/ground/" in progress
    assert "tasking/status owner slices" in progress
