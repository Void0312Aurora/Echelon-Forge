...
from tools.maintenance import (  # noqa: E402
    a2_blastfrag_package_provenance_identity_gate as gate,
)


def test_a2_blastfrag_package_provenance_identity_gate_is_blocked() -> None:
    artifact = gate.generate_package_provenance_identity_gate(repo_root=REPO_ROOT)
...
def test_a2_blastfrag_package_provenance_identity_gate_fails_closed_for_optimistic_release_fields(
    monkeypatch,
) -> None:
...
def test_a2_blastfrag_package_provenance_identity_gate_fails_closed_on_placeholder_hits(
    monkeypatch,
) -> None:
    original_read_text = gate._read_text
    ...
    def placeholder_read_text(path: Path) -> str:
        text = original_read_text(path)
        if path == gate.DOC_REFS["validation_provenance_identity_gate"]:
            return f"{text}\n<待填>\n"
        return text

    monkeypatch.setattr(gate, "_read_text", placeholder_read_text)
...
def test_a2_blastfrag_package_provenance_identity_gate_cli_writes_json(
    tmp_path: Path,
) -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            "tools/maintenance/a2_blastfrag_package_provenance_identity_gate.py",
            "--output",
            str(output_path),
        ],
        cwd=REPO_ROOT,
