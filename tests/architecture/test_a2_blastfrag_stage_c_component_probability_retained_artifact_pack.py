...
def test_a2_blastfrag_stage_c_component_probability_retained_artifact_pack_writes_retained_files(
    tmp_path: Path,
) -> None:
...
def test_a2_blastfrag_stage_c_component_probability_retained_artifact_pack_cli_writes_manifest(
    tmp_path: Path,
) -> None:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            "tools/maintenance/a2_blastfrag_stage_c_component_probability_retained_artifact_pack.py",
            "--output-dir",
            str(output_dir),
        ],
        cwd=REPO_ROOT,
