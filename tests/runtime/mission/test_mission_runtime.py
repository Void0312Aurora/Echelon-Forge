...
class MissionRuntimeTests(unittest.TestCase):
    ...
    def test_waypoint_nav_products_reuse_cached_route_guidance_within_step(self) -> None:
        scenario = {
            "scenario_name": "waypoint_nav_products_cache_probe",
            "environment": {
                "time_step": 0.05,
                "terrain_type": "legacy",
                "wind": {"speed_mps": 0.0, "dir_from_deg": 0.0, "shear_mps_per_km": 0.0},
        ...
        self.assertTrue(sim.load_database(resolve_repo_path("examples", "config", "database")))
        loader = ScenarioLoader(sim)
        agent_id = loader.load_scenario_data(copy.deepcopy(scenario), seed=61)
        self.assertIsNotNone(agent_id)
        ...
        with mock.patch.object(
            ...
            first = loader._get_waypoint_nav_products(truth=truth, inst=inst)
            second = loader._get_waypoint_nav_products(truth=truth, inst=inst)
