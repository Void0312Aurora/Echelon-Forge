...
# own-ship authoritative truth (health / z / pitch / speed). As of the T8 second
# slice those reads flow through the declared observation view
# (observation_view.own_ship_field), which owns the raw truth access; TL10 is
...
def build_safety_runtime_inputs(
    ...
    *,
    ...
    inputs.health = float(observation_view.own_ship_field(truth, "health", 100.0))
    ...
    inputs.pitch_abs_deg = abs(float(observation_view.own_ship_field(truth, "pitch", 0.0)))
    ...
    inputs.speed_mps = float(observation_view.own_ship_field(truth, "speed", 0.0))
